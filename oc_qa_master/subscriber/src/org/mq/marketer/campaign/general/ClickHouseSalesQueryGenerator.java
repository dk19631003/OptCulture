...
public class ClickHouseSalesQueryGenerator {
	...
	private static String generalSql = 
		...
		"SELECT DISTINCT any(c.cid) FROM contacts c , contacts_mlists cm " +
		"<FROM_TABLES>" +
		"" +
		"" +
	...
	private static String tot_subQuery = " SELECT DISTINCT any(c.cid) FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
						" <FROM_TABLE_FILTERS>  AND (<COND>) ";
	
	private static String email_subQuery = " SELECT DISTINCT any(c.cid) FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
						" <FROM_TABLE_FILTERS>  AND (c.email_id is not null AND c.email_id!='') AND  (<COND>)  ";
	
	private static String mobile_subQuery = " SELECT DISTINCT any(c.cid) FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
						" <FROM_TABLE_FILTERS> AND (c.mobile_phone is not null AND c.mobile_phone !='') AND (<COND>)   ";
	
	
	private static String general_email_subQuery = " SELECT DISTINCT any(c.cid) FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
										" <FROM_TABLE_FILTERS>  AND (c.email_id is not null AND c.email_id!='' AND c.email_status='Active') AND  (<COND>)  ";
	
private static String general_mobile_subQuery = " SELECT DISTINCT any(c.cid) FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
" <FROM_TABLE_FILTERS> AND (c.mobile_phone is not null AND c.mobile_phone !='' AND c.mobile_status='Active'  <MOBILEOPTIN> ) AND (<COND>)   ";


private static String not_received_total =  OCConstants.CHsegmentQryPrefiX+// FROM contacts WHERE cid IN(SELECT DISTINCT c.cid "+
...
private static String not_received_totalCount =  " SELECT DISTINCT any(c.cid) "+
												" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
												"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")"
												+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
												"  WHERE o.contact_id is null  "+
	...
	private static String not_received_email =  OCConstants.CHsegmentQryPrefiX+// FROM contacts WHERE cid IN(SELECT DISTINCT c.cid "+
												" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
												"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
												+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
												"  WHERE o.contact_id is null  "+
	...
	private static String not_received_emailCount =  " SELECT DISTINCT any(c.cid) "+
													" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
													"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")"
													+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
													"  WHERE o.contact_id is null  "+
	...
	private static String not_received_email_status =  OCConstants.CHsegmentQryPrefiX+// FROM contacts WHERE cid IN(SELECT DISTINCT c.cid "+
														" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
														"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
														+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
														"  WHERE o.contact_id is null  "+
...
private static String not_received_email_Count_status =  " SELECT DISTINCT any(c.cid) "+
														" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
														"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")"
														+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
														"  WHERE o.contact_id is null  "+
...
private static String not_received_mobile =  OCConstants.CHsegmentQryPrefiX+// FROM contacts WHERE cid IN(SELECT DISTINCT c.cid "+
											" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
											"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
											+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
											"  WHERE o.contact_id is null  "+
...
private static String not_received_mobileCount =  " SELECT DISTINCT any(c.cid) "+
												" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
												"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")"
												+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
												"  WHERE o.contact_id is null  "+
...
private static String not_received_mobile_status =  OCConstants.CHsegmentQryPrefiX+// FROM contacts WHERE cid IN(SELECT DISTINCT c.cid "+
												" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
												"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
												+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
												"  WHERE o.contact_id is null  "+
...
private static String not_received_mobile_Count_status =  " SELECT DISTINCT any(c.cid) "+
													" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
													"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")"
													+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
													"  WHERE o.contact_id is null  "+
...
private static String generalSqlWithEmailStatusCountForHavenotIsuued ="SELECT DISTINCT any(c.cid)  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND> AND c.cid not in "+promotion_have_not_issued_qury_template+"  <GROUPBYSTR> <HAVINGSTR>";

private static String email_generalSqlWithEmailStatusCountForHavenotIsuued ="SELECT DISTINCT  any(c.cid)  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0  <FROM_TABLE_FILTERS> <COND>  AND c.cid not in  "+promotion_have_not_issued_qury_template
		    + "  AND (c.email_id is not null AND c.email_id!='') AND c.email_status='Active'  GROUP BY c.email_id <GROUPBYSTR> <HAVINGSTR>";

private static String mobile_generalSqlWithEmailStatusCountForHavenotIsuued ="SELECT DISTINCT any(c.cid)  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in "+promotion_have_not_issued_qury_template
		    + " AND (c.mobile_phone is not null AND c.mobile_phone !='') GROUP BY c.mobile_phone <GROUPBYSTR> <HAVINGSTR>";

private static String generalSqlWithEmailStatusForHavenotIsuued = OCConstants.CHsegmentQryPrefiX+"  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in "+promotion_have_not_issued_qury_template+"  <GROUPBYSTR> <HAVINGSTR>";

private static String email_generalSqlWithEmailStatusForHavenotIsuued = OCConstants.CHsegmentQryPrefiX+"  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in  "+promotion_have_not_issued_qury_template
		    + "  AND (c.email_id is not null AND c.email_id!='') AND c.email_status='Active'  GROUP BY c.email_id <GROUPBYSTR> <HAVINGSTR>";

private static String mobile_generalSqlWithEmailStatusForHavenotIsuued = OCConstants.CHsegmentQryPrefiX+"  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in "+promotion_have_not_issued_qury_template
		    + " AND (c.mobile_phone is not null AND c.mobile_phone !='') GROUP BY c.mobile_phone <GROUPBYSTR> <HAVINGSTR>";
...
/*private static String Old_generalSqlWithEmailStatus =
	...
	*/
		...
		OCConstants.CHsegmentQryPrefiX+//FROM contacts WHERE cid IN(SELECT DISTINCT c.cid" +
		" FROM contacts c " +
	...
	" <FROM_TABLE_FILTERS>   <COND> <GROUPBYSTR> <HAVINGSTR>";*/
		...
		//SQL-TO-CH
		private static String generalSqlWithEmailStatus =
				"SELECT DISTINCT any(c.cid) <SELECTFIELDS>" +
				//OCConstants.CHsegmentQryPrefiX+//FROM contacts WHERE cid IN(SELECT DISTINCT c.cid " +
				" FROM contacts c " +
			" <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
			" <FROM_TABLE_FILTERS>   <COND> <GROUPBYSTR> <HAVINGSTR>";
	...
	private static String mobile_generalSqlWithEmailStatusCount =
			...
			" SELECT DISTINCT any(c.cid) FROM(SELECT  any(c.cid)"+
			" FROM contacts c " +
		" <FROM_TABLES> WHERE c.user_id = <USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
...
/*private static String mobile_generalSqlWithMobileStatus =
	...
	private static String mobile_generalSqlWithMobileStatusCount =
			...
			" SELECT DISTINCT c.cid FROM ( SELECT any(c.cid) as cid "+
			" FROM contacts c " +
		" <FROM_TABLES> WHERE c.user_id = <USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
...
//private static String oneCFSqlFormat = "SELECT * FROM contacts WHERE list_id = <LISTID> AND email_status='Active' "; 
	...
	public static HashMap<String, String> ltyConditionMap=new HashMap<String, String>();
		...
		private static String notification_subQuery = " SELECT DISTINCT c.cid FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
		...
		private static String general_notification_subQuery = " SELECT DISTINCT c.cid FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
		" <FROM_TABLE_FILTERS> AND c.push_notification = 1 AND (c.instance_id is not null AND c.instance_id !='' ) AND (c.device_Type IS NOT NULL AND c.device_Type !='') AND (<COND>)   ";

		private static String not_received_notification =  "  SELECT DISTINCT any(c.cid) "+// FROM contacts WHERE cid IN(SELECT DISTINCT c.cid "+
				" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
				"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
				+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
				"  WHERE o.contact_id is null  "+
		...
		private static String not_received_notificationCount =  " SELECT DISTINCT any(c.cid) "+
														"  FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
														"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")"
														+  Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
														"  WHERE o.contact_id is null  "+
		...
		private static String not_received_notification_Count_status =  " SELECT DISTINCT any(c.cid) "+
															" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
															"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")"
															+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
															"  WHERE o.contact_id is null  "+
			...
			private static String notification_generalSqlWithNotificationStatus =
					...
					"SELECT DISTINCT any(c.cid) "+// FROM contacts WHERE cid IN(SELECT DISTINCT c.cid"+
					" FROM contacts c " +
				" <FROM_TABLES> WHERE c.user_id = <USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
...
//				" AND DATEDIFF(DATE_ADD(<FIELD>, INTERVAL -YEAR(<FIELD>)+1 YEAR), '<ZEROYEAR_DATE>' ) % 365 <= <VAL2> ");
		...
		//tokensHashMap.put("date:ignoreyear_is", "(toMonth(<FIELD>) = toMonth(Date('<VAL1>')) AND toDayOfMonth(<FIELD>) = toDayOfMonth(Date('<VAL1>')))");
	...
	public static String generateListSegmentQuery(String queryString, boolean ignoreEmailStatus, String qryType, long mlBit) {
		...
		logger.info(">>>inside generateListSegmentQuery from clickhouse");
		try {
...
//			logger.info(listIdStr+"=FN="+finalStr);
	...
	public static String generateListSegmentCountQuery(String queryString, boolean ignoreEmailStatus, String qryType, long mlBit) {
		...
		//String returnStr=null;		
		logger.info(">>>inside generateListSegmentCountQuery from clickhouse");
		try {
