...
//
	...
	public void addAndStartPurging(Long userId, List<Long> listIds) {
		if(listIds.size()==0) return;
		
		for (Long listId : listIds) {
			addForPurge(userId, listId);
		}
	...
	public void checkForValidDomainByEmailId(Contacts contact) {
		...
		if (retMsg == null || retMsg.equals("Active")) {
...
public  void checkForValidDomainByEmailIdByForm(Contacts contact, FormMapping formMapping, MailingList mailingList  ) {
		...
		if(retMsg == null || retMsg.equals("Active")) {
			...
			//observe the contcat's emailstatus first
			//if it is active no need to change its emailStatus
			//if not try to modify the status based on the optin medium and and all such...
			if(contact.getEmailStatus() != null && contact.getEmailStatus().equalsIgnoreCase(Constants.CONT_STATUS_ACTIVE)){
				contact = validateSuprresed(contact);
				contact = validateUnsubscribe(contact);
				return;
			}
			...
			//contact.setEmailStatus(Constants.CONT_STATUS_ACTIVE);
			//String optinMedium = contact.getOptinMedium();
			if(formMapping.isCheckParentalEmail()) {
				
				contact.setEmailStatus(getContactEmailStatus(contact, mailingList.getCheckDoubleOptin(), formMapping.isCheckParentalEmail()));
