...
public class TermsController extends GenericForwardComposer {
    ...
    public void setEngData() {
		...
		List<String> content = faqDaoForDML.findTermsDataByType("English",userId);
	...
	public void setSpanData() {
		Long userId=user.getUserId();
		List<String> content = faqDaoForDML.findTermsDataByType("Spanish",userId);
		if(content!=null && content.size()>0) {
			bodyContentId.setValue(content.get(0));
		} else {
			bodyContentId.setValue("");
	...
	public void termsContentSave (String messageContent) throws Exception {
		FAQ faq = new FAQ();
		...
		faq.setTermsAndCondition(messageContent);
		faq.setCreatedDate(Calendar.getInstance());
		faqDaoForDML.saveOrUpdate(faq);
	}
	
	public void updateTerms (FAQ faq, String messageContent) throws Exception {
		faq.setOrgId(userOrg.getUserOrgId());
		faq.setUserId(user.getUserId());
		faq.setLanguage(languageTypeLbId.getSelectedItem().getValue().toString());
		faq.setTermsAndCondition(messageContent);
	...
	public void onClickSaveFaqData$jsonData(ForwardEvent event) throws Exception {
		...
		if(languageTypeLbId.getSelectedItem().getValue().toString().equalsIgnoreCase("English")) {
			if(eng==null) {
				termsContentSave(htmlQuill);
			} else {
				updateTerms(eng,htmlQuill);
			}
		}
