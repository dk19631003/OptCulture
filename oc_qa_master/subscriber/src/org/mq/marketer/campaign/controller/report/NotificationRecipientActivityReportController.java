...
public class NotificationRecipientActivityReportController extends GenericForwardComposer{
	...
	private final String STATUS_DELIVERED = "Sent";
	...
	private final String TYPE_FAILURE ="Failure";
	public NotificationRecipientActivityReportController(){
	...
	public void doAfterCompose(Component comp){
		try{
			...
			logger.debug("---just entered----in SMSRecipientReport");
			...
			tBarBtn = new Toolbarbutton();
			tBarBtn.setLabel(TYPE_FAILURE);
			tBarBtn.setStyle("font-weight:bold;color:#43A0BA;");
			tBarBtn.setAttribute("addEvent", TYPE_FAILURE);
			tBarBtn.addEventListener("onClick", this);
			tBarBtn.setParent(toolbarButtonHboxId);
		...
		//	getClicks();
			...
			/*List<Object[]> catList = smsCampaignSentDao.getAllSentCategories(smsCampaignReport.getSmsCrId());
			int count = 0; 
			for(Object[] category : catList) {
				if(category[0] != null && !((String) category[0]).equalsIgnoreCase(STATUS_DELIVERED)) {
					if(count<4) {
							tBarBtn = new Toolbarbutton();
		  ...
		  if(catList.size()>5) {
			  	tBarBtn = new Toolbarbutton();
			  	tBarBtn.setLabel(TYPE_OTHERS);
				tBarBtn.setStyle("font-weight:bold;color:#43A0BA");
				tBarBtn.setAttribute("addEvent", TYPE_OTHERS);
				tBarBtn.addEventListener("onClick", this);
				tBarBtn.setParent(toolbarButtonHboxId);
		  }*/
		}catch (Exception e) {
	 ...
	 public void onClick$exportBtnId() {
		...
		public void export(Combobox fileType) throws Exception{
			...
			if(type.equalsIgnoreCase("csv")){
		        ...
		        long totalContacts  = notificationCampaignReport.getClicks();
		        int size = 1000;
		        ...
		        if(!isOpenedClicked) {
		        	...
		        	totalContacts  = notificationCampaignSentDao.getCount(notificationCampaignReport.getNotificationCrId(), statusType);
		        }
				...
				for(int i=0;i < totalContacts; i+=size) {
					...
					if(!isOpenedClicked) {
						rcptRepList = notificationCampaignSentDao.getRepByStatus(notificationCampaignReport.getNotificationCrId(), statusType, i, size);
						...
						if(isOpenedClicked) {
							//rcptRepList = notificationCampaignSentDao.getOpenedMobilesByCrId(notificationCampaignReport.getNotificationCrId(), i, size);
						//}else{
							
							rcptRepList = notificationCampaignSentDao.getClickedMobilesByCrId(notificationCampaignReport.getNotificationCrId(), i, size);
						}
				 ...
				 Filedownload.save(sb.toString().getBytes(), "text/plain", "NotificationRecipientActivityReport.csv");
		...
		private void getPending() {
			...
			int totCount = notificationCampaignSentDao.getCount(notificationCampaignReport.getNotificationCrId(), (String)repPagingId.getAttribute("type"));
...
private void getFailed() {
		...
		repPagingId.setAttribute("type","'"+TYPE_FAILURE+"'");
		int totCount = notificationCampaignSentDao.getCount(notificationCampaignReport.getNotificationCrId(), (String)repPagingId.getAttribute("type"));
	...
	*/
	...
	public void getClicks(){
		...
		int clickCount = notificationCampaignReport.getClicks();
		
		//List<Object[]> list = notificationCampaignSentDao.getClickedMobilessByCrId(notificationCampaignReport.getNotificationCrId(),start_index,_size); 
		...
		super.onEvent(event);
			...
			if(event.getTarget() instanceof Toolbarbutton){
				...
				if(toolBarBtnAttr.equalsIgnoreCase(STATUS_DELIVERED)){
					...
					int totCount = notificationCampaignSentDao.getCount(notificationCampaignReport.getNotificationCrId(), type);
					repPagingId.setTotalSize(totCount);
		...
		try {
			...
			for(;count>0;count--){
			 ...
			 if(type.equalsIgnoreCase("opened")) {
				...
				//List<Object[]> list = notificationCampaignSentDao.getOpenedMobilesByCrId(notificationCampaignReport.getNotificationCrId(),start_index,_size);
				...
				List<Object[]> list = notificationCampaignSentDao.getClickedMobilesByCrId(notificationCampaignReport.getNotificationCrId(),start_index,_size);
				...
				List<Object[]> list = notificationCampaignSentDao.getRepByStatus(notificationCampaignReport.getNotificationCrId(),type, start_index,_size);
				logger.debug("the list size is====>"+list.size());
				Listitem li;
				for (Object[] obj : list) {
	...
	private void doPopUp(long sentId,String mobileNumber,String type){
		try {
			...
			if(type.equals(TYPE_CLICKED)){
				...
				List<Object[]> clickList = notificationClicksDao.getClickInfoBySentId(sentId);
				Listitem li ;
				Listcell lc ;
				Label lbl ;
	...
	public int checkForPendingAndStatusPending(String status){
		int count = notificationCampaignSentDao.getCount(notificationCampaignReport.getNotificationCrId(),status);
