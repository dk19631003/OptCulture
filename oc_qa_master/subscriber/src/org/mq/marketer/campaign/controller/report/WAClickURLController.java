...
import org.mq.marketer.campaign.dao.WACampaignUrlsDao;
...
public class WAClickURLController extends GenericForwardComposer implements EventListener {
	...
	private WACampaignUrlsDao waCampaignUrlsDao;
	...
	public void defaultSettings(){
		...
		try {
			...
			WACampaignReport waCr = (WACampaignReport)sessionScope.getAttribute("waCampaignReport");
			if(waCr != null){
				try {
					Calendar tempCal1 = (Calendar)Utility.makeCopy(waCr.getSentDate());
					Calendar tempCal2 = (Calendar)Utility.makeCopy(waCr.getSentDate());
			...
			if(waCampaignReport == null){
				...
				waCampaignReport = (WACampaignReport)desktop.getAttribute("waCampaignReport");
			}
			
			Long waCrid= waCampaignReport.getWaCrId();
			List<WACampaignUrls> waCampUrlsList = waCampaignUrlsDao.getClickList(waCrid);
			 
		/*	clicksPagingId.setActivePage(0);
			clicksPagingId.setTotalSize(waCampUrlsList.size());
			clicksPagingId.addEventListener("onPaging", this);*/
			 
			getWAClickReport(waCampUrlsList);
	...
	public void  getWAClickReport( List<WACampaignUrls> waCampUrlsList){
		...
		try {
			...
			Long waCrid= waCampaignReport.getWaCrId();
			 ...
			 List<Object[]> waClicksList = waClicksDao.getClicksByCampUrlId();
			 
			 
			 
			 
			for (WACampaignUrls waCampUrls : waCampUrlsList) {
					...
					if(waCampUrls.getTypeOfUrl().equals(OCConstants.SHORTURL_TYPE_BARCODE_TYPE_MULTIPLE) || 
							waCampUrls.getTypeOfUrl().equals(OCConstants.SHORTURL_TYPE_BARCODE_TYPE_SINGLE)){
						...
						if(waCampUrls.getShortUrl().trim() != null && !waCampUrls.getShortUrl().trim() .isEmpty() ){
							
							
							originalUrl+=waCampUrls.getOriginalUrl().trim();
							//shortUrl+=waCampUrls.getShortUrl().trim();
						}
					...
					Label shortUrlLbl= new Label(waCampUrls.getShortUrl());
					...
					for (Object[] objects : waClicksList) {
						
						if(((Long)objects[0]).longValue() == waCampUrls.getId().longValue()){
	...
	public void setFooter(Footer totClicksLBFId) {
			...
			try {
				...
				long crId = waCampaignReport.getWaCrId();
				//long count = WaCampaignUrlsDao.getClickCountByCrId(crId).longValue();
				long count=waCampaignSentDao.findTotClicksByCrid(crId);
			...
			if(option.equals("0")) {
				...
				if(clientFromDtCal==null ) {
					/*MessageUtil.setMessage("Selected date should be later than email sent date.",
							"color:red", "top");*/
					MessageUtil.setMessage("Selected date should be later than WA sent date.",
							"color:red", "top");
					return;
				} 
		...
		if(type.equalsIgnoreCase("csv")){
	        ...
	        if(clicksRateLbId.getItemCount()==0){
	        	try {
					...
					MessageUtil.setMessage("No records for clicks over time found for this Wa campaign to export.", "color:blue");
				} catch (Exception e) {
				}
	        	return;
	        }
	        ...
	        Filedownload.save(sb.toString().getBytes(), "text/plain", "WaClicksReports.csv");
		}
	} // fileDownload
