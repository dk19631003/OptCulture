...
public class ViewHomesPassedSegmentsController extends GenericForwardComposer implements EventListener{
	...
	public void exportCSV(String ext, Detail detail) {
		...
		try {
			...
			String selQry = "SELECT any(h.hp_id) "+ subQry + segmentQry+" "+grpByQry;
			...
			if(!currentUser.isEnableClickHouseDBFlag())
				...
				totalContacts = contactsDao.getSegmentedContactsCountFromCH(selQry);
	...
	public void exportCustomerCSV(String ext, Detail detail) {
		...
		try {
			...
			String selQry = "SELECT any(c.cid) "+ subQry + segmentQry+" "+grpByQry;
