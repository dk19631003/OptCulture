...
public class ManageUsersAdminController extends GenericForwardComposer {
	...
	private Button step1NxtBtnId, BackBtnId;
	private Button step1SaveBtnId;

	private Textbox searchBoxId;
	private Listbox userByLbId, filterOptionLbId, orgListBxId, orgListBx, usersListBxId;
...
//	List<Users> usersList;
	...
	Groupbox createListGroupBoxId, manageUserListGroupBoxId;
	...
	public ManageUsersAdminController() {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		creditsDivId.setVisible(true);

	}

	public void selectedOrganizationUsers() {

		domainLbId.getItems().clear();
		RoleLbId.getItems().clear();
		customersGridId.setRowRenderer(renderer);
		customersGridId.setModel(new ListModelList());
		Long orgId = currentUser.getUserOrganization().getUserOrgId();
		if (session.getAttribute("selectedUserOrgId") != null)

			orgId = (Long) session.getAttribute("selectedUserOrgId");

		List<UsersDomains> domainsList = usersDao.getAllDomainsByUserOrgId(orgId);
	...
	private void setUserOrg() {
		...
		for (UserOrganization userOrganization : orgList) {
			...
			if (userOrganization.getOrganizationName() == null
					|| userOrganization.getOrganizationName().trim().equals(""))
				continue;
	...
	private void setUserOrgID() {
		...
		for (UserOrganization userOrganization : orgList) {
			...
			// set Organization Name
			if (userOrganization.getOrgExternalId() == null || userOrganization.getOrgExternalId().trim().equals(""))
				continue;
	...
	public void onSelect$orgListBxId() {
		...
		for (Listitem orgIdItem : orgListBx.getItems()) {
			...
			if (orgIdItem.getValue() != null && orgIdItem.getValue().equals(orgId)) {
				...
				session = Sessions.getCurrent();
				session.setAttribute("selectedUserOrgId", orgId);
				break;
			}
	...
	public void onSelect$orgListBx() {
		...
		for (Listitem orgIdItem : orgListBxId.getItems()) {
			...
			if (orgIdItem.getValue() != null && orgIdItem.getValue().equals(orgEId)) {
				orgIdItem.setSelected(true);
				session = Sessions.getCurrent();
				session.setAttribute("selectedUserOrgId", orgEId);
				break;
			}
	...
	public void onClick$saveDomainBtnId$createDomainWinId() {
		...
		Long orgOwnerId = usersDao.getOwnerofOrg(orgId);
		...
		try {
			...
			int usersDomains = usersDomainsDao.findBydomainName(examineDomName,
					orgOwner.getUserOrganization().getUserOrgId());// user name should be unique accross the
																				// organization
			if (usersDomains != 0) {
				createDomainWinId$msgLblId
		...
		usersDomains.setDomainManagerId(orgOwner);
		usersDomains.setDomainName(domainName);
		usersDomains.setUserOrganization(orgOwner.getUserOrganization());
		...
		//orgId = orgOwner.getUserOrganization().getUserOrgId();
		// if (session.getAttribute("selectedUserOrgId") != null)
		...
		orgOwner.setUserDomains(currDomainSet);
		// usersDao.saveOrUpdate(currentUser);
		usersDaoForDML.saveOrUpdate(orgOwner);
			...
			if (confirm == Messagebox.OK) {
				...
				try {
					if (user == null && type == null) {
						...
						userName = userName + "__org__" + orgOwner.getUserOrganization().getOrgExternalId();
						...
						user.setloyaltyServicetype(orgOwner.getloyaltyServicetype());

					}

					int availableCredits = orgOwner.getEmailCount() - orgOwner.getUsedEmailCount();
					...
					if (givenCredits != null) {
						...
						orgOwner.setUsedEmailCount(orgOwner.getEmailCount() - availableCredits);
						// usersDao.saveOrUpdate(currentUser);
						usersDaoForDML.saveOrUpdate(orgOwner);
					}
					...
					if (smsDivId.isVisible()) {
						...
						int smsAvailableCount = orgOwner.getSmsCount() - orgOwner.getUsedSmsCount();
						...
						if (givenSMSCredits != null) {
							...
							orgOwner.setUsedSmsCount(orgOwner.getSmsCount() - smsAvailableCount);
							// usersDao.saveOrUpdate(currentUser);
							usersDaoForDML.saveOrUpdate(orgOwner);
						}
					...
					// ******************************
					user = setUserData(user, orgOwner);
	...
	public void userSettings() {
		...
		if (manageUsersTabBoxId.getSelectedIndex() == 1) {
			...
			type = (String) session.getAttribute("manageUserAdminType");
			user = (Users) session.getAttribute("manageUserAdmin");
...
//							RoleLbId.setDisabled(true);
				...
				/*
				 * if (RoleLbId !=
				 * null&&RoleLbId.getSelectedItem().getValue().equals("StoreOperator")) {
				 * RoleLbId.getSelectedItem().setValue("Store Operator"); //
				 * userRolesList.addAll(userRolesNameList); }
				 */
...
//					rePasswordTbId.setReadonly(true);
		...
		try {
			...
			try {
				...
				int confirm = Messagebox.show(
						"Are you sure you want to " + msg + " the user : " + user.getUserName() + "?", msgHeader,
	...
	public void onClick$editUserBtnId() {
		...
		Users userObj = (Users) session.getAttribute("manageUserAdmin");
...
//		rePasswordTbId.setReadonly(true);
	...
	public void onSelect$manageUsersTabBoxId() {
		...
		type = (String) session.removeAttribute("manageUserAdminType");
		user = (Users) session.removeAttribute("manageUserAdmin");
		...
		public void onEvent(Event evt) throws Exception {
			...
			if (obj instanceof Image) {
				...
				if (evtType.equalsIgnoreCase("userView")) {
					...
					session.setAttribute("manageUserAdminType", "view");
					session.setAttribute("manageUserAdmin", user);
					manageUsersTabBoxId.setSelectedIndex(1);
					userSettings();

				} else if (evtType.equalsIgnoreCase("userEdit")) {

					session.setAttribute("manageUserAdminType", "edit");
					session.setAttribute("manageUserAdmin", user);
					manageUsersTabBoxId.setSelectedIndex(1);
					userSettings();
					...
					try {
						...
						/*
						 * Redirect.goTo(PageListEnum.EMPTY);
						 * Redirect.goTo(PageListEnum.ADMIN_MANAGE_USERS);
						 */
						selectedOrganizationUsers();
						// Executions.getCurrent().getDesktop().invalidate();
					} catch (Exception e) {
