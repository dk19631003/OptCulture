...
public class ManageAutoSmsController extends GenericForwardComposer{
	...
	private Listbox typeOfAutoSMSListLbId,insertCouponLbId,insertKeywordLbId,insertMergetagsLbId,multipleSenderIdsLbId,insertValueCodeLbId; // 4083
	...
	private Div outBoundedDivId,multipleSenderIdDivId,insertKeywrdDivId,insertCouponDivId,insertValueCodeDivId; //4083
	...
	private List<ValueCodes> listOfValueCodes;
	...
	//		long orgId =  currentUser.getUserOrganization().getUserOrgId();
			listOfValueCodes = valueCodesDao.findValueCodeBy(currentUser.getUserOrganization().getUserOrgId());
			if(listOfValueCodes.size() >= 1){
				populateValueCodes();//  list of valuecodes
			}
	...
	private void populateValueCodes() {
		// TODO Auto-generated method stub
		Components.removeAllChildren(insertValueCodeLbId);
		Listitem aValCodeListItem = null;
		
		aValCodeListItem = new Listitem("Insert Value Codes");
		aValCodeListItem.setParent(insertValueCodeLbId);

		List<ValueCodes> valueCodesList = listOfValueCodes;
		for(ValueCodes valueCode : valueCodesList){
			aValCodeListItem = new Listitem(valueCode.getValuCode());
			aValCodeListItem.setValue(valueCode.getValuCode());
			aValCodeListItem.setParent(insertValueCodeLbId);
		}

		if(insertValueCodeLbId.getChildren().size() >=1 ){
			insertValueCodeDivId.setVisible(true);
			insertValueCodeLbId.setSelectedIndex(0);
			onSelect$insertValueCodeLbId();

		}else{
		
			insertValueCodeDivId.setVisible(false);
		}
	...
	public void onSelect$insertValueCodeLbId() {

		if(insertValueCodeLbId.getSelectedIndex() <= 0) {
			logger.debug("selected 0");
			return ;
		}
		String selValCode = insertValueCodeLbId.getSelectedItem().getValue();
		String replaceStr = Constants.CF_START_TAG+Constants.VALUECODE_TOKEN+selValCode
	...
	public void onClick$addNewValueCodeTBId() {
		Redirect.goTo(PageListEnum.LOYALTY_VALUE_CODES);

	}//onClick$addNewValueCodeTBId()
