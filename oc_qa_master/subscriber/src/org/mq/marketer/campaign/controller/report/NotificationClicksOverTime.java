...
public class NotificationClicksOverTime extends GenericForwardComposer {
	...
	public NotificationClicksOverTime() {
		this.sessionScope = Sessions.getCurrent();
		notificationCampaignReport = (NotificationCampaignReport) Sessions.getCurrent()
				.getAttribute("notificationCampReport");
	}
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		NotificationCampaignReport cr = (NotificationCampaignReport) sessionScope
				.getAttribute("notificationCampReport");
		if (cr != null) {
			try {
	 ...
	 * (TimeZone)sessionScope.getAttribute("clientTimeZone"); Calendar sentDate =
	 * campaignReport.getSentDate();
	 * 
	 * tempHourShiftInt =
	 * TimeZone.getDefault().getOffset(sentDate.getTimeInMillis()); tempHourShiftInt
	 * = tempHourShiftInt - clientTimeZone.getOffset(sentDate.getTimeInMillis());
	 * tempHourShiftInt = tempHourShiftInt/(1000*60*60);
	 * 
	 * drawChart(null, null); }
	 */
		...
		try {
			...
			if (option.equals("0")) {
				...
				if (clientFromDtCal == null) {
					MessageUtil.setMessage("Selected date should be later than Push Notification sent date.", "color:red", "top");
					return;
				}
...
//				logger.debug("====serverFromDateCal= "+fromDatebox.getServerValue());
	...
	public Chart drawChart(Calendar startDate, Calendar endDate) {
		...
		try {
			...
			/*
			 * if (opensDao == null) opensDao = (OpensDao) SpringUtil.getBean("opensDao");
			 */
			...
			int stHour = 0;
			if (startDate != null)
				stHour = startDate.get(Calendar.HOUR_OF_DAY);
			...
			// opensDao.getOpenRateByCrId(notificationCampaignReport.getCrId(),startDateStr,
			// endDateStr) ;
			 ...
			 * 
			 * for (Object[] objects : openRates) { for (Object obj : objects) {
			 * logger.debug("  "+obj); } logger.info("\n-----------"); } }
			 */

			if (notificationClicksDao == null)
				notificationClicksDao = (NotificationClicksDao) SpringUtil.getBean("notificationClicksDao");
			List<Object[]> clickRates = notificationClicksDao
					.getClickRateByCrId(notificationCampaignReport.getNotificationCrId(), startDateStr, endDateStr);
			if (logger.isDebugEnabled())
				logger.debug(" Click rate List size :" + clickRates.size());
			...
			for (int i = 0; i < xScale.length; i++) {
				 ...
				 * 
				 * if (temp == xScale[i] && temp >= openRateFirstCol) { totalOpenData[i] =
				 * Integer.parseInt(obj[1].toString()); uniqueOpenData[i] =
				 * Integer.parseInt(obj[2].toString()); break; } } // for obj
				 */
				for (Object[] obj : clickRates) {
					temp = Integer.parseInt(obj[0].toString());
					// temp = (24+temp-tempHourShiftInt)%24;

					if (clickRateFirstCol == -1 && isFistAndSingleDay)
						clickRateFirstCol = temp;
			...
			for (int i = 0; i < xScale.length; i++) {
				...
				/*
				 * lc = new Listcell(xScale[i] + ":00 - " + ((xScale[i] + 1) == 24 ? 0 :
				 * (xScale[i] + 1)) + ":00");
				 */
				lc = new Listcell(xScale[i]+":00 - "+((xScale[i]+1)==24?0:(xScale[i]+1)) +":00");
				lc.setParent(li);
				/*
				 * lc = new Listcell("" + uniqueOpenData[i]); lc.setParent(li); lc = new
				 * Listcell("" + totalOpenData[i]); lc.setParent(li);
				 */
	...
	public Chart consolidatedChart(Calendar startDate, Calendar endDate) {
		...
		try {
			...
			// List<Object[]> openRates =
			// opensDao.getOpenRateByCrId(campaignReport.getCrId(), startDateStr,
			// endDateStr);

			/*
			 * if (logger.isDebugEnabled()) { logger.debug(" Open rate List size :" +
			 * openRates.size());
			 * 
			 * 
			 * for (Object[] objects : openRates) { for (Object obj : objects) {
			...
			if (notificationClicksDao == null)
				notificationClicksDao = (NotificationClicksDao) SpringUtil.getBean("notificationClicksDao");
			List<Object[]> clickRates = notificationClicksDao.getClickRateByCrId(notificationCampaignReport.getNotificationCrId(), startDateStr,
					endDateStr);
			if (logger.isDebugEnabled())
				logger.debug(" Click rate List size :" + clickRates.size());
			...
			Calendar tempSentDate = MyCalendar.string2Calendar(
					MyCalendar.calendarToString(sentDate, MyCalendar.FORMAT_DATETIME_STYEAR, clientTimeZone));
			...
			logger.info(
					"Hour according to tempSentDate calendar is ===========" + tempSentDate.get(Calendar.HOUR_OF_DAY));
			logger.info("Hour according to tempSentDate calendar is ==========="
					+ tempSentDate.get(MyCalendar.HOUR_OF_DAY));
			...
			for (int i = 0; i < xScale.length; i++) {
				...
				/*
				 * for (Object[] obj : openRates) { temp = Integer.parseInt(obj[0].toString());
				 * logger.info("temp before shift====" + temp); // temp =
				 * (24+temp-tempHourShiftInt)%24; logger.info("temp after shift====" + temp);
				 * 
				 * if (openRateFirstCol == -1 && isFistAndSingleDay) openRateFirstCol = temp;
				 * 
				 * if (temp == xScale[i] && temp >= openRateFirstCol) { totalOpenData[i] =
				...
				//logger.info("x axis after adding temphourshift ===" + (24 + xScale[i] + tempHourShiftInt) % 24,
						//totalOpenData[i]);
				logger.info("y axis after adding temphourshift  ===" + (24 + xScale[i] + tempHourShiftInt) % 24,
						totalClickData[i]);
			...
			for (int i = 0; i < xScale.length; i++) {
				...
				if (totalClickData[i] == 0)
					continue;

				li = new Listitem();
				li.setParent(opensClicksRateLbId);
				// lc = new Listcell(xScale[i]+":00 - "+((xScale[i]+1)==24?0:(xScale[i]+1))
				// +":00");
				/*
				 * lc = new Listcell(xScale[i]+tempHourShiftInt +":00 - "+
				 * ((xScale[i]+tempHourShiftInt+1)>=24? (xScale[i]+tempHourShiftInt+1)-24:
				 * (xScale[i]+tempHourShiftInt+1)) +":00");
				 */
				lc = new Listcell(((xScale[i] + tempHourShiftInt) >= 24 ? (xScale[i] + tempHourShiftInt - 24)
						: (xScale[i] + tempHourShiftInt)) + ":00 - "
						+ ((xScale[i] + tempHourShiftInt + 1) >= 24 ? (xScale[i] + tempHourShiftInt + 1) - 24
								: (xScale[i] + tempHourShiftInt + 1))
						+ ":00");
				lc.setParent(li);
				/*
				 * lc = new Listcell("" + uniqueOpenData[i]); lc.setParent(li); lc = new
	...
	public void fileDownload(Combobox fileType) throws Exception {
		...
		if (type.equalsIgnoreCase("csv")) {
			...
			if (opensClicksRateLbId.getItemCount() == 0) {
				try {
					// Messagebox.show("No records for opens & clicks over time found for this
					// campaign to export.");
					MessageUtil.setMessage("No records for opens & clicks over time found for this campaign to export.",
							"color:blue");
				} catch (Exception e) {
				}
				return;
			...
			Filedownload.save(sb.toString().getBytes(), "text/plain", "NotificationClicksReports.csv");
		}
	} // fileDownload
