...
public class BeeAutoEmailController extends MyTempEditorController {
	...
	public BeeAutoEmailController() throws JSONException {
		...
		frmSrc= Sessions.getCurrent().getAttribute("fromPage")!=null ? (String)Sessions.getCurrent().getAttribute("fromPage") : Constants.STRING_NILL ;
	...
	private void saveInMyTemplatesAutoEmail(Window createCustTemplateWinId, MyTemplates myTemplates) {
		...
		try{
			...
			try {
				...
				if(frmSrc != null && !
						frmSrc.isEmpty()){
					if(frmSrc.contains("loyaltyPrgmOverview")) 
					Redirect.goTo(PageListEnum.LOYALTY_PROGRAM_OVERVIEW);
					if(frmSrc.contains("loyaltyAutoCommunication"))
					Redirect.goTo(PageListEnum.LOYALTY_AUTO_COMMUNICATION); 
					if(frmSrc.contains("manageAutoEmails"))
