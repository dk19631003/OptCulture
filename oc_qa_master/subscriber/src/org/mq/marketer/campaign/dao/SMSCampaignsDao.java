...
//				"where m.users=" + userId + " order by m.createdDate";
 ...
 public List<SMSCampaigns> findIfSegmentAssociates(String searchStr, Long segId){
 	/*
	 String hql = "FROM SMSCampaigns WHERE listType='"+searchStr+"'";
	 	...
	 	return null;*/
	 String hql = "FROM SMSCampaigns WHERE  listType like '"+searchStr+"' OR listType like 'S%:"+segId+",%' OR listType like 'S%,"+segId+",%' OR listType like 'S%,"+segId+"' ";

 	logger.info("hql query"+hql);
 	
 	List<SMSCampaigns> segAssocitedSmsCampList = executeQuery(hql);
 	
 	if(segAssocitedSmsCampList.size() > 0) {
 		
 		return (List<SMSCampaigns>)segAssocitedSmsCampList;
 ...
 public List<SMSCampaigns> getCampaignsByCampaignName(String campaignName,Long userId, int firstResult, int maxResult,String orderby_colName,String desc_Asc) throws Exception{
	...
	/**
	    * This method get's List of all active sms campaigns.
	    * @param userId
	    * @param status
	    * @return SMSCampaigns
	    */
	 public List<SMSCampaigns> getSMSCampaignsByUserIDStatus(Long userId, String status){
		 String hql = "FROM SMSCampaigns WHERE users IN ( "+userId+" ) and status in ('" + status + "','Running')" ;

		 	logger.info("hql query"+hql);
		 	
		 	List<SMSCampaigns> activeSmsCampList = executeQuery(hql);
		 	
		 	if(activeSmsCampList.size() > 0) {
		 		
		 		return (List<SMSCampaigns>)activeSmsCampList;
