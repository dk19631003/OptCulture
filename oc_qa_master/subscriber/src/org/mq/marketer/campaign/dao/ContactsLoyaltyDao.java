...
*/
	...
	public List<Object[]> getTotalMemberships(Long userId, Long prgmId, String startDateStr,
			...
			logger.info("Total Memberships:"+query);
...
public List<Object[]> getStoreContactLtyList(Long prgmId, Long userId, String startDate, String endDate,boolean isMobile, String type) {
		...
		logger.info("Store Enrolments query:"+query);  
		List<Object[]> tempList = getHibernateTemplate().find(query);
		  if(tempList != null && tempList.size() > 0) return tempList;
		  return null;
	}
...
public ContactsLoyalty findBydate(Long userId, String emailID, String mobilePhone,String date ){
	...
	//modify
	public List<Object[]> getAllOCMembershipsForComm(Long userId, Long programId, long loyaltyId){//APP-4728
		...
		try {
			...
			queryStr = "SELECT   cl, c.emailId, c.mobilePhone FROM ContactsLoyalty cl, Contacts c "
					...
					+ " AND cl.membershipStatus = '"+OCConstants.LOYALTY_MEMBERSHIP_STATUS_ACTIVE+"' "
							+ "AND (c.emailId IS NOT NULL OR c.mobilePhone IS NOT NULL) AND cl.loyaltyId > "+loyaltyId+" ORDER BY cl.loyaltyId ASC limit 10000 ";
			logger.info("getAllOCMembershipsForComm ===== "+queryStr);
			/*Query query = getSession().createQuery(queryStr);
			query.setFirstResult(currentSizeInt);
			query.setMaxResults(offset);*/
...
public List<ContactsLoyalty> getAllOCMemberships(Long userId, Long programId, long loyaltyId){//APP-4728
		...
		try {
			...
			queryStr = " FROM ContactsLoyalty WHERE userId="+userId+" AND programId="+programId+"  AND serviceType = '"+OCConstants.LOYALTY_SERVICE_TYPE_OC+"'"
					+ " AND membershipStatus = '"+OCConstants.LOYALTY_MEMBERSHIP_STATUS_ACTIVE+"' AND loyaltyId > "+loyaltyId+" ORDER BY loyaltyId ASC LIMIT 10000 ";
