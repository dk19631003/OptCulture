...
public class RetailProSalesDao extends AbstractSpringDao {
...
//    	super.saveOrUpdateAll(retailProSalesCollection);
    ...
    public List findRecordsByRecieptNumber(String recieptNum ,Long userId) {
		try {
   ...
   public List<Map<String, Object>> findStoreRedemptionList(long userId, int start_Idx, int endIdx, String promoCode, String fromDateString, String toDateString, String orderby_colName, String desc_Asc) {
	...
	public List<Map<String, Object>> findPurchaseDetBasedOnCid(Long contactId, Long userId, String limit) {
		...
		try {
			...
			String qry = "";
			if(limit.trim().equals(Constants.LIMIT)) {
				qry = "SELECT reciept_number,ROUND(sum((quantity*sales_price)+tax-(IF(discount is null,0,discount))),2 ) as SALES_PRICE, DATE_FORMAT(sales_date, '%Y-%m-%d %H:%i:%s') as sales_date  " +
					  " FROM retail_pro_sales " +
					  " where user_id ="+userId.longValue()+" and  cid ="+contactId.longValue() +" " +
					  " group by sales_date,reciept_number order by sales_date desc limit 5 ";
				...
				qry = "SELECT reciept_number,ROUND(sum((quantity*sales_price)+tax-(IF(discount is null,0,discount))),2) as SALES_PRICE, DATE_FORMAT(sales_date, '%Y-%m-%d %H:%i:%s') as sales_date " +
					  " FROM retail_pro_sales " +
					  " where user_id ="+userId.longValue()+" and  cid ="+contactId.longValue() +" " +
					  " group by sales_date,reciept_number order by sales_date desc ";
				//list  = getHibernateTemplate().find("from RetailProSalesCSV  where listId ="+list_id+" and  customerId ='"+customerId +"'  order by sales_date desc ");
			}
	...
	public List<Map<String, Object>> findSkuDetBasedOnCid(Long contactId, long userId, String limit) {
		...
		try {
			...
			if(limit.equals(Constants.LIMIT) ) {
				qry =" SELECT sales.reciept_number, sales.sku, sku.item_category, sum(sales.quantity) as qty, DATE_FORMAT(sales.sales_date, '%Y-%m-%d %H:%i:%s') as sales_date " + 
...
//			List<Object[]> promoCodeList = null;
			...
			if(limit.equals(Constants.LIMIT) ) {
				qry ="SELECT sales.promo_code, DATE_FORMAT(sales.sales_date, '%Y-%m-%d %H:%i:%s') as sales_date, ROUND((sales.quantity*sales.sales_price)+sales.tax-(IF(sales.discount is null,0,sales.discount)),2) as price, sku.store_number " + 
					 " FROM retail_pro_sku sku, retail_pro_sales sales  " +
				...
				qry ="SELECT sales.promo_code, DATE_FORMAT(sales.sales_date, '%Y-%m-%d %H:%i:%s') as sales_date, ROUND((sales.quantity*sales.sales_price)+sales.tax-(IF(sales.discount is null,0,sales.discount)),2) , sku.store_number " + 
					 " FROM retail_pro_sku sku, retail_pro_sales sales  " +
					 " where  sales.user_id ="+userId.longValue()+"  AND sku.user_id ="+userId.longValue()+"  and sku.sku_id=sales.inventory_id " +
					 " and sales.cid= "+contactId.longValue()+" and sales.promo_code is not null " +
		...
		try {
			skuListArr = jdbcTemplate.query(qry, new RowMapper() {
				public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					object[2] = rs.getDouble(3); // sales price
					double qty= rs.getDouble(4); // quantity
					//APP-3935
					DecimalFormat df= new DecimalFormat("#.####");
					object[3]=df.format(qty);
					return object;
				}
	...
	public Object[] getTotalRevenueAndNoOfPurchase(Long userId,String startDate,String endDate, boolean isTaxIncluded) {
		
		List<Object[]> optintelActivityArr = null;
		String calcRevenue = Constants.STRING_NILL;
		if(isTaxIncluded) {
			calcRevenue = "+ tax";
		}
	/*	String qry ="SELECT  ROUND(SUM((quantity*sales_price) + tax-(IF(discount is null,0,discount))),2) , COUNT(distinct doc_sid) FROM  retail_pro_sales  " +
		...
		String qry ="SELECT  ROUND(SUM((quantity*sales_price)"+calcRevenue+" -(IF(discount is null,0,discount))),2) , "
				+ "COUNT(distinct doc_sid)  FROM  retail_pro_sales " +
				" WHERE user_id="+ userId + " AND sales_date >='" + startDate + "' AND sales_date <='"+ endDate + "'" ;
	...
	public Object[] getPromosTotalAndRevenue(Long userId,String startDate,String endDate,boolean isTaxIncluded) {
		...
		String qry ="SELECT  ROUND(SUM((quantity*sales_price)" +calcRevenue+"-(IF(discount is null,0,discount))),2)," + "COUNT(distinct reciept_number) FROM  retail_pro_sales " +"WHERE user_id="+ userId + " AND sales_date >= '"+ startDate +"' AND sales_date <= '"+ endDate +"' AND promo_code IS NOT NULL";	
		logger.info("getPromosTotalAndRevenue >>>>>"+qry);
		optintelActivityArr = jdbcTemplate.query(qry, new RowMapper() {
			 public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
...
//		String qury = "";
	...
	public List getMissingReceiptNumbers(Long userId, String from, String to, int firstCount, int count) {
	...
	*/
	...
	public String findLastpurchasedAmount(Long  cid, Long userId) {
	   ...
	   public List<Object[]> getTierVisitsAndRevenue(Long userId,
		   ...
		   String qry = "";
		   qry = "SELECT  COUNT(DISTINCT sales.doc_sid),"
		   		+ "ROUND(SUM((sales.quantity*sales.sales_price)+ sales.tax -(IF(sales.discount is null,0,sales.discount))),2), cl.program_tier_id " +
					   ...
					   " AND sales.sales_date BETWEEN '" + startDateStr + "' AND '"+ endDateStr + "' Group by cl.program_tier_id" ;
	   ...
	   public List<Object[]> getTierVisitsAndRevenue(Long userId,
		   ...
		   logger.info("Kpis Report table:"+qry);
		   List<Object[]> tempList = null;
	   ...
	   public List<Object[]> getStorelevelKPI(Long userId,
		   ...
		   String qry = "";
		   qry = "SELECT  sales.store_number,COUNT(DISTINCT sales.doc_sid),ROUND(SUM((sales.quantity*sales.sales_price)+ sales.tax -(IF(sales.discount is null,0,sales.discount))),2), sales.subsidiary_number " +
					   " FROM  retail_pro_sales sales, (SELECT DISTINCT contact_id FROM contacts_loyalty " +
					   " WHERE user_id = " + userId + " AND program_id = " + prgmId + ") cl  " +
					   " WHERE sales.user_id = " + userId + " AND sales.cid IS NOT NULL AND sales.cid = cl.contact_id" + 
					   " AND sales.sales_date BETWEEN '" + startDateStr + "' AND '"+ endDateStr + "' AND sales.store_number IS NOT NULL AND sales.subsidiary_number IS NOT NULL" +
					   " GROUP BY sales.store_number , sales.subsidiary_number" ;
		  ...
		  /* qry = "SELECT  sales.store_number,COUNT(DISTINCT sales.doc_sid),ROUND(SUM((sales.quantity*sales.sales_price)+ sales.tax -(IF(sales.discount is null,0,sales.discount))),2) ," +
				   " sum(if(trx.transaction_type='Issuance' && trx.source_type='Store',ifnull(trx.earned_amount,0),0)) as issued,sum(if(trx.transaction_type='Redemption'  && trx.source_type='Store', trx.entered_amount,0)) as redeemed "+
				   "FROM  retail_pro_sales sales, loyalty_transaction_child trx, (SELECT DISTINCT contact_id FROM contacts_loyalty " +
				   " WHERE user_id = " + userId + " AND program_id = " + prgmId + ") cl  " +
				   " WHERE sales.user_id = " + userId + " and trx.user_id ="+userId+" and  trx.program_id ="+prgmId+" AND sales.cid IS NOT NULL AND sales.cid = cl.contact_id " + 
	   ...
	   public List<Object[]> getLoyaltyRevenueAndVisits(Long userId,
			   Long prgmId, String startDateStr, String endDateStr, String storeNo, Long cardsetId, String typeDiff,String subId,Long tierId) {
		   String storeQry = "";
		   String cardsetQry = "";
		   if(storeNo != null && storeNo.length() != 0) {
			   storeQry += " AND sales.store_number in ("+storeNo+")";
			...
			if(subId != null && subId.length() != 0) {
				cardsetQry += " AND subsidiary_number in ("+subId+")";
			}
			if(tierId != null) {
				cardsetQry += " AND program_tier_id =" + tierId.longValue();
			}
		   String qry = "";
		   if(typeDiff.equalsIgnoreCase("days")) { 
			   qry = "SELECT  ROUND(SUM((sales.quantity*sales.sales_price) + sales.tax -(IF(sales.discount is null,0,sales.discount))),2) , "
			   		+ "DATE(sales.sales_date), COUNT(DISTINCT sales.doc_sid) as visits ,cl.program_tier_id as tier" +
					   " FROM  retail_pro_sales sales, (SELECT DISTINCT contact_id, program_tier_id FROM contacts_loyalty " +
					   " WHERE user_id = " + userId + " AND program_id = " + prgmId+" AND program_tier_id IS NOT NULL " + cardsetQry+ ") cl  " +
					   " WHERE sales.user_id = " + userId + " AND sales.cid IS NOT NULL AND sales.cid = cl.contact_id" + storeQry + 
					   " AND sales.sales_date BETWEEN '" + startDateStr + "' AND '"+ endDateStr + "'" +
			   ...
			   qry = "SELECT  ROUND(SUM((sales.quantity*sales.sales_price) + sales.tax -(IF(sales.discount is null,0,sales.discount))),2) ,"
			   		+ " MONTH(sales.sales_date) , COUNT(DISTINCT sales.doc_sid) as visits ,cl.program_tier_id as tier" +
					   " FROM  retail_pro_sales sales, (SELECT DISTINCT contact_id, program_tier_id FROM contacts_loyalty " +
					   " WHERE user_id = " + userId + " AND program_id = " + prgmId +" AND program_tier_id IS NOT NULL " + cardsetQry+ ") cl  " +
					   " WHERE sales.user_id = " + userId + " AND sales.cid IS NOT NULL AND sales.cid = cl.contact_id" + storeQry +
					   " AND sales.sales_date BETWEEN '" + startDateStr + "' AND '"+ endDateStr + "'" +
			   ...
			   qry = "SELECT  ROUND(SUM((sales.quantity*sales.sales_price) + sales.tax -(IF(sales.discount is null,0,sales.discount))),2) , "
			   		+ "YEAR(sales.sales_date) , COUNT(DISTINCT sales.doc_sid) as visits ,cl.program_tier_id as tier" +
					   " FROM  retail_pro_sales sales, (SELECT DISTINCT contact_id , program_tier_id FROM contacts_loyalty " +
					   " WHERE user_id = " + userId + " AND program_id = " + prgmId +" AND program_tier_id IS NOT NULL " + cardsetQry+ ") cl  " +
					   " WHERE sales.user_id = " + userId + " AND sales.cid IS NOT NULL AND sales.cid = cl.contact_id" + storeQry +
					   " AND sales.sales_date BETWEEN '" + startDateStr + "' AND '"+ endDateStr + "'" +
		   ...
		   logger.info("loyalty revenue query:"+qry);
		   List<Object[]> tempList = null;
	   ...
	   public List<Object[]> getLoyaltyRevenueByPrgmId(Long userId,
		   ...
		   logger.info("Program Overview Graph query:"+qry);
		   List<Object[]> tempList = null;
	   ...
	   public List<Map<String,Object>> getTotalRevenueByUserId(Long userId, String startDateStr,
			   String endDateStr) {
//Rajeev
		   String qry = " SELECT ROUND(SUM((quantity*sales_price) + tax - if(discount is null, 0, discount)),2) as amt,  "
		   		+ "COUNT(DISTINCT doc_sid) as visits FROM retail_pro_sales " +
	...
	public List<Object[]> getLoyaltyVisitsByPrgmId(Long userId, Long prgmId,
		...
		String storeQry = "";
		   ...
		   logger.info("loyalty visits qry:"+qry);
		   List<Object[]> tempList = null;
	...
	public List<Object[]> getTotalRevenue(Long userId,
		...
		if(storeNo != null && storeNo.length() != 0) {
		   ...
		   if(typeDiff.equalsIgnoreCase("days")) { 
			   qry = "SELECT  ROUND(SUM((sales.quantity*sales.sales_price) + sales.tax -(IF(sales.discount is null,0,sales.discount))),2) ,"
			   		+ " DATE(sales.sales_date) , COUNT(DISTINCT doc_sid) as visits" +
					   " FROM  retail_pro_sales sales "+
			   ...
			   qry = "SELECT  ROUND(SUM((sales.quantity*sales.sales_price) + sales.tax -(IF(sales.discount is null,0,sales.discount))),2) ,"
			   		+ " MONTH(sales.sales_date), COUNT(DISTINCT doc_sid) as visits " +
					   " FROM  retail_pro_sales sales"+
					   " WHERE sales.user_id = " + userId + subQry + 
					   " AND sales.sales_date BETWEEN '" + startDateStr + "' AND '"+ endDateStr + "'" +
					   " GROUP BY MONTH(sales.sales_date) ORDER BY MONTH(sales.sales_date) " ;
			   ...
			   qry = "SELECT  ROUND(SUM((sales.quantity*sales.sales_price) + sales.tax -(IF(sales.discount is null,0,sales.discount))),2) , "
			   		+ "YEAR(sales.sales_date) , COUNT(DISTINCT doc_sid) as visits" +
					   " FROM  retail_pro_sales sales " +
					   " WHERE sales.user_id = " + userId + subQry + 
					   " AND sales.sales_date BETWEEN '" + startDateStr + "' AND '"+ endDateStr + "'" +
					   " GROUP BY YEAR(sales.sales_date) ORDER BY YEAR(sales.sales_date) " ;
		   ...
		   logger.info("total revenue qry:"+qry);
	...
	public List<Object[]> getNonLoyaltyVisits(Long userId,String startDateStr, String endDateStr, String storeNo,String typeDiff) {
		...
		if(storeNo != null && storeNo.length() != 0) {
		   ...
		   logger.info("Non loyalty visits"+qry);
	...
	public List<Object[]> getTotalVisits(Long userId,
		...
		if(storeNo != null && storeNo.length() != 0) {
		   ...
		   logger.info("total visits qry:"+qry);
	...
	public ResultSet executeQryForPurchaseHistory(String memberShipNumber, String cid, String receiptNumber,  String storeNumber,
			String startDate, String endDate, String offset, String maxRecords , Long userId) {
		...
		subQuery += (storeNumber != null && !storeNumber.isEmpty()) ? (" and sal.store_number='"+storeNumber+"' ") : Constants.STRING_NILL;
		
		subQuery += (cid != null && !cid.isEmpty()) ? (" and sal.cid= "+cid ) : Constants.STRING_NILL;
		
		String query = " SELECT sal.* , sku.* FROM retail_pro_sales sal left outer join retail_pro_sku sku on sku.user_id="+userId+" AND (sal.inventory_id=sku.sku_id OR sal.item_sid=sku.item_sid) "
				+ " WHERE  sal.user_id="+userId
				+  subQuery;//+" limit "+offset+ Constants.DELIMETER_COMMA+maxRecords;
	...
	public ResultSet executeQueryForPurchaseHistory( String receiptNumber, String docSID,
			String subsidiayNumber, String storeNumber, String startDate, String endDate, String offset, String maxRecords , Long userId) {
		...
		subQuery += (docSID != null && !docSID.isEmpty()) ? (" and sal.doc_sid='"+docSID+"' ") : Constants.STRING_NILL;
	...
	public List<Map<String, Object>> findTotalShoppersVisits(Long userId,  String startDate, String endDate) {
		try {
			String qry = "SELECT COUNT(DISTINCT doc_sid) as visits " +
						 " FROM retail_pro_sales  " +
						 " WHERE user_id=" + userId +
						 " AND sales_date BETWEEN '"+ startDate +"' AND '"+endDate+"' ";
			logger.info("findTotalShoppers... >>>"+qry);
 ...
 public List<RetailProSalesCSV> findBy(Long userID, String docSID, String receiptNumber, String subsidiaryNumber, String storeNumber) {
	 String subQry = Constants.STRING_NILL;
	 if(docSID != null && !docSID.isEmpty()) {
		 
		 subQry += " AND docSid='"+docSID+"'";
	 }
	 else if(receiptNumber != null && !receiptNumber.isEmpty() &&(
			 (subsidiaryNumber != null && !subsidiaryNumber.isEmpty()) || (storeNumber != null && !storeNumber.isEmpty()) ) ) {
			subQry += " AND ( recieptNumber = '"+receiptNumber+"' ";
	 ...
	 String query =" FROM RetailProSalesCSV WHERE userId="+userID+" AND docSid='"+docSID+"' "+subQry;
	 logger.info("findBy======="+query);
 ...
 //APP-4908
 public List<Object[]> getLastMonthSalesByUserId(Long userId) {
	 
	 List<Object[]> skuListArr = null;
	 String queryStr = "SELECT store_number,sum(((sales_price*quantity)-(IF(udf2 is NULL,0,udf2)))-(IF(discount is NULL,0,discount))) as sale_amount "
	 		+ "FROM retail_pro_sales WHERE user_id="+userId+" AND "
	 				+ "sales_date>=str_to_date(concat(DATE_FORMAT(DATE_SUB(now(), INTERVAL 1 MONTH), '%Y-%m'),'-01'),'%Y-%m-%d') "
	 				+ "AND sales_date<str_to_date(concat(DATE_FORMAT(now(), '%Y-%m'),'-01'),'%Y-%m-%d') GROUP BY store_number ";
	 
	 try {
		 logger.info("getSalesListByUserId "+queryStr);
			skuListArr = jdbcTemplate.query(queryStr, new RowMapper() {
				 public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
					 
					 Object[] object =  new Object[2];
					 object[0] = rs.getString(1); // store number
					 object[1] = rs.getString(2); // sale amount
					 

					 return object;
				 }
			});
			logger.info("skuListArr size "+skuListArr);
			return skuListArr;
