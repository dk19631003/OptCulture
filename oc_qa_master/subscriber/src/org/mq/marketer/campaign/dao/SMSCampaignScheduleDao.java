...
public SMSCampaignSchedule findBySmsCampRepId(Long smsCampRepId) {
		
		List list= getHibernateTemplate().find("FROM SMSCampaignSchedule WHERE crId ="+smsCampRepId);
		
		if(list != null && list.size() > 0) {
			return (SMSCampaignSchedule)list.get(0);
		}
		else {
			return null;
		}
	...
	public int getSMSCampaignCount(Long userIds) {
    	try{
    		String query = null;
    		query=" SELECT count(distinct smsCampaignId) FROM SMSCampaignSchedule WHERE userId IN ( "+userIds.longValue()+" ) and status = 0 " ;
