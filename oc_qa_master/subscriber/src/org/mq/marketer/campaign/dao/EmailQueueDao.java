...
public class EmailQueueDao extends AbstractSpringDao {
   ...
   /* public void delete(EmailQueue testEmailQueue) {
    ...
    public List findByType(Long userId,String type){
    	return getHibernateTemplate().find(" from EmailQueue where user="+ userId +  " AND type='" + type + "'  ORDER BY sentDate DESC");
    }
    ...
    public int findCountByDate(Long userId, String startDate, String endDate, String type){
    	try{
    		String query = "SELECT COUNT(type) FROM EmailQueue WHERE user = "+ userId +(type != null ? (" AND type in ("+type+") "): "")+" AND sentDate BETWEEN '"+startDate+"' AND '"+endDate+"' AND status = 'Sent'" ;
    	
    	/*if(type != null){
    		query += " AND type in ("+type+") ";
    		
    	}*/
    	query += " GROUP BY type, customTemplateId ";
    ...
    public List<Object[]> fetchRecordsByDate(Long userId, String startDate, String endDate, String type, int firstResulset, int size,String orderby_colName_keywords,String desc_Asc){
    	try{
    		String query = "SELECT eq.cust_temp_name as templateName, eq.type, max(eq.sent_date) as latestSentDate, count(1) sentCount, SUM(IF(eq.delivery_status ='Success', 1,0)) as delivered, SUM(IF(eq.opens >0, 1,0)) as uniopens,SUM(IF(eq.clicks >0, 1,0)) as uniclicks, eq.cust_temp_id, ct.template_name " +
    				" FROM  email_queue eq LEFT OUTER JOIN custom_templates ct ON ct.template_id = eq.cust_temp_id  WHERE  eq.user_id = "+ userId + (type != null ? (" AND eq.type in ("+type+") "): "")+" AND eq.sent_date BETWEEN '"+startDate+"' AND '"+endDate+"' AND eq.status = 'Sent' ";
    		if(type != null){
        		//query += ;
    ...
    public Object[] findConsolidatedReportByTemplateId(Long userId, String startDate, String endDate, String type, Long templateId){
    	try{
    		String query = "SELECT count(eq.id) as sentCount, SUM(IF(eq.delivery_status ='Success', 1,0)) as delivered, SUM(IF(eq.delivery_status in('" +Constants.DR_STATUS_BOUNCED+"','"+Constants.DR_STATUS_DROPPED+"','" +Constants.DR_STATUS_BOUNCE+"'), 1,0)) as bounced, " +
    				...
    				" SUM(IF(eq.delivery_status in ('" +Constants.DR_STATUS_SUBMITTED+"'), 1,0)) as notSentCount FROM  email_queue eq WHERE eq.user_id = "+ userId + (type != null ? (" AND eq.type in ('"+type+"') "): "")+"  AND eq.sent_date BETWEEN '"+startDate+"' AND '"+endDate+"' AND eq.status = 'Sent'";
    		/*if(type != null){
        		query += " AND type in ('"+type+"') ";
    ...
    public int findCountByTemplateId(Long userId, String startDate, String endDate, String type, Long templateId){
    	int size = 0;
    	try{
    		String query = "SELECT COUNT(id) FROM EmailQueue WHERE user = "+ userId +(type != null ? (" AND type in ('"+type+"') "): "")+"  AND sentDate BETWEEN '"+startDate+"' AND '"+endDate+"' AND status = 'Sent'";
	...
	public List<Object[]> findTotalReportRateByTemplateId(Long userId, boolean[] isChecked, String fromDate, String endDate, String tempType, Long templateId,String xaxisType,int tzOffSet) {
		try{
			...
			String column[] = {"count(eq.id) as sentCount", "SUM(IF(eq.delivery_status ='Success', 1,0)) as delivered","SUM(IF(eq.delivery_status in('" +Constants.DR_STATUS_BOUNCED+"','"+Constants.DR_STATUS_DROPPED+"','" +Constants.DR_STATUS_BOUNCE+"'), 1,0)) as bounced",
					"SUM(IF(eq.opens >0, 1,0)) as uniopens","SUM(IF(eq.clicks >0, 1,0)) as uniclicks","SUM(IF(eq.delivery_status ='Submitted', 1,0)) as notSentCount"};
			String query ="";
			String grpBy ="";
			...
			query += " FROM  email_queue eq  WHERE eq.user_id = "+ userId+ (tempType != null ? (" AND eq.type in ('"+tempType+"') "): "")+"  AND eq.sent_date BETWEEN '"+fromDate+"' AND '"+endDate+"' AND eq.status = 'Sent'";
	...
	public List<EmailQueue> getRowsByTemplateId(Long userId, Long tempId, String type, String fromDate, String endDate, int firstResultset, int size, String key, String emailSatusTobeFetched) {
		...
		try{
			...
			query += " AND delivery_status NOT IN('Submitted') ";
	...
	public List<EmailQueue> getStatusPendingRowsByTemplateId(Long userId, Long tempId, String type, String fromDate, String endDate, int firstResultset, int size, String key, String emailSatusTobeFetched) {
	...
	try{
		...
		String query = "SELECT count(id) as statusPendingCount, to_email_id,sent_date FROM email_queue " +
		...
		list = jdbcTemplate.query(query, new RowMapper() {
			...
			public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				emailQueue.setId(rs.getLong("statusPendingCount"));
				if(emailQueue.getId()!=0){
				emailQueue.setToEmailId(rs.getString("to_email_id"));
				Calendar cal = Calendar.getInstance();
				logger.info("rs.getTimestamp(sent_date)"+rs.getTimestamp("sent_date"));
				cal.setTime(rs.getTimestamp("sent_date"));
				emailQueue.setSentDate(cal);
				}
				return emailQueue;
	...
	public int getSizeByTemplateId(Long userId, Long tempId, String type,
			...
			try{
				String query = "SELECT COUNT(id) FROM EmailQueue WHERE user="+userId+" AND type='"+type+"'  AND sentDate BETWEEN '"+fromDate+"' AND '"+endDate+"' AND status='Sent'";
				if(tempId !=null  && tempId != 0){
					query += " AND customTemplates="+tempId+" ";
				}else{
					query += " AND customTemplates is NULL ";
...
public int getStatusPendingByTemplateId(Long userId, Long tempId, String type,
			String fromDate, String endDate, String key) {
			...
			try{
				...
				query += "AND deliveryStatus='"+Constants.DR_STATUS_SUBMITTED+"' ";
				if(tempId !=null  && tempId != 0){
					query += " AND customTemplates="+tempId+" ";
				}else{
					query += " AND customTemplates is NULL ";
