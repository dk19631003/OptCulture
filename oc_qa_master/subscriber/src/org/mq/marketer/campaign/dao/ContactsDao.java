...
import org.mq.marketer.campaign.controller.contacts.ClickHouseDBConnection;
...
public class ContactsDao extends AbstractSpringDao {
	...
	public int getSegmentedContactsCountFromCH(String qry) {
		try {
			...
			try {
				if(resultSet==null ) return count;
	...
	public Contacts findContactByEmailId(String emailId, Long user) {

		logger.debug("just enter ContactDao");
		List<Contacts> contactsList = null;
		String qryStr = " FROM Contacts  WHERE emailId='" + emailId + "' AND  users=" + user;
		logger.info("qryStr hibernate value is "+qryStr);
		contactsList = getHibernateTemplate().find(qryStr);
		if (contactsList.size() > 0) {
	...
	public long getOptinContactsCount(Users userId, Calendar fromCal, Calendar toCal) {
		try {
	...
	public ResultSet findContactsInfoBy(Users user, String customerIDs, String emailIDs, String mobileStr,String firstNames,String lastNames){
		//For overcoming MySQL syntax problem. 
		
		
		if(customerIDs.isEmpty() && emailIDs.isEmpty() && mobileStr.isEmpty() && firstNames.isEmpty() && lastNames.isEmpty()) {
			return null;
		}
		...
		String qry = "SELECT  c.*,cl.card_number,cl.pos_location_id,cl.subsidiary_number as sbs_num "
				...
				if(!firstNames.isEmpty())    //APP-3622
					qry+= " AND c.first_Name in("+firstNames+") ";
				if(!lastNames.isEmpty())
					qry+= " AND c.last_Name in("+lastNames+") ";
	...
	public ResultSet findContactsByLookup(Lookup lookup, Users user) {
		...
		//APP-3622
		if (lookup.getFirstName() != null && !lookup.getFirstName().isEmpty()) {
			if (count > 0)
				subQry += " UNION ";

			subQry += qry + " c.first_name='" + lookup.getFirstName()+ "'";
			count++;
		}
		if (lookup.getLastName()!= null && !lookup.getLastName().isEmpty()) {
			if (count > 0)
				subQry += " UNION ";

			subQry += qry + " c.last_name ='" + lookup.getLastName() + "'";
			count++;
		}
       // subQry += " ) ";
