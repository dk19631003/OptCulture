...
public class POSMappingDao extends AbstractSpringDao {
    ...
    public List<POSMapping> findByType(String mappingType, Long userId) {
    	...
    	try {
			return executeQuery("FROM POSMapping Where userId="+userId+ " AND mappingType in("+mappingType+")");
		} catch (DataAccessException e) {
			// TODO Auto-generated catch block
			logger.error("Exception ::" , e);
			return null;
    ...
    public List<POSMapping> findByTypeForSKU(String mappingType, Long userId) {
    	
    	try {
			return executeQuery("FROM POSMapping Where userId="+userId+ " AND mappingType ='"+mappingType+"'");
		} catch (DataAccessException e) {
			// TODO Auto-generated catch block
			logger.error("Exception ::" , e);
			return null;
 ...
 public List<POSMapping> findOnlyByType(String mappingType, Long userId) {
    ...
    public POSMapping findRecordBycustomFieldName(String posAttr,String mappingType ,Long userId) {
    	try {
			List<POSMapping> list = getHibernateTemplate().find("from POSMapping where userId = " + userId +" and  " +
																"mappingType ='"+mappingType+"' and customFieldName='"+posAttr+"'");
			if(list.size() >0) return list.get(0);
			else return null;
		} catch (DataAccessException e) {
			// TODO Auto-generated catch block
    ...
    public List<POSMapping> findDateMappings(String mappingType, Long userId) {
    	
    	try {
			String hql = "FROM POSMapping where userId = " + userId +" AND mappingType ='"+mappingType+"' " +
							" AND dataType LIKE 'Date(%' ";
