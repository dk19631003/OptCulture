...
public class EventTriggerDao extends AbstractSpringDao {
      ...
      public List<EventTrigger> findAllBetweenDatesByUserId(Long userId, String fromDate, String endDate,int start_Idx, int endIdx, String status) {
		...
		/**
		    * This method get's List of all active event triggers.
		    * @param userId
		    * @param status
		    * @return campaigns
		    */
		public List<EventTrigger> findByUserIdActiveStatus(Long userId,String status) {
	    	  
			List<EventTrigger> finalList = new ArrayList<EventTrigger>();
			String query = "FROM EventTrigger WHERE users="+userId.longValue();
			
			logger.debug(" Transaction qry is  :"+query);
			List<EventTrigger> retList = getHibernateTemplate().find(query);
			
	    	  for(EventTrigger anEventTrigger : retList){
					if(status.equals("Active")) {
						if((anEventTrigger.getOptionsFlag() & Constants.ET_TRIGGER_IS_ACTIVE_FLAG) == 1){
							finalList.add(anEventTrigger);
						}
