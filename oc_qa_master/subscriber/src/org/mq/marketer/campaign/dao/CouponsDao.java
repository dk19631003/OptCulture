...
//		logger.info(" &&&&&&&&&&&&& =====coupon count query is  ::"+qry);
	...
	public List<String> findCouponNames(Long orgId, String couponIdStr){
		...
		if(couponIdStr != null){//APP-4874 - couponName to couponCode
			queryStr = " SELECT couponCode FROM Coupons WHERE orgId = "+orgId+" AND couponId IN ("+couponIdStr+") AND status in ('"+Constants.COUP_STATUS_ACTIVE+"','"+Constants.COUP_STATUS_RUNNING+"')";
		}
		else{
			queryStr = " SELECT couponCode FROM Coupons WHERE orgId = "+orgId+" AND status in ('"+Constants.COUP_STATUS_ACTIVE+"','"+Constants.COUP_STATUS_RUNNING+"')";
		}
