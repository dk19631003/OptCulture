...
public class CampaignScheduleDaoForDML extends AbstractSpringDaoForDML {
    ...
    public CampaignScheduleDaoForDML() {}
...
//			logger.info("queryString :::::::"+queryString);
	...
	public int updateAllFuturePasuedSchedulesToActive(Users users) {
		byte status = 0;
		byte currentStatus = 3;
		Calendar cal = Calendar.getInstance();
		java.sql.Timestamp sDate = new java.sql.Timestamp(cal.getTime().getTime());
		String currentDateTime = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(sDate);
		logger.info("currentDateTime in campSchedule is =====>" + currentDateTime);
		String queryString = "UPDATE campaign_schedule SET status="+status +" WHERE user_id ="+users.getUserId()+" and scheduled_date >= DATE_ADD('"+currentDateTime+"', INTERVAL -3 HOUR) and status ="+currentStatus +"";
		logger.info("queryString in campSchedule is =====>" + queryString);
		int rowsEffected = jdbcTemplate.update(queryString);
		logger.info(rowsEffected);
	 	 return rowsEffected;
	}
}
