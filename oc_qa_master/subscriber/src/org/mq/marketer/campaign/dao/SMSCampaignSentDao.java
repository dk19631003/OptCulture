	...
	public List<String> getOptedOut(Long smsCampRepId, int startFrom, int count) {
	 ...
	 public List<Object[]> getOpenedMobilesByCrId(Long crId,int firstResult,int maxResults) {
	  ...
	  public List<Object[]> getAllSentCategories(Long smsCampRepId) {
		  ...
		  String queryStr = "select  status, count(sent_id) from sms_campaign_sent where status Not " +
			 		"IN('"+ Constants.SMS_CAMPAIGN_STATUS_BOUNCE + "','" +
			 		SMSStatusCodes.CLICKATELL_STATUS_RECEIVED+ "') And sms_cr_id =" + smsCampRepId + " group by status order by count(sent_id)  desc";
	  ...
	  public SMSCampaignSent findByMrIdAndMobile(String mrId, String mobile) {
		  ...
		  if(mobile.length() == 12) {
			...
			String query = "FROM SMSCampaignSent where apiMsgId='"+mrId+"' AND mobileNumber like '%"+mobile+"%'";
			logger.info("query--"+query);
			try{
			List<SMSCampaignSent> retSmsCampaignSentList =  getHibernateTemplate().find(query);
			...
			//return (SMSCampaignSent) (executeQuery(query).get(0));
			
			return null;
		}
	  public SMSCampaignSent findByRefIdAndMobile(String mrId, String mobile) {
