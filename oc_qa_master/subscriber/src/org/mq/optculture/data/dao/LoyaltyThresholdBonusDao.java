	...
	public List<LoyaltyThresholdBonus> getAllOnlyAutoCommExpireBy() {
		try {
			...
			String queryStr = " SELECT DISTINCT lb FROM LoyaltyThresholdBonus lb ,LoyaltyProgram lp WHERE lb.programId=lp.programId "
					...
					+ " AND lb.bonusExpiryDateValue IS NOT NULL AND lb.bonusExpiryDateType IS NOT NULL AND "
					+ " (lb.emailExpiryTempId IS NOT NULL OR lb.smsExpiryTempId IS NOT NULL) AND "
			...
			logger.debug("getAllOnlyAutoCommExpireBy queryStr and list size "+queryStr+" "+programList.size());
