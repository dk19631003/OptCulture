...
*/
	...
	public String getCardName(String prgmId) {
		
		
		String queryStr = "SELECT cardSetName FROM LoyaltyCardSet WHERE cardSetId ="+prgmId;
	...
	public LoyaltyCardSet findCardSetByCardSetName(Long prgmId, String cardSetName) {
		
		List<LoyaltyCardSet> cardSetList = getHibernateTemplate().find(" FROM LoyaltyCardSet WHERE programId = "+prgmId.longValue()+" AND cardSetName = '"+cardSetName+"'");
		if(cardSetList != null && cardSetList.size() > 0) return cardSetList.get(0);
		else return null;
	}
	
	public LoyaltyCardSet findCardSetByCardSetAndPrgm(Long prgmId, String cardSetId) {
		
		List<LoyaltyCardSet> cardSetList = getHibernateTemplate().find(" FROM LoyaltyCardSet WHERE programId = "+prgmId.longValue()+" AND cardSetId = '"+cardSetId+"'");
		if(cardSetList != null && cardSetList.size() > 0) return cardSetList.get(0);
		else return null;
	}
...
public List<LoyaltyCardSet> findBy(Long prgmId, String status, String cardGenerationType) {
	...
	public int findTierLinkedToCardset(int cardSetId) {
		List<Integer> cardSetList = getHibernateTemplate(). find("SELECT linkedTierLevel FROM LoyaltyCardSet WHERE cardSetId = "+cardSetId);
		if(cardSetList != null && cardSetList.size() > 0) return cardSetList.get(0);
		else return 0;
	}
