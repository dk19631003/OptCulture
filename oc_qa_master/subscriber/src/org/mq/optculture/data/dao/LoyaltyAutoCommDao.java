...
public class LoyaltyAutoCommDao extends AbstractSpringDao{
	...
	public LoyaltyAutoComm findBy(Long prgmId) {
		List<LoyaltyAutoComm> list = getHibernateTemplate().find("FROM LoyaltyAutoComm WHERE "
				+ "programId="+ prgmId.longValue()+" AND (adjustmentAutoSmsTmplId IS NOT NULL OR adjustmentAutoEmailTmplId IS NOT NULL) " );
