...
public class SendDRByXmlMultiThreadService implements Runnable{
	...
	public SendDRByXmlMultiThreadService(){}
	...
	public SendDRByXmlMultiThreadService(File xmlFile, 
			String inboxPath, String outboxPath, String donePath, String owner, String group, boolean isSpecific){
...
public SendDRByXmlMultiThreadService(String myListItem){
		...
		try {
			//if(inboxFileExist(inboxDRPath)){
				...
				//for(File xmlFile1 : xmlFiles){
					...
					boolean validXmlFile = true;//validateXmlWithXsd(xmlFile.toString());
					if(!validXmlFile){
	...
	public void writeFailedMessage(String reasonType, String errorMsg, String xmlfileName, String extension){
		...
		try {
			String chOwnCmd = "chown "+owner+":"+group+" "+outboxPath+"/"+fileName;
			logger.debug("chaging the file permissions =="+ chOwnCmd);
			Runtime.getRuntime().exec(chOwnCmd);
			
			String chmodCmd = "chmod 777 "+outboxPath+"/"+fileName;
	...
	public void processXmlFile(File xmlfile){
		...
		try{
			...
			outboxPath += outboxPath.endsWith(File.separator) ? "" : File.separator;
			String digiReceiptCSVFilePath = outboxPath+xmlfile.getName().replace(FilenameUtils.getExtension(xmlfile.getName()), "csv");
