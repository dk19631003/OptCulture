...
public class AsyncLoyaltyEnrollmentServiceImpl implements AsyncLoyaltyEnrollmentService{
	...
	public BaseResponseObject processRequest(BaseRequestObject baseRequestObject)
		...
		if(serviceRequest == null || !serviceRequest.equals(OCConstants.LOYALTY_SERVICE_ACTION_ENROLMENT)){
			...
			AsyncLoyaltyEnrollmentService loyaltyEnrollService = (AsyncLoyaltyEnrollmentService) ServiceLocator.getInstance().getServiceByName(OCConstants.ASYNC_LOYALTY_ENROLMENT_BUSINESS_SERVICE);
			...
			logger.error("Exception in loyalty base service.", e);
			throw new BaseServiceException("Server Error.",e);
		}
			logger.info("Completed processing baserequest... ");
			return responseObject;
	 ...
	 */
	@Override
	public LoyaltyEnrollResponseObject processEnrollmentRequest(LoyaltyEnrollRequestObject enrollRequest, String mode, String trxID, String trxDate)
			throws BaseServiceException {
		...
		EnrollmentInfo enrollmentInfo = enrollRequest.getENROLLMENTINFO();
		enrollmentInfo.setTIERNAME("");
		
		try{
			//validation on userDetails and customer key identifiers
			...
			if(user == null){
				...
				if(statusInfo != null && OCConstants.JSON_RESPONSE_FAILURE_MESSAGE.equals(statusInfo.getSTATUS())){
					enrollResponse = prepareEnrollmentResponse(enrollRequest.getHEADERINFO(), enrollRequest.getUSERDETAILS(),
							enrollmentInfo, null, statusInfo, enrollRequest.getCUSTOMERINFO(), null);
					return enrollResponse;
				}
			}
			...
			if(OCConstants.LOYALTY_SERVICE_TYPE_SBTOOC.equals(user.getloyaltyServicetype())){
				...
				LoyaltyEnrollResponse responseObject = loyaltyEnrollService.processEnrollmentRequest(loyaltyEnrollRequest, OCConstants.LOYALTY_ONLINE_MODE, trxID, trxDate);
				
				enrollmentInfo.setTIERNAME(responseObject.getMembership().getTierName());
				...
				*/
					...
					String cardNUmber  = responseObject.getMembership().getCardNumber() != null && !responseObject.getMembership().getCardNumber().isEmpty() ? 
							responseObject.getMembership().getCardNumber() : 
								(responseObject.getMembership().getPhoneNumber() != null && 
								!responseObject.getMembership().getPhoneNumber().isEmpty() ? responseObject.getMembership().getPhoneNumber() : "");
							
					 enrollmentInfo = prepareEnrollmentInfo(cardNUmber, responseObject.getMembership().getCardPin(),
							enrollRequest.getENROLLMENTINFO().getCARDTYPE(), enrollRequest.getENROLLMENTINFO().getEMPID(),
			...
			try {
				...
				//this step can be postponed
				sbenrollResponse = enrollHelper.asynEnroll();
				
				if(sbenrollResponse != null) {
					logger.info("if sbenrollresponse is null>>>>1");
					...
					logger.debug("cardpin ====>"+sbenrollResponse.getCardPin());
					 enrollmentInfo = prepareEnrollmentInfo(cardnumberStr, sbenrollResponse.getCardPin(),
							enrollRequest.getENROLLMENTINFO().getCARDTYPE(), enrollRequest.getENROLLMENTINFO().getEMPID(),
							enrollRequest.getENROLLMENTINFO().getSTORELOCATIONID(), "");
					
					statusInfo = prepareStatusInfo(sbenrollResponse.getErrorCode(), sbenrollResponse.getErrorMessage(),
							sbenrollResponse.getStatus());
					if(statusInfo.getERRORCODE() == null && statusInfo.getMESSAGE() == null) {
						statusInfo = prepareStatusInfo("0", "Successfully enrolled.", OCConstants.LOYALTY_ENROLL_STATUS_SUCCESS);
					}
					enrollResponse = prepareEnrollmentResponse(enrollRequest.getHEADERINFO(), enrollRequest.getUSERDETAILS(),
							enrollmentInfo, null, statusInfo, enrollRequest.getCUSTOMERINFO(), enrollRequest.getAMOUNTDETAILS());
					...
					logger.info("else sbenrollresponse is null>>>>2");
					 enrollmentInfo = prepareEnrollmentInfo(cardNumber, "",
							enrollRequest.getENROLLMENTINFO().getCARDTYPE(), enrollRequest.getENROLLMENTINFO().getEMPID(),
							enrollRequest.getENROLLMENTINFO().getSTORELOCATIONID(),"");
				...
				/*if(statusInfo.getERRORCODE().equals("0") && dbContact.getEmailId() != null){
				...
				logger.info("Completed processing enrollment request..."+enrollResponse.getSTATUS());
				return enrollResponse;
			...
			enrollHelper = new LoyaltyEnrollmentHelper(dbContact, jsonContact,
					cardNumber, enrollRequest.getENROLLMENTINFO().getCARDPIN(),
					enrollRequest.getHEADERINFO().getSTORENUMBER(), user.getUserId(), sparkbaseLocation,
					enrollRequest.getENROLLMENTINFO().getCARDTYPE(), enrollRequest.getENROLLMENTINFO().getEMPID(), mode);
	...
	public LoyaltyEnrollResponseObject processNewEnrolmentRequest(AsyncLoyaltyTrx asyncLoyaltyTrx) throws BaseServiceException{
		logger.info("Started processing enrollment request...");
		LoyaltyEnrollResponseObject enrollResponse = null;
		StatusInfo statusInfo = null;
		Users user = null;
		...
		try{
		  ...
		  LoyaltyEnrollJsonRequest rootJsonRequestObject = null;
		  LoyaltyEnrollJsonResponse jsonResponseObject = null;
		   LoyaltyTransaction loyaltyTransaction =  asyncLoyaltyTrx.getLoyaltyTransaction();
		   ...
		   if(loyaltyTransaction != null){
			   ...
			   requestJson = asyncLoyaltyTrx.getLoyaltyTransaction().getJsonRequest();
			   ...
			   try{
					jsonRequestObject = gson.fromJson(requestJson, LoyaltyEnrollJsonRequest.class);
					enrollRequest = jsonRequestObject.getENROLLMENTREQ();
					enrollmentInfo = enrollRequest.getENROLLMENTINFO();
					enrollmentInfo.setTIERNAME("");
					...
					statusInfo = new StatusInfo("101002", ""+PropertyUtil.getErrorMessage(101002, OCConstants.ERROR_LOYALTY_FLAG), OCConstants.JSON_RESPONSE_FAILURE_MESSAGE);
					enrollResponse.setSTATUS(statusInfo);
			   ...
			   MailingListDao mailingListDao = (MailingListDao) ServiceLocator.getInstance().getDAOByName(OCConstants.MAILINGLIST_DAO);
				...
				if(ltyStage != null) {
					...
					cardNumber = ltyStage.getCardNumber();
			 ...
			 //this step can be avoided
				...
				enrollHelper = new LoyaltyEnrollmentHelper(dbContact, jsonContact,
						cardNumber, enrollRequest.getENROLLMENTINFO().getCARDPIN(),
						enrollRequest.getHEADERINFO().getSTORENUMBER(), enrollRequest.getHEADERINFO().getSUBSIDIARYNUMBER(), user, sparkbaseLocation,
						enrollRequest.getENROLLMENTINFO().getCARDTYPE(), enrollRequest.getENROLLMENTINFO().getEMPID(), OCConstants.LOYALTY_ONLINE_MODE, enrollRequest.getHEADERINFO().getSOURCETYPE());
				logger.debug("======calling asyncEnroll==");
				//this step can be postponed
				sbenrollResponse = enrollHelper.enrollAsync();
				...
				//Send Loyalty Registration Email
				if(statusInfo.getERRORCODE().equals("0") && (jsonContact.getEmailId() !=null || dbContact.getEmailId() != null ) && mlList.isCheckLoyaltyOptin()){
					sendLoyaltyTemplateMail(mlList.getLoyaltyCutomTempId(), user, sbenrollResponse.getCardNumber(),
							sbenrollResponse.getCardPin(), dbContact.getContactId(), dbContact.getFirstName(), jsonContact.getEmailId() != null ? jsonContact.getEmailId():dbContact.getEmailId() );
				} 
	...
	private Contacts validateAndSavedbContact(CustomerInfo customerInfo, Contacts jsonContact, MailingList mlList, Users user) throws Exception {
		...
		if(dbContact == null) {
			...
			/*if(dbContact.getMlBits().longValue() == 0l ) {//marked as deleted
				...
				dbContact.setEmailId(validateEmailId(jsonContact.getEmailId()));
				dbContact.setExternalId(jsonContact.getExternalId());
				...
				dbContact.setMobilePhone(jsonContact.getMobilePhone()== null ? null : Utility.phoneParse(jsonContact.getMobilePhone(), user!=null ? user.getUserOrganization() : null ));
				validateMobilePhoneStatus(dbContact);
	...
	private Contacts prepareContactFromJsonData(CustomerInfo customerInfo, Long userId) throws Exception {
		...
		if(customerInfo.getEMAIL() != null && customerInfo.getEMAIL().trim().length() > 0) {
			inputContact.setEmailId(validateEmailId(customerInfo.getEMAIL().trim())!=null?customerInfo.getEMAIL().trim():null);
		...
		if(smsSettings.isEnable() && 
			...
			if(contact.getLastSMSDate() == null && contact.isMobileOptin() != true) {
				...
				if( (  (currentUser.getSmsCount() == null ? 0 : currentUser.getSmsCount()) - (currentUser.getUsedSmsCount() == null ? 0 : currentUser.getUsedSmsCount() ) ) >=  1) {
					...
					smsQueueHelper.updateSMSQueue(phone,msgContent,smsSettings.getType(), user, smsSettings.getSenderId());
				}else {
	...
	public void sendCardCountAlerts(SparkBaseLocationDetails sparkBaseLoc, Users user){
		try {
			...
			try{
				...
				int dateDiff = 0;
				if(sparkBaseLoc.isEmailAlerts() && dbCurrentDate != null){
...
//				String message=null;
					...
					messageToUser = messageToUser.replace(Constants.NEW_USER_DETAILS_PLACEHOLDERS_USERNAME, Utility.getOnlyUserName(user.getUserName()));
					messageToUser = messageToUser.replace(Constants.NEW_USER_DETAILS_PLACEHOLDERS_ORGID, Utility.getOnlyOrgId(user.getUserName()));
					...
					Long count=Long.parseLong(PropertyUtil.getPropertyValueFromDB("DefaultLoyaltyThreshold")); // Changes 2.5.4.0
			...
			logger.error("Exception while sending alert ==", e);
		}
	
	}//sendCardCountAlerts()
	
	public void sendWelcomeEmail(Contacts contact, MailingList mailingList, Users user) throws Exception {
