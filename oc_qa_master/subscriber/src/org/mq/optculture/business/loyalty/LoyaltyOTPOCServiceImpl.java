	...
	public LoyaltyOTPResponse processOTPRequest(LoyaltyOTPRequest otpRequest,
		...
		*/	
			...
			if(otpRequest.getHeader().getRequestType() != null && 
				...
				//APP-3228
				OTPGeneratedCodes otpgeneratedcode = findOTPCodeByPhone(phone, user, OCConstants.OTP_GENERATED_CODE_STATUS_ACTIVE,email,null);
				...
				OTPGeneratedCodes otpgeneratedcodeForOTP = findOTPCodeByPhone(phone, user, OCConstants.OTP_GENERATED_CODE_STATUS_ACTIVE,email,otpRequest.getOtpCode());	
				...
				//OTP details were not found.
				 otpgeneratedcodeForOTP = findOTPCodeByPhone(phone, user, OCConstants.OTP_GENERATED_CODE_STATUS_ACTIVE,email,otpRequest.getOtpCode());	
				if(otpgeneratedcodeForOTP == null ) {
					status = new Status("111602", PropertyUtil.getErrorMessage(111602, OCConstants.ERROR_LOYALTY_FLAG), OCConstants.JSON_RESPONSE_FAILURE_MESSAGE);
					otpResponse = prepareOTPResponse(responseHeader, status,Constants.STRING_NILL);
					return otpResponse;
...
private List<MatchedCustomer> prepareMatchedCustomers(List<ContactsLoyalty> enrollList,LoyaltyOTPRequest loginRequest) throws Exception {
	...
	private OTPGeneratedCodes findOTPCodeByPhone(String phone, Users user, String status,String email, String OTP) throws Exception {
		...
		return otpGenCodesDao.findOTPCodeByPhonenew(phone, user, status,email,OTP);
	}
			...
			Long programId =  ltyprgrmservice.getCustomizedOTPEnabledProgList(userid, false);//find the customized OTP msg enabled program under user;
				if(programId !=null) {
					...
					if(templateId !=null) {
						...
						msgobj=  customtempDao.findCustTemplateById(templateId);
				...
				usersDaoForDML.updateUsedEmailCount(user.getUserId(), 1);
				 ...
				 if(Constants.VMTA_SENDGRIDAPI.equalsIgnoreCase(user.getVmta().getVmtaName())){	
						try {
							...
							from = msgobj.getFromName()+ "<" +msgobj.getFromEmail()+ ">" ;
							replyTo = msgobj.getReplyToEmail();
							subject= msgobj.getSubject();
							logger.info("the subjectis " +msgobj.getSubject());

							}else {
								
							from = user.getUserOrganization().getOrganizationName()+ "<" +user.getEmailId()+ ">" ;
							replyTo = from;
							subject= otpEmail.getSubject();
