 ...
 *
 */
public class AsyncLoyaltyIssuanceServiceImpl implements LoyaltyIssuanceService{
	...
	public BaseResponseObject processRequest(BaseRequestObject baseRequestObject)
		...
		LoyaltyIssuanceService loyaltyIssuanceService = (LoyaltyIssuanceService) ServiceLocator.getInstance().getServiceByName(OCConstants.ASYNC_LOYALTY_ISSUANCE_BUSINESS_SERVICE);
		LoyaltyIssuanceResponseObject issuanceResponse = loyaltyIssuanceService.processIssuanceRequest(issuanceRequest, OCConstants.LOYALTY_ONLINE_MODE, baseRequestObject.getTransactionId(), baseRequestObject.getTransactionDate());
		LoyaltyIssuanceJsonResponse jsonResponseObject = new LoyaltyIssuanceJsonResponse();
		jsonResponseObject.setLOYALTYISSUANCERESPONSE(issuanceResponse);
	...
	public LoyaltyIssuanceResponseObject processIssuanceRequest(
		...
		try{
			...
			if(OCConstants.LOYALTY_SERVICE_TYPE_SBTOOC.equals(user.getloyaltyServicetype())){
				...
				if(user.isEnableLoyaltyExtraction() && user.isIssuanceFromDR() ) {
					statusInfo = new StatusInfo("0", "Issuance will be done shortly.", OCConstants.JSON_RESPONSE_IGNORED_MESSAGE);
					issuanceResponse = prepareIssuanceResponse(issuanceRequest.getHEADERINFO(), issuanceRequest.getUSERDETAILS(), issuanceInfoRequest, null, statusInfo);
					return issuanceResponse;//Special Reward changes
				}
				//call the seperated
				statusInfo = validateIssuanceRequestFields(issuanceRequest,mode);
				if(statusInfo != null && OCConstants.JSON_RESPONSE_FAILURE_MESSAGE.equals(statusInfo.getSTATUS())){
					issuanceResponse = prepareIssuanceResponse(issuanceRequest.getHEADERINFO(), issuanceRequest.getUSERDETAILS(), issuanceInfoRequest, null, statusInfo);
					return issuanceResponse;
				}
				...
				LoyaltyIssuanceResponse responseObject = loyaltyIssuanceService.processIssuanceRequest(loyaltyIssuanceRequest, mode, trxID, trxDate,null);
		    ...
		    if(issuanceRequest.getISSUANCEINFO().getENTEREDAMOUNT() != null){
		     
		     if(issuanceRequest.getISSUANCEINFO().getVALUECODE().equalsIgnoreCase("USD")){
		    	 bal = new Balances();
		    	 bal.setAMOUNT(issuanceRequest.getISSUANCEINFO().getENTEREDAMOUNT());
		    	 bal.setEXCHANGERATE("");
		         bal.setDIFFERENCE("0");
		    	 bal.setVALUECODE(issuanceRequest.getISSUANCEINFO().getVALUECODE());
		    	 balances.add(bal);
		    	 bal = new Balances();
		    	 bal.setAMOUNT("0");
		    	 bal.setEXCHANGERATE("");
		     ...
		     else if(issuanceRequest.getISSUANCEINFO().getVALUECODE().equalsIgnoreCase("points")){
	...
	public StatusInfo validateIssuanceRequestFields(LoyaltyIssuanceRequestObject issuanceRequest, String mode) {
		...
		if(issuanceRequest.getISSUANCEINFO().getENTEREDAMOUNT() != null && !issuanceRequest.getISSUANCEINFO().getENTEREDAMOUNT().trim().isEmpty()
				){
			try {
				double enteredValue = Double.valueOf(issuanceRequest.getISSUANCEINFO().getENTEREDAMOUNT().trim());
				logger.info("enteredvalue = " + enteredValue);
				if (enteredValue < 0) {
					logger.info("enteredvalue less than 1");
					statusInfo = new StatusInfo("111557", PropertyUtil.getErrorMessage(111557, OCConstants.ERROR_LOYALTY_FLAG), OCConstants.JSON_RESPONSE_FAILURE_MESSAGE);
					return statusInfo;
	 ...
	 * @throws Exception
	 */
	private List<Balances> prepareBalancesObject(Object issuanceResponse) throws Exception{
		...
		if(issuanceResponse instanceof org.mq.marketer.sparkbase.transactionWsdl.LoyaltyIssuanceResponse){
			aBalances = ((org.mq.marketer.sparkbase.transactionWsdl.LoyaltyIssuanceResponse) issuanceResponse).getBalances();
		}
		else if(issuanceResponse instanceof GiftIssuanceResponse){
			aBalances = ((GiftIssuanceResponse) issuanceResponse).getBalances();
		}else if(issuanceResponse instanceof GiftIssuanceResponse){
			aBalances = ((GiftIssuanceResponse) issuanceResponse).getBalances();
		}else if(issuanceResponse instanceof LoyaltyIssuanceResponse) {
			List<Balance> ocGivenBals = ((LoyaltyIssuanceResponse)issuanceResponse).getBalances();
	...
	private LoyaltyIssuanceResponseObject prepareIssuanceResponse(HeaderInfo headerInfo, UserDetails userDetails, IssuanceInfo issuanceInfo,
		...
		LoyaltyIssuanceResponseObject issuanceResponse = new LoyaltyIssuanceResponseObject();
		if(balances == null) {
			balances = new ArrayList<Balances>();
		}
	...
	public LoyaltyIssuanceResponseObject processNewIssuanceRequest(AsyncLoyaltyTrx asyncLoyaltyTrx) {
		...
		LoyaltyIssuanceJsonRequest rootJsonRequestObject = null;
		LoyaltyIssuanceResponseObject issuanceResponse = null;
		IssuanceInfo issuanceInfoRequest = null;
		try{
			...
			rootJsonRequestObject = gson.fromJson(requestJson, LoyaltyIssuanceJsonRequest.class);	
			LoyaltyIssuanceRequestObject issuanceRequest =  rootJsonRequestObject.getLOYALTYISSUANCEREQ();
			issuanceInfoRequest = issuanceRequest.getISSUANCEINFO();
			issuanceInfoRequest.setTIERNAME("");
			
			user = getUser(issuanceRequest.getUSERDETAILS().getUSERNAME(), issuanceRequest.getUSERDETAILS().getORGANISATION(), issuanceRequest.getUSERDETAILS().getTOKEN());
			...
			cardNumber = issuanceRequest.getISSUANCEINFO().getCARDNUMBER();
			if(cardNumber != null){
				issuanceRequest.getISSUANCEINFO().setCARDNUMBER(cardNumber);
			}
		...
		if(statusInfo != null && statusInfo.getERRORCODE().equals("0")){
			...
			if(loyaltyNotFound){
				...
				Contacts contact =  contactsDao.findContactByValues(null, null, issuanceRequest.getISSUANCEINFO().getCUSTOMERID(), user.getUserId());
				contactsLoyalty.setContact(contact);
				contactsLoyalty.setCustomerId(issuanceRequest.getISSUANCEINFO().getCUSTOMERID());
				if(card!=null)contactsLoyalty.setCardPin(card.getCardPin());
				contactsLoyalty.setEmpId(issuanceRequest.getISSUANCEINFO().getEMPID());
			...
			createPurchaseTransaction(issuanceRequest,purchaseAmount, contactsLoyalty, earnedAmount,  earnedPoints,
					earnType,enteredAmtType, sparkbaseLocation.getUserOrganization().getUserOrgId(), pointsDiff, amtDiff , loyaltyTransaction.getId().longValue()+"", convertionRate, subPoints+"" );
		...
		logger.debug("issuanceResponse>>>>>>>>>>>>>>>>>>>>>>"+issuanceResponse.getJsonValue());
		return issuanceResponse;
	}	
	}catch(Exception e){
		logger.info("Exception while updating SB issuance ::",e);
	}
	return issuanceResponse;
	}
