...
public class CampaignReportUpdateHelper {
	...
	public List<TemplateCategory> updateUnsubscribe(Long userId, Long crId, Long sentId,
		...
		CampaignSentDaoForDML campaignSentDaoForDML = null;
		try {
			campaignSentDao = (CampaignSentDao)ServiceLocator.getInstance().getDAOByName("campaignSentDao");
			campaignSentDaoForDML = (CampaignSentDaoForDML)ServiceLocator.getInstance().getDAOForDMLByName("campaignSentDaoForDML");
		} catch (Exception e) {
	...
	/*	CampaignReportDao campaignReportDao = (CampaignReportDao)
		...
		try {
			...
			unsubscribesDaoForDML = (UnsubscribesDaoForDML)ServiceLocator.getInstance().getDAOForDMLByName("unsubscribesDaoForDML");
		} catch (Exception e) {
			logger.error("Exception ::",e);
		}
