...
public class GatewayRequestProcessHelper {
	...
	public static OCSMSGateway getOcSMSGateway(Users user, String type, boolean isOTPRoute) throws BaseServiceException{
		...
		if(isOTPRoute) {
			userSmsGateway = userSMSGatewayDao.findByUserId(user.getUserId(), "OTP");
		}
		if(userSmsGateway == null) {// if the user has no configuration for otp still send over TR route 
...
public static OCSMSGateway getOcSMSGateway(Users user, String type) throws BaseServiceException{
		return getOcSMSGateway(user, type, false);
