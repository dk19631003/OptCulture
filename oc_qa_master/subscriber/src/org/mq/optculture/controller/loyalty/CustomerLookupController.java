 ...
 * @author vinod.bokare
 */
public class CustomerLookupController extends GenericForwardComposer<Window> implements EventListener{
	...
	private Div datesDivId,skuDetailsDivId,promotionDetailsDivId,whichprogramDivId,custExport$chkDivId,moveTierDivId,searchCriteriaDivId;
	...
	private Label finalBalanceRewardsLblId,finalBalancePointsLblId,latestDetailsSubWinId$searchCriteriaLblId,latestDetailsSubWinId$closedCardLblId;
	private Label balancePointLblId,balanceCurrencyLblId,expiredPointsLblId,expiredAmountLblId,
				  	balanceGiftAmountLblId,expiredGiftAmountLblId,transferedOrNewCardLblId;
	...
	private Label programNameLbId,cardSetNameLbId,currentTierLbId;
	private Button suspendMembershipBtnId,addBtnId,subtractBtnId,submitPwdBtnMvtierId,moveTierBtnId;
	private Listbox transListLbId,timeDurLbId,searchbyLbId,addLbId,subLbId,pageSizeLbId,addTypeLbId,subTypeLbId,transferListLbId,toTierLbId;
	...
	private Textbox ltyPwdTbId,ltyPwdTbSubId,ltyPwdTbSuspendId, firstNameTbId, lastNameTbId, emailTbId, phoneTbId, emailTxtBxId,phnTxtBxId;;
	...
	public void onClick$saveBtnId(){
		...
		try{
			...
			if( !firstNameTbId.getValue().trim().isEmpty()){
				...
				/*if(Utility.validateName(firstNameTbId.getValue().trim())){
					contact.setFirstName(firstNameTbId.getValue().toString().trim());
				}
				else{
			...
			if( !lastNameTbId.getValue().trim().isEmpty()){
				...
				/*if(contact != null){
					if(contact.getLastName() != null && contact.getFirstName() != null){
						fullName = contact.getLastName()+", "+contact.getFirstName();
					}
					else if(contact.getLastName() == null && contact.getFirstName() != null){
				...
				/*if(Utility.validateName(lastNameTbId.getValue().trim())){
					contact.setLastName(lastNameTbId.getValue().toString().trim());
				}
				else{
	...
	private void displayChangingTier(){
		...
		try{
			...
			/*int num1 = Integer.valueOf(currentTier.getTierType().substring(5)).intValue();
			
			int num2; 
			for(LoyaltyProgramTier tier : loyaltyProgramTiers){
				num2 = Integer.valueOf(tier.getTierType().substring(5)).intValue();
				if(num1 > num2){
					downgradeList.put(tier.getTierName(), tier.getTierId());
				}else if(num1 < num2){
					upgradeList.put(tier.getTierName(), tier.getTierId());
				}
			}*/
			
			/*upgradeId.removeAttribute("tiers");
			downgradeId.removeAttribute("tiers");
			
			if(upgradeList.size() == 0){
				upgradeId.setVisible(false);
			}else{
				upgradeId.setVisible(true);
				upgradeId.setAttribute("tiers", upgradeList);
			}
			if(upgradeList.size() == 0){
				downgradeId.setVisible(true);
				downgradeId.setVisible(false);
			}else{
				downgradeId.setAttribute("tiers", upgradeList);
			}*/
...
//		LoyaltyTransactionChild loyaltyTransactionChild = ltyPrgmService.getLoyaltyTransactionChild(contactsLoyalty.getCardNumber(),currUser.getUserId());
	...
	private void viewAddSubtractBtn(boolean flag) {
		...
		subTbId.setDisabled(flag);
		addBtnId.setDisabled(flag);
	...
	private void displayBalanceSummary(ContactsLoyalty contactsLoyalty) {
		...
		if(contactsLoyalty.getRewardFlag() != null && OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_G.equalsIgnoreCase(contactsLoyalty.getRewardFlag())){
			...
			balanceGiftAmountLblId.setValue(contactsLoyalty.getGiftBalance() == null ? "$ 0.00":Utility.getAmountInUSFormat(contactsLoyalty.getGiftBalance()  ));
			expiredGiftAmountLblId.setValue(contactsLoyalty.getExpiredGiftAmount() == null ? "$ 0.00":Utility.getAmountInUSFormat(contactsLoyalty.getExpiredGiftAmount() ));
		}
			...
			balanceCurrencyLblId.setValue(contactsLoyalty.getGiftcardBalance() == null ? "$ 0.00":Utility.getAmountInUSFormat(contactsLoyalty.getGiftcardBalance()  ));
			...
			expiredAmountLblId.setValue(contactsLoyalty.getExpiredRewardAmount() == null ? "$ 0.00":Utility.getAmountInUSFormat(contactsLoyalty.getExpiredRewardAmount()  ));
	...
	*/}///displaySalesSummary
	...
	private void displayMembershipSummary(ContactsLoyalty contactsLoyalty,Contacts contacts) {
		...
		/*String lifeTimePurchaseValue = "0.00";
		ContactsDao contactsDao = (ContactsDao) ServiceLocator.getInstance().getDAOByName(OCConstants.CONTACTS_DAO);
		if(contacts != null) {
			List<Map<String, Object>> contactPurcahseList = contactsDao.findContactPurchaseDetails(contactsLoyalty.getUserId(),contacts.getContactId());
			if(contactPurcahseList != null && contactPurcahseList.size() == 1) {
				for (Map<String, Object> eachMap : contactPurcahseList) {
					if(eachMap.containsKey("tot_purchase_amt")){
						lifeTimePurchaseValue = eachMap.get("tot_purchase_amt") != null ? eachMap.get("tot_purchase_amt").toString() : "0.00";
						logger.info("purchase value = "+lifeTimePurchaseValue);
					}
				}
			}
		}*/
	/*	if(contacts != null && contacts.getExternalId() != null){
			lifeTimePurchaseValue = Utility.getAmountInUSFormat(contactsDao.findContactPurchaseDetails(currUser.getUserId(),contacts.getContactId()));
		}*/
		...
		//lifeTimeLoyaltyPurchaseValue = contactsLoyalty.getLifeTimePurchaseValue() == null ? 0.0 : contactsLoyalty.getLifeTimePurchaseValue();//loyaltyTransactionChildDao.getLifeTimeLoyaltyPurchaseValue(contactsLoyalty.getUserId(), contactsLoyalty.getProgramId(), contactsLoyalty.getLoyaltyId());
	...
	private void displayTransactionSummary(ContactsLoyalty contactsLoyalty) {
		...
		if(contactsLoyalty.getRewardFlag() != null && OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_G.equalsIgnoreCase(contactsLoyalty.getRewardFlag())){
			...
			//Gift Card
			totalGiftTopupAmountTSummaryLblId.setValue(contactsLoyalty.getTotalGiftAmount() == null ? "$ 0.00" : Utility.getAmountInUSFormat(contactsLoyalty.getTotalGiftAmount() ));
			giftAmountRedeemedTSummaryLblId.setValue(contactsLoyalty.getTotalGiftRedemption() == null ? "$ 0.00" : Utility.getAmountInUSFormat(contactsLoyalty.getTotalGiftRedemption() ));
		}
		else if(contactsLoyalty.getRewardFlag() != null && OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_L.equalsIgnoreCase(contactsLoyalty.getRewardFlag())){
			...
			totalAmountEarnedTSummaryLblId.setValue(contactsLoyalty.getTotalGiftcardAmount() == null ? "$ 0.00" :Utility.getAmountInUSFormat(contactsLoyalty.getTotalGiftcardAmount() ));
			amountRedeemedTSummaryLblId.setValue(contactsLoyalty.getTotalGiftcardRedemption() == null ? "$ 0.00" : Utility.getAmountInUSFormat(contactsLoyalty.getTotalGiftcardRedemption() ));
	...
	private void displayFinalBalances(ContactsLoyalty contactsLoyalty) {
		...
		finalBalanceRewardsLblId.setValue(contactsLoyalty.getGiftcardBalance() == null ? "$ 0.00":Utility.getAmountInUSFormat(contactsLoyalty.getGiftcardBalance()));
	...
	private void updateLoyaltyData(ContactsLoyalty contactsLoyalty) {
		...
		//balance rewards	
		balanceCurrencyLblId.setValue(ltyObj.getGiftcardBalance() == null ? "$ 0.00":Utility.getAmountInUSFormat(ltyObj.getGiftcardBalance() ));
		//total amount earned	
		totalAmountEarnedTSummaryLblId.setValue(ltyObj.getTotalGiftcardAmount() == null ? "$ 0.00" :Utility.getAmountInUSFormat(ltyObj.getTotalGiftcardAmount() ));

		//BALANCE REWARDS
		finalBalanceRewardsLblId.setValue(ltyObj.getGiftcardBalance() == null ? "$ 0.00":Utility.getAmountInUSFormat(ltyObj.getGiftcardBalance()));
		//BALANCE POINTS
			...
			if(contactsLoyaltyObj != null) {
				...
				if(loyaltyProgram.getStatus() != null && OCConstants.LOYALTY_PROGRAM_STATUS_ACTIVE.equalsIgnoreCase(loyaltyProgram.getStatus()) && OCConstants.LOYALTY_MEMBERSHIP_STATUS_ACTIVE.equalsIgnoreCase(contactsLoyaltyObj.getMembershipStatus()) ){
					...
					if(amount[1].startsWith("-") || amount[1].startsWith("+")) {
						...
						if(addTypeLbId.isVisible() && addTypeLbId.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold")) {
						...
						if(isValidNumber) {
							...
							if(Messagebox.show("Are you sure you want to add loyalty "+displayLabel+" ? ", "Confirm" ,Messagebox.OK | Messagebox.CANCEL, 
									Messagebox.QUESTION) == Messagebox.OK) {
								...
								if(!OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_G.equalsIgnoreCase(contactsLoyaltyObj.getRewardFlag())){
									...
									try {
										if(addTypeLbId.isVisible() && addTypeLbId.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold")) {
											...
											createPurchaseTransaction(contactsLoyaltyObj, balanceToAdd, valueCode, Constants.STRING_NILL,activationDate,OCConstants.LOYALTY_PROGRAM_TRANS_STATUS_NEW, OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT_ADD);
											displayFinalHoldBalances(contactsLoyaltyObj);
											addTypeLbId.setSelectedIndex(0);
											...
											if(flag){	
												...
												if(contactsLoyaltyObj.getProgramTierId() == null) {
													...
													if (loyaltyProgramTier == null) {
														// CALL BONUS
														updateThresholdBonus(contactsLoyaltyObj, loyaltyProgram, fromLtyBalance, fromAmtBalance, null,false);
														ltyPrgmService.saveOrUpdateContactLoyalty(contactsLoyaltyObj);
												...
												// CALL BONUS
												updateThresholdBonus(contactsLoyaltyObj, loyaltyProgram, fromLtyBalance, fromAmtBalance, loyaltyProgramTier,tierUpgd);
					...
					if(amount[1].startsWith("-") || amount[1].startsWith("+")) {
						...
						if(isValidNumber) {
							...
							if(valueCode.equals("Amount")) {
								...
								if(subTypeLbId.isVisible() && subTypeLbId.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold")) {
									if((contactsLoyaltyObj.getHoldAmountBalance() == null || contactsLoyaltyObj.getHoldAmountBalance().toString().isEmpty())) {
								...
								if(!OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_G.equalsIgnoreCase(contactsLoyaltyObj.getRewardFlag())){
									...
									try {
										if(subTypeLbId.isVisible() && subTypeLbId.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold")) {
												...
												createPurchaseTransaction(contactsLoyaltyObj, balanceToSub, valueCode, Constants.STRING_NILL, activationDate, null, OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT_SUB);
												if(valueCode.equals(OCConstants.LOYALTY_TYPE_POINTS)){
											...
											if(valueCode.equalsIgnoreCase("Amount")) {
												...
												contactsLoyaltyObj.setTotalLoyaltyEarned(totalLoyaltyEarned);
												ltyPrgmService.saveOrUpdateContactLoyalty(contactsLoyaltyObj);
												//contactsLoyaltyObj = reloadLoyaltyObj(contactsLoyaltyObj);
												logger.info("New LoyaltyBalance is ::::::::::::"+loyaltyBalance);
											}
	...
	private void settingLoyaltyTransactionRow(LoyaltyTransactionChild loyaltyTransactionChild) {
		...
		if((receiptNo == null || receiptNo.isEmpty())&&(loyaltyTransactionChild.getDocSID() != null)){//|| loyaltyTransactionChild.getReceiptNumber().isEmpty() removed this APP-3126
		...
		listCell = new Listcell();
		listCell.setStyle("padding-left:10px;");
		//String transType = loyaltyTransactionChild.getTransactionType().equalsIgnoreCase(OCConstants.LOYALTY_TRANSACTION_ISSUANCE)?loyaltyTransactionChild.getEnteredAmountType().equalsIgnoreCase(OCConstants.LOYALTY_TYPE_PURCHASE)?"Loyalty Issuance":"Gift Issuance":loyaltyTransactionChild.getTransactionType();
		...
		if (((loyaltyTransactionChild.getEarnType() != null && 
				((String) loyaltyTransactionChild.getEarnType()).equalsIgnoreCase(OCConstants.LOYALTY_POINTS)) 
...
/*
	...
	public void onSelect$searchbyLbId(){
		...
		searchByTbId.setValue("");//APP-3126(Comments)
	...
	private void exportCSV(String value, int[] indexes) {
		...
		if(type.contains("csv")){
			...
			try {
				...
				/*String query = "SELECT membership_number, created_date, docsid, store_number, transaction_type, entered_amount_type, entered_amount, excluded_amount, amount_balance, " +
						" gift_balance, points_balance, description2, hold_points, hold_amount  FROM loyalty_transaction_child " +
				...
				/*String query = "SELECT membership_number, created_date, docsid, store_number, transaction_type, entered_amount_type, entered_amount, excluded_amount, amount_balance, " +
						" gift_balance, points_balance, description2, hold_points, hold_amount,amount_difference,earn_type ,points_difference  FROM loyalty_transaction_child " +
				" WHERE user_id ="+ currUser.getUserId() + " AND loyalty_id in ("+transferLoyaltyId+")"+
				" AND transaction_type IN('"+OCConstants.LOYALTY_TRANS_TYPE_ISSUANCE+"','"+OCConstants.LOYALTY_TRANS_TYPE_REDEMPTION+"','"+
				OCConstants.LOYALTY_TRANS_TYPE_ENROLLMENT+"','"+OCConstants.LOYALTY_TRANS_TYPE_BONUS+"','"+OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT+"','"+OCConstants.LOYALTY_TRANS_TYPE_RETURN+"')"+
				...
				while(resultSet.next()){
					...
					/*if(indexes[6]==0) {
						double enteredAmt = resultSet.getDouble("entered_amount") == 0 ? 0.00 : resultSet.getDouble("entered_amount");
						double excludeAmt = resultSet.getDouble("excluded_amount") == 0 ? 0.00 : resultSet.getDouble("excluded_amount");
						//sb.append("\"");sb.append(Utility.getAmountInUSFormat(enteredAmt-excludeAmt)); sb.append("\",");
						String earnType=resultSet.getString("earn_type")!=null ? resultSet.getString("earn_type") :"";
						String enteredAmtType=resultSet.getString("entered_amount_type")!=null? resultSet.getString("entered_amount_type"):"";
	                    String enterAmount=""; 
					    if(enteredAmtType.equalsIgnoreCase(OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT_ADD) || enteredAmtType.equalsIgnoreCase(OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT_SUB))
					           if(earnType.equalsIgnoreCase(OCConstants.LOYALTY_POINTS))
					    	       enterAmount = (enteredAmt-excludeAmt)!=0 ?Integer.valueOf ((int) (enteredAmt-excludeAmt))+" Points":"";
						  ...
						  enterAmount = (enteredAmt-excludeAmt)!=0 ? "$ "+f.format(enteredAmt-excludeAmt):"$ 0.00";
						  sb.append("\"");sb.append(enterAmount); sb.append("\","); 
					}*/
...
*/					if(resultSet.getDouble("amount_balance") != 0 &&  
						...
						balanceStr = "Gift : " + Utility.getAmountInUSFormat(resultSet.getDouble("gift_balance")) +" Loyalty : "+ Utility.getAmountInUSFormat(resultSet.getDouble("amount_balance"));
					}else if(resultSet.getDouble("amount_balance") == 0 && resultSet.getDouble("gift_balance") != 0){
						balanceStr = Utility.getAmountInUSFormat(resultSet.getDouble("gift_balance"));
					}else if(resultSet.getDouble("amount_balance") != 0
							&& resultSet.getDouble("gift_balance") == 0){
						balanceStr = Utility.getAmountInUSFormat(resultSet.getDouble("amount_balance"));
					}else {
						balanceStr = Utility.getAmountInUSFormat(0.00)+"";
					}
					if(indexes[9]==0) {
	...
	private void getDateValues() {
		...
		else if(timeDurLbId.getSelectedItem().getLabel().equalsIgnoreCase("Today")) {
	...
	/**
	 * This method clear's Last Purchase
	 */
	/*private void clearLastPurchase() {
		logger.debug(">>>>>>>>>>>>> entered in clearLastPurchase");
		visitOnLblId.setValue(Constants.STRING_NILL);
		storeLblId.setValue(Constants.STRING_NILL);
		purchaseAmountLblId.setValue(Constants.STRING_NILL);
		issuedAmountLblId.setValue(Constants.STRING_NILL);
		earningLblId.setValue(Constants.STRING_NILL);
		redemptionLblId.setValue(Constants.STRING_NILL);
		itemPurchasedLblId.setValue(Constants.STRING_NILL);
		promotionsAppliedLblId.setValue(Constants.STRING_NILL);	
		promotionDetailsDivId.setVisible(false);
		skuDetailsDivId.setVisible(false);
		logger.debug("<<<<<<<<<<<<< completed clearLastPurchase");
	}//clearLastPurchase
*/
	/**
	 ...
	 * @throws Exception
	 */
	private void updateThresholdBonus(ContactsLoyalty contactsLoyalty, LoyaltyProgram program, 
			Double fromLtyBalance, Double fromAmtBalance, LoyaltyProgramTier loyaltyProgramTier,boolean tierUpgd) throws Exception {
...
*/
	...
	private LoyaltyProgramTier findTier(ContactsLoyalty contactsLoyalty) throws Exception {
		...
		if(OCConstants.LOYALTY_LIFETIME_POINTS.equals(tiersList.get(0).getTierUpgdConstraint())){
			...
			//LoyaltyTransactionChildDao loyaltyTransactionChildDao = (LoyaltyTransactionChildDao)ServiceLocator.getInstance().getDAOByName(OCConstants.LOYALTY_TRANSACTION_CHILD_DAO);
			//totPurchaseValue = contactsLoyalty.getLifeTimePurchaseValue() == null ? 0.0 : contactsLoyalty.getLifeTimePurchaseValue();//Double.valueOf(loyaltyTransactionChildDao.getLifeTimeLoyaltyPurchaseValue(contactsLoyalty.getUserId(), contactsLoyalty.getProgramId(), contactsLoyalty.getLoyaltyId()));
...
private String[] applyConversionRules(ContactsLoyalty contactsLoyalty, LoyaltyProgramTier tier){
	...
	private LoyaltyTransactionChild createPurchaseTransaction(ContactsLoyalty loyalty, double adjustValue, String earnType, 
		...
		try{
			...
			if(earnType.equals(OCConstants.LOYALTY_TYPE_POINTS)){
				long adjustValueLong = (long) adjustValue;//APP-823
