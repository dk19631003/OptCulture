 ...
 * @author vinod.bokare
 */
public class LoyaltyCustomerLookUpController extends GenericForwardComposer<Window> implements EventListener{


	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getLogger(Constants.SUBSCRIBER_LOGGER);
	private Users currUser;
	private Window loyaltyCustomerLookUpWinId,latestDetailsSubWinId,custExport;

	private Div datesDivId,skuDetailsDivId,promotionDetailsDivId,whichprogramDivId,custExport$chkDivId,
	moveTierDivId,searchCriteriaDivId,latestDetailsSubWinId$descDivId,element1Id,element2Id,element3Id,balance1Id,balance2Id,balance3Id,expBalance1Id,expBalance2Id,expBalance3Id,expElement1Id,expElement2Id,expElement3Id;
	private Div latestDetailsSubWinId$viewSkuDetailsDivId,latestDetailsSubWinId$viewPromotionDetailsDivId,valueDiv,latestDetailsSubWinId$foundMultipleMatchesDivId,latestDetailsSubWinId$closeCardDivId;
	private Rows  latestDetailsSubWinId$viewAllSKURowsId,latestDetailsSubWinId$promotionRowsId;
	...
	private Label balancePointLblId,balanceCurrencyLblId,expiredPointsLblId,expiredAmountLblId,
				  	balanceGiftAmountLblId,expiredGiftAmountLblId,transferedOrNewCardLblId,valueCodesId,balanceId;
	...
	private Textbox searchByTbId,addTbId,subTbId,first_nameTbId,last_nameTbId,ltyPwdTbMvtierdId,subTb2Id;
	private Label programNameLbId,cardSetNameLbId,currentTierLbId,descpopupId$descLableId;
	private Button suspendMembershipBtnId,addBtnId,subtractBtnId,submitPwdBtnMvtierId,moveTierBtnId,loadBtnId;
	...
	private Listitem listItemHoldPts,listItemHoldAmt;
	...
	private Textbox testId,testExpiredId;
	private Rows discountsRowsId,latestDetailsSubWinId$multipleFoundRowId;
	private Div discountGbId;
	/**
	 * default constructor
	 */
	public LoyaltyCustomerLookUpController(){
		super();
...
*/
		...
		if(transferCardNO!=null){
			searchbyLbId.setSelectedIndex(3);
...
/*	    String currSymbol = currUser.getCountryCurrency();
	...
	private void adjustmentValueCodes(){
		LoyaltyBalanceDao loyaltyBalanceDao;
		ValueCodesDao valueCodesDao;
		try {
			
			logger.info("inside adjustment value codes----");
			
			valueCodesDao=(ValueCodesDao) ServiceLocator.getInstance().getDAOByName(OCConstants.VALUE_CODES_DAO);
			List<ValueCodes> valueCodesList =valueCodesDao.findValueCodeBy(currUser.getUserOrganization().getUserOrgId());

						
			for (ValueCodes valueCode : valueCodesList) {
				if(!valueCode.isAssociatedWithFBP()){
					String vCode=valueCode.getValuCode();
					Listitem listItem = new Listitem();		
					listItem.setLabel(vCode);
					listItem.setValue(vCode);
					listItem.setParent(addLbId2);
				}
			}
	...
	/*private String displayActiveValueCodes(){
		try {
			LoyaltyBalanceDao loyaltyBalanceDao;
		...
		if(contactsLoyaltyObj!=null){
			...
			List<LoyaltyBalance> valueCodesList =loyaltyBalanceDao.findBy(currUser.getUserId(),contactsLoyaltyObj.getLoyaltyId());
			List<ValueCodes> nonFbpValueCodeList = valueCodesDao.findNonFBPValueCodes(currUser.getUserOrganization().getUserOrgId());
			
			StringBuffer valueCodeBuffer=null;
			valueCodeBuffer = new StringBuffer();
			
			if(valueCodesList!=null && !valueCodesList.isEmpty() && nonFbpValueCodeList!=null && !nonFbpValueCodeList.isEmpty()){
				
				for (LoyaltyBalance loyaltyBalance : valueCodesList) {
					
					for(ValueCodes nonFbpValueCode : nonFbpValueCodeList){
						
						if(loyaltyBalance.getValueCode().equals(nonFbpValueCode.getValuCode())){
							
							flag=true;
							
							if(valueCodeBuffer.length() > 0) valueCodeBuffer.append(",");
							
							valueCodeBuffer.append(loyaltyBalance.getValueCode()+" "+":"+" "+loyaltyBalance.getBalance()); 
							
						}
					}
				}
			}else return null;
			logger.info("valueCodesList----"+valueCodesList.toString());
			logger.info("valueCodeBuffer---"+valueCodeBuffer.toString());
			
			if(!flag) return null;
			
			return valueCodeBuffer.toString();
		}else return null;
		} catch (Exception e) {
			logger.info("Exception --"+e);
			return null;
	...
	//		boolean flag=false;
		...
		if(contactsLoyaltyObj!=null){
			...
			List<LoyaltyBalance> valueCodesList =loyaltyBalanceDao.findOnlyRewardsBy(currUser.getUserId(),contactsLoyaltyObj.getLoyaltyId(), currUser.getUserOrganization().getUserOrgId());
	...
	/*private String displayExpiredValueCodes(){
				...
				for (LoyaltyBalance loyaltyBalance : valueCodesList) {
					...
					for(ValueCodes nonFbpValueCode : nonFbpValueCodeList){
						...
						if(loyaltyBalance.getValueCode().equals(nonFbpValueCode.getValuCode())){
							...
							Long expiredBalance=(long) (loyaltyBalance.getTotalExpiredBalance()==null?0:loyaltyBalance.getTotalExpiredBalance());
							
							if(valueCodeBuffer.length() > 0) valueCodeBuffer.append(",");
							
							valueCodeBuffer.append(loyaltyBalance.getValueCode()+" "+":"+" "+expiredBalance); 
						}
					}
				}
			}else return null;
	...
	private String fetchStoreName(String store_number, String subsidiary_number){
		 for (Map<String, Object> eachMap: storeNumberNameMapList) {
			 if(eachMap.get("subsidary_id")!=null)
			 {
				 if(eachMap.get("home_store_id").toString().equals(store_number)&&
			     	eachMap.get("subsidary_id").toString().equals(subsidiary_number)){
							 if(eachMap.get("store_name") != null || eachMap.get("subsidiary_name")==null){
	...
	try {
		...
		loyaltyCustomerLookUpWinId.setHeight((900+transferListLbId.getChildren().size()*30+heightForTrans)+"px");*/
		displayChangingTier();
		String jsonActiveValueCodes = displayActiveValueCodes();
		String jsonExpiredValueCodes=displayExpiredValueCodes();
		
		/*if(jsonActiveValueCodes!=null){
			logger.info("inside if");
			Clients.evalJavaScript("display();");
		}*/
		
	//	addLbId.getItemAtIndex(2).setVisible(true);
		onSelect$addLbId();
		
		adjustmentValueCodes();
		editContactDivId.setVisible(false);
		logger.debug(">>>>>> end of the method :: onClick$filterBtnId");
		testId.setValue(jsonActiveValueCodes);
		Clients.evalJavaScript("test();");
		testExpiredId.setValue(jsonExpiredValueCodes);
		Clients.evalJavaScript("testExpired();");
		/*if(jsonActiveValueCodes!=null && !jsonActiveValueCodes.isEmpty()){
		}
		if(jsonExpiredValueCodes!=null && !jsonActiveValueCodes.isEmpty()){
		}else{
			expBalance1Id.setVisible(false);
			expBalance2Id.setVisible(false);
			expBalance3Id.setVisible(false);
			expElement1Id.setVisible(false);
			expElement2Id.setVisible(false);
			expElement3Id.setVisible(false);
		}*/
	}catch(Exception e) {
		logger.info("while filtering lookup "+e);
	}
	}//onClick$filterBtnId
	
	public void onClick$loadBtnId(){
		...
		try {
			...
			if(Constants.CARD_NUMBER.equals(searchKey)){
				if(validateMembershipNumber(searchVal)){
					...
					if(contactsLoyaltyObj != null){
						if(contactsLoyaltyObj.getMembershipStatus().equalsIgnoreCase(OCConstants.LOYALTY_MEMBERSHIP_STATUS_CLOSED)){
							...
							displayTransactionSummary(contactsLoyaltyObj);
				...
				List<Map<String, Object>> contactLoyalties = null;
				if(Constants.FULL_NAME.equals(searchbyLbId.getSelectedItem().getValue())) {
					...
					contactLoyalties = ltyPrgmService.getContactLtyBySearchCriteria(currUser, searchKey, searchVal);
			...
			logger.debug(">>>>>> end of the method :: onClick$loadBtnId");
			}catch(Exception e) {
				logger.info("while onClick$loadBtnId details"+e);
			}
	}
	...
	try {
		...
		emailTbId.setValue(contact.getEmailId()!=null?contact.getEmailId():"");
		phoneTbId.setValue(contact.getMobilePhone()!=null?contact.getMobilePhone():"");
	}
		catch(Exception e) {
			logger.debug("** Error while editing contacts**");
			MessageUtil.setMessage("Error while editing contact details.", "red");
	...
	public void onClick$saveBtnId(){
		...
		try{
			...
			if( !emailTbId.getValue().trim().isEmpty() && 
					(contact.getEmailId()==null || !contact.getEmailId().equalsIgnoreCase(emailTbId.getValue().trim()))){
			...
			if(!phoneTbId.getValue().trim().isEmpty() && 
					(contact.getMobilePhone()==null || !contact.getMobilePhone().equalsIgnoreCase(phoneTbId.getValue()))){
	...
	private void fetchLoyaltyDetails() {
		...
		logger.debug(">>>>>> end of the method :: fetchLoyaltyDetails");
			...
			logger.info("while fetching loyalty details"+e);
		}
...
//	}//onClick$selectMultipleChoiceBtnId$latestDetailsSubWinId
	...
	public void openMembership(String contLoyaltyIdString){
		logger.debug(">>>>>>>>>>>>> entered ionClick$selectMultipleChoiceBtnId$latestDetailsSubWinIdn onClick$selectMultipleChoiceBtnId$latestDetailsSubWinId");
		LoyaltyProgramService ltyPrgmService = new LoyaltyProgramService();
...
//		LoyaltyTransactionChild loyaltyTransactionChild = ltyPrgmService.getLoyaltyTransactionChild(contactsLoyalty.getCardNumber(),currUser.getUserId());
		...
		constructDisplayTemplate(currUser,contactsLoyalty);
		displaybrdrLOut.setHeight((700+transferListLbId.getChildren().size()*30+heightForTrans)+"px");
		loyaltyCustomerLookUpWinId.setHeight((900+transferListLbId.getChildren().size()*30+heightForTrans)+"px");
	...
	private void displayBalanceSummary(ContactsLoyalty contactsLoyalty) {
		...
		if(contactsLoyalty.getRewardFlag() != null && OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_G.equalsIgnoreCase(contactsLoyalty.getRewardFlag())){
			...
			balancePointLblId.setValue(contactsLoyalty.getLoyaltyBalance() == null ? "0":contactsLoyalty.getLoyaltyBalance().intValue()+" ");
			...
			expiredPointsLblId.setValue(contactsLoyalty.getExpiredPoints() == null ? "0":contactsLoyalty.getExpiredPoints().intValue()+" ");
	...
	*/}///displaySalesSummary
	...
	//APP-3219
	private LoyaltyTransactionChild getLatestTransaction(Long loyaltyId) {
	...
	private void displayMembershipSummary(ContactsLoyalty contactsLoyalty,Contacts contacts) {
		...
		storeNumberMSummaryLblId.setValue(contactsLoyalty.getPosStoreLocationId() == null ? Constants.STRING_NILL : fetchStoreName(contactsLoyalty.getPosStoreLocationId(),contactsLoyalty.getSubsidiaryNumber()));
		Double lifeTimeLoyaltyPurchaseValue = 0.00;
		lifeTimeLoyaltyPurchaseValue = LoyaltyProgramHelper.getLPV(contactsLoyalty);
	...
	private void updateLoyaltyData(ContactsLoyalty contactsLoyalty) {
		...
		//balance points
		balancePointLblId.setValue(ltyObj.getLoyaltyBalance() == null ? "0":ltyObj.getLoyaltyBalance().intValue()+" ");
		//balance rewards	
...
//removed loyalty
								...
								if(!OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_G.equalsIgnoreCase(contactsLoyaltyObj.getRewardFlag())){ //ask mam
									...
									try {
										if((addLbId2.isVisible() && addLbId2.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold-Points"))||(addLbId2.isVisible() && addLbId2.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold-Amount"))) {
											...
											createPurchaseTransaction(contactsLoyaltyObj, balanceToAdd, valueCode, Constants.STRING_NILL,activationDate,OCConstants.LOYALTY_PROGRAM_TRANS_STATUS_NEW, OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT_ADD,description2,null);
											...
											if(valueCode.equalsIgnoreCase("Amount")) {
												...
												String valueCodeName =addLbId2.getSelectedItem().getLabel();
												LoyaltyBalanceDao loyaltyBalanceDao;
												ValueCodesDao valueCodesDao;
												...
												loyaltyBalanceDao = (LoyaltyBalanceDao) ServiceLocator.getInstance().getDAOByName(OCConstants.LOYALTY_BALANCE_DAO);
												List<LoyaltyBalance> valueCodesList =loyaltyBalanceDao.findbyCradNo(contactsLoyaltyObj.getCardNumber(), currUser.getUserId());
												valueCodesDao=(ValueCodesDao) ServiceLocator.getInstance().getDAOByName(OCConstants.VALUE_CODES_DAO);
												
												List<ValueCodes> vCodesList =valueCodesDao.findValueCodeBy(currUser.getUserOrganization().getUserOrgId());
												Set<String> vcodesSet=new HashSet<String>();
											try {
												if(valueCodesList==null || valueCodesList.isEmpty()){
													lbal=new LoyaltyBalance();
													lbal.setBalance(balanceToAdd.longValue());
													lbal.setOrgId(currUser.getUserOrganization().getUserOrgId());
													...
													lbal.setValueCode(valueCodeName);
													...
													loyaltyBalanceDaoforDML.saveOrUpdate(lbal);
													flag=true;
												}else{
												
												   for(LoyaltyBalance loyaltyBalance : valueCodesList){
													Long balance=loyaltyBalance.getBalance()==null?0:loyaltyBalance.getBalance();
													if(loyaltyBalance.getValueCode().equalsIgnoreCase(valueCodeName)){
														...
														lbal=loyaltyBalance;
															...
															vcodesSet.add(valueCodeName);
															flag=true;
													}
												}
													//for(ValueCodes vcodes : vCodesList){
														if(!vcodesSet.contains(valueCodeName)){
															lbal=new LoyaltyBalance();
															lbal.setBalance(balanceToAdd.longValue());
															lbal.setOrgId(currUser.getUserOrganization().getUserOrgId());
															lbal.setMemberShipNumber(contactsLoyaltyObj.getCardNumber());
							...
							if(valueCode.equals("Amount")) {
								...
								if((addLbId2.isVisible() && addLbId2.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold-Points"))||(addLbId2.isVisible() && addLbId2.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold-Amount"))) {
									...
									if(balanceToSub > existingLoyaltyBalance){
										...
										balanceToSub =(long) balanceToSub;
										double totalConvertedPoints = getAutoConvertionReversalVal(contactsLoyaltyObj, tier);
										totalConvertedPoints=totalConvertedPoints+existingLoyaltyBalance;
										if(totalConvertedPoints<balanceToSub) {
									...
									return;
										...
										try {
												Set<String> vcodesSet=new HashSet<String>();
												 ...
												 if(!vcodesSet.contains(valueCodeName)){
													 MessageUtil.setMessage("Balance is insufficient to substract.", "color:red;");
													 ltyPwdTbSubId.setText("");
													 addTbId.setText("");
													 addTbId.setFocus(true);
													 return;
											...
											for(LoyaltyBalance loyaltyBalance : valueCodesList){
												if(loyaltyBalance.getValueCode().equalsIgnoreCase(valueCodeName)){
													 existingVCBal=loyaltyBalance.getBalance()==null?0:loyaltyBalance.getBalance();
													 logger.info("Temp ::"+balanceToSub+"Existing existing VC Balance --"+existingVCBal);
													 if(balanceToSub > existingVCBal){
														 MessageUtil.setMessage("Value code to be subtracted should be less than or equal to existing Value Code.", "color:red;");
														 ltyPwdTbSubId.setText("");
														 addTbId.setText("");
														 addTbId.setFocus(true);
														 return;
								...
								if(!OCConstants.LOYALTY_MEMBERSHIP_REWARD_FLAG_G.equalsIgnoreCase(contactsLoyaltyObj.getRewardFlag())){
									...
									try {
										if((addLbId2.isVisible() && addLbId2.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold-Points"))||(addLbId2.isVisible() && addLbId2.getSelectedItem().getValue().toString().equalsIgnoreCase("Hold-Amount"))) {
												...
												createPurchaseTransaction(contactsLoyaltyObj, balanceToSub, valueCode, Constants.STRING_NILL, activationDate, null, OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT_SUB,description2,null);
												if(valueCode.equals(OCConstants.LOYALTY_TYPE_POINTS)){
											...
											if(valueCode.equalsIgnoreCase("Amount")) {
												...
												logger.info("Previous LoyaltyBalance was ::::::::::::"+loyaltyBalance);
												Double totalConvertedPoints=getAutoConvertionReversalVal(contactsLoyaltyObj, tier);
												logger.info("TotalPoints :"+totalConvertedPoints);
												...
												double amtToSub=loyaltyBalance/tier.getConvertFromPoints();
												int convAmt = (int)amtToSub;
						    					contactsLoyaltyObj.setGiftcardBalance(tier.getConvertToAmount()*convAmt);
												contactsLoyaltyObj.setLoyaltyBalance(loyaltyBalance % tier.getConvertFromPoints());
												contactsLoyaltyObj.setTotalLoyaltyEarned(totalLoyaltyEarned);
												ltyPrgmService.saveOrUpdateContactLoyalty(contactsLoyaltyObj);
												logger.info("New LoyaltyBalance is ::::::::::::"+loyaltyBalance);
											}
											...
											try {
												for(LoyaltyBalance loyaltyBalance : valueCodesList){
													Long balance=loyaltyBalance.getBalance()==null?0:loyaltyBalance.getBalance();
													if(loyaltyBalance.getValueCode().equalsIgnoreCase(valueCodeName)){
														balance=(long) (balance-balanceToSub);
														loyaltyBalance.setBalance(balance);
														valueCode=loyaltyBalance.getValueCode();
														ltyBalanceObj=loyaltyBalance;
														
															loyaltyBalanceDaoforDML.saveOrUpdate(loyaltyBalance);
													}
												}
												deductRewardAmtFromExpiryTable(contactsLoyaltyObj,contactsLoyaltyObj.getUserId(),balanceToSub,valueCode);
	...
	public void onSelect$pageSizeLbId(){
		...
		try {
			...
			loyaltyCustomerLookUpWinId.setHeight((900+heightForTrans+transferListLbId.getChildren().size()*30)+"px");
	...
	private void settingLoyaltyTransactionRow(LoyaltyTransactionChild loyaltyTransactionChild) {
		...
		if((receiptNo == null || receiptNo.isEmpty()) &&loyaltyTransactionChild.getDocSID() != null ){
		...
		listCell = new Listcell();
		listCell.setStyle("padding-left:10px;");
		listCell.setLabel(loyaltyTransactionChild.getStoreNumber()!=null?fetchStoreName(loyaltyTransactionChild.getStoreNumber(), loyaltyTransactionChild.getSubsidiaryNumber()):"--");
		listCell.setTooltiptext(loyaltyTransactionChild.getStoreNumber()!=null?fetchStoreName(loyaltyTransactionChild.getStoreNumber(), loyaltyTransactionChild.getSubsidiaryNumber()):"--");
		listCell.setParent(listItem);
		...
		String valueEnteredTypString = Constants.STRING_NILL;
		if (((loyaltyTransactionChild.getEarnType() != null && ((String) loyaltyTransactionChild.getEarnType()).equalsIgnoreCase(OCConstants.LOYALTY_POINTS))
			...
			Double enteredAmt = loyaltyTransactionChild.getEnteredAmount() == null ? 0 : Double.parseDouble(loyaltyTransactionChild.getEnteredAmount().toString());
			 valueEntered = ((String) loyaltyTransactionChild.getTransactionType()).equalsIgnoreCase(OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT) ? 
					...
					: "0")
					: "" + enteredAmt.longValue();
					valueEnteredTypString =" Points";
			listCell.setLabel(valueEntered + valueEnteredTypString);
			listCell.setTooltiptext(valueEntered + valueEnteredTypString);
		...
		if(loyaltyTransactionChild.getValueCode()!=null) {
			valueEnteredTypString =" Reward";
			listCell.setLabel(valueEntered + valueEnteredTypString);
		}
...
//                logger.info(" point diff "+pointDiff);
		...
		String valueDiff = Constants.STRING_NILL; 
			...
			if(loyaltyTransactionChild.getAmountDifference()!=null && !loyaltyTransactionChild.getAmountDifference().isEmpty() && Double.parseDouble(loyaltyTransactionChild.getAmountDifference()) != 0 ) {
				valueDiff = loyaltyTransactionChild.getAmountDifference();
				valueCode = OCConstants.LOYALTY_TYPE_CURRENCY;
			}else if(loyaltyTransactionChild.getPointsDifference()!=null && !loyaltyTransactionChild.getPointsDifference().isEmpty() && Double.parseDouble(loyaltyTransactionChild.getPointsDifference()) != 0) {
				valueDiff = Utility.truncateToInteger(loyaltyTransactionChild.getPointsDifference());
				valueCode = OCConstants.LOYALTY_TYPE_POINTS;
			}else if(loyaltyTransactionChild.getEnteredAmountType()!=null && loyaltyTransactionChild.getEnteredAmountType().equalsIgnoreCase(OCConstants.LOYALTY_TRANS_ENTEREDAMOUNT_TYPE_REWARD) ) {
				if(loyaltyTransactionChild !=null && loyaltyTransactionChild.getRewardDifference()!=null && !loyaltyTransactionChild.getRewardDifference().isEmpty() &&
					!loyaltyTransactionChild.getRewardDifference().equalsIgnoreCase("null") && Double.parseDouble(loyaltyTransactionChild.getRewardDifference()) != 0 ){
				valueDiff = loyaltyTransactionChild.getRewardDifference()!=null?loyaltyTransactionChild.getRewardDifference():"0";
				valueCode = loyaltyTransactionChild.getEarnType().equals(OCConstants.LOYALTY_EXPIRY_TYPE_AMOUNT)
					? OCConstants.LOYALTY_TYPE_CURRENCY
					: loyaltyTransactionChild.getEarnType();
				}else {
					
					valueDiff = loyaltyTransactionChild.getEarnedPoints()!=null && 
							loyaltyTransactionChild.getEarnedPoints() != 0 ?loyaltyTransactionChild.getEarnedPoints()+"":
								loyaltyTransactionChild.getEarnedAmount()!=null &&
										loyaltyTransactionChild.getEarnedAmount()!=0 ? loyaltyTransactionChild.getEarnedAmount()+"":
											loyaltyTransactionChild.getEnteredAmount()+"";
					
					valueCode = loyaltyTransactionChild.getEarnType()!=null && !loyaltyTransactionChild.getEarnType().isEmpty() &&
							loyaltyTransactionChild.getEarnType().equals(OCConstants.LOYALTY_EXPIRY_TYPE_AMOUNT)
						? OCConstants.LOYALTY_TYPE_CURRENCY
						: loyaltyTransactionChild.getValueCode();
				}
			}
		//Setting difference
		listCell=new Listcell();
		listCell.setStyle("padding-left:10px;");
		listCell.setLabel(valueDiff);
		listCell.setTooltiptext(valueDiff);
		listCell.setParent(listItem);

		//Setting Value code		
		listCell=new Listcell();
		listCell.setStyle("padding-left:10px;");
		listCell.setLabel(valueCode);
		listCell.setTooltiptext(valueCode);
		listCell.setParent(listItem);
...
/*
	...
	public void createWindow()	{
		...
		try {
			...
			tempChk2.setChecked(true);
			//Difference in value
			tempChk2 = new Checkbox("Difference in value");
			tempChk2.setSclass("custCheck");
			tempChk2.setParent(custExport$chkDivId);
			tempChk2.setChecked(true);
			//ValueCode
			tempChk2 = new Checkbox("Value Code");
			tempChk2.setSclass("custCheck");
			tempChk2.setParent(custExport$chkDivId);
			tempChk2.setChecked(true);
			//Balance Amount
				...
				HSSFWorkbook hwb = new HSSFWorkbook();
				 ...
				 jdbcResultsetHandler = new JdbcResultsetHandler();
					...
					String query = "SELECT membership_number, created_date, docsid, subsidiary_number, store_number, transaction_type, entered_amount_type, entered_amount, excluded_amount, amount_balance, " +
							" gift_balance, points_balance, description2, hold_points, hold_amount,amount_difference,earn_type ,points_difference,reward_difference,receipt_number  FROM loyalty_transaction_child " +
					...
					OCConstants.LOYALTY_TRANS_TYPE_ENROLLMENT+"','"+OCConstants.LOYALTY_TRANS_TYPE_EXPIRY+"','"+OCConstants.LOYALTY_TRANS_TYPE_BONUS+"','"+OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT+"','"+OCConstants.LOYALTY_TRANS_TYPE_RETURN+"')"+
					...
					int rowId=1;
						while(resultSet.next()) {
							row = sheet.createRow((short) rowId++);
							 ...
							 if(indexes[4]==0) {
								 cell = row.createCell( columnId++);
								 cell.setCellValue(resultSet.getString("store_number") == null ?"--" : fetchStoreName(resultSet.getString("store_number"),resultSet.getString("subsidiary_number"))) ;
								 logger.info(""+cell.getColumnIndex());
								 }
							 if(indexes[5]==0) {
								 cell = row.createCell( columnId++);
							 ...
							 if(indexes[7]==0) {
									...
									if(resultSet.getString("amount_difference")!=null && !resultSet.getString("amount_difference").isEmpty() && 
											Double.parseDouble(resultSet.getString("amount_difference"))!=0) {
										valueDiff = resultSet.getString("amount_difference");
											...
											&& Double.parseDouble(resultSet.getString("points_difference")) != 0) {
										valueDiff = Utility.truncateToInteger(resultSet.getString("points_difference"));
									}else if(resultSet.getString("entered_amount_type") != null &&
											((String) resultSet.getString("entered_amount_type")).equalsIgnoreCase(OCConstants.LOYALTY_TRANS_ENTEREDAMOUNT_TYPE_REWARD)) {
										
										if(resultSet.getString("reward_difference")!=null && !resultSet.getString("reward_difference").isEmpty() 
												&& Double.parseDouble(resultSet.getString("reward_difference")) != 0){
													valueDiff = resultSet.getString("reward_difference");
										}else {
											
										
											valueDiff = resultSet.getString("points_difference")!=null && !resultSet.getString("points_difference").isEmpty() 
													&& Double.parseDouble(resultSet.getString("points_difference"))!=0 ? resultSet.getString("points_difference") : (resultSet.getString("amount_difference")!=null && !resultSet.getString("amount_difference").isEmpty() && 
													Double.parseDouble(resultSet.getString("amount_difference"))!=0 ? resultSet.getString("amount_difference") : 
														resultSet.getString("entered_amount")!=null && Double.parseDouble(resultSet.getString("entered_amount"))!=0 ? resultSet.getString("entered_amount") :"0");							
										}
									}
							 ...
							 if(indexes[8]==0) {
								 String valueCode=Constants.STRING_NILL;
									...
									if(resultSet.getString("amount_difference")!=null && !resultSet.getString("amount_difference").isEmpty() && 
										...
										valueCode = resultSet.getString("earn_type")!=null ? resultSet.getString("earn_type") : resultSet.getString("value_code");
									}
	...
	private void exportCSV(String value, int[] indexes) {
		...
		if(type.contains("csv")){
			...
			try {
				...
				/*int count = ltyPrgmService.getAllTransactionsCount(userId, prgmId, MyCalendar.calendarToString(startDate, null),
									MyCalendar.calendarToString(endDate, null),key,transType,storeNo,cardsetId);*/
				//int count = ltyPrgmService.getAllTransactionsByMembershipnumberCount(currUser.getUserId(), contactsLoyaltyObj.getLoyaltyId(), MyCalendar.calendarToString(startDate, null), MyCalendar.calendarToString(endDate, null));
				...
				if(indexes[7]==0) {
					...
					udfFldsLabel += "\""+"Difference in Value."+"\""+",";
				}
				if(indexes[8]==0) {

					udfFldsLabel += "\""+"Value Code."+"\""+",";
				}
				if(indexes[9]==0) {
				...
				String query = "SELECT membership_number, created_date, docsid, subsidiary_number, store_number, transaction_type, entered_amount_type, entered_amount, excluded_amount, amount_balance, " +
						" gift_balance, points_balance, description2, hold_points, hold_amount,amount_difference,earn_type ,points_difference,reward_difference,receipt_number,trans_child_id,value_code  FROM loyalty_transaction_child " +
				" WHERE user_id ="+ currUser.getUserId() + " AND loyalty_id in ("+transferLoyaltyId+")"+
				" AND transaction_type IN('"+OCConstants.LOYALTY_TRANS_TYPE_ISSUANCE+"','"+OCConstants.LOYALTY_TRANS_TYPE_REDEMPTION+"','"+
				OCConstants.LOYALTY_TRANS_TYPE_ENROLLMENT+"','"+OCConstants.LOYALTY_TRANS_TYPE_EXPIRY+"','"+OCConstants.LOYALTY_TRANS_TYPE_BONUS+"','"+OCConstants.LOYALTY_TRANS_TYPE_ADJUSTMENT+"','"+OCConstants.LOYALTY_TRANS_TYPE_RETURN+"')"+
				...
				while(resultSet.next()){
					...
					if(indexes[4]==0) {
						sb.append("\"");sb.append(resultSet.getString("store_number") == null ? "--" : fetchStoreName(resultSet.getString("store_number"), resultSet.getString("subsidiary_number"))); sb.append("\",");
					}
					if(indexes[5]==0) {
						sb.append("\"");sb.append(resultSet.getString("transaction_type").equalsIgnoreCase(OCConstants.LOYALTY_TRANSACTION_ISSUANCE)? resultSet.getString("entered_amount_type").equalsIgnoreCase(OCConstants.LOYALTY_TYPE_PURCHASE)
							  ?"Loyalty Issuance":resultSet.getString("entered_amount_type").equals(OCConstants.LOYALTY_TYPE_REWARD)? OCConstants.LOYALTY_TYPE_REWARD:"Gift Issuance":resultSet.getString("transaction_type")); sb.append("\",");
					}
					...
					if(indexes[7]==0) {
						...
						sb.append("\"");sb.append(valueDiff+" "); sb.append("\",");
					} 
					...
					if(indexes[8]==0) {
						...
						sb.append("\"");sb.append(valueCode+" "); sb.append("\",");
					} 
					String balanceStr = "";
					/*amount_balance" +
					" gift_balance, points_balance 
*/					if(resultSet.getDouble("amount_balance") != 0 &&  
					...
					if(indexes[11]==0) {
						String holdBalanceStr = "";
						if((resultSet.getString("transaction_type").equalsIgnoreCase(OCConstants.LOYALTY_TRANSACTION_ISSUANCE) && resultSet.getString("entered_amount_type").equalsIgnoreCase(OCConstants.LOYALTY_TYPE_PURCHASE))
								||(resultSet.getString("transaction_type").equalsIgnoreCase(OCConstants.LOYALTY_TRANSACTION_ADJUSTMENT))){
							if(resultSet.getString("hold_points") != null){
								Double holdBal = 0.0;
							holdBal = Double.valueOf(resultSet.getString("hold_points"));
							if(resultSet.getString("description2") != null && !resultSet.getString("description2").isEmpty()){
								String transactions[] = resultSet.getString("description2").split(Constants.ADDR_COL_DELIMETER);
								for(String eachTrx:transactions){
									if(eachTrx !=null && eachTrx.contains(":"))
									holdBal +=Double.valueOf(eachTrx.split(":")[1].trim());
								}
							}
							holdBalanceStr = 	holdBal !=0 ? holdBal.intValue()+" Points":"";
	...
	private void clearBalanceSummary() {
		...
		testId.setValue(null);
		Clients.evalJavaScript("test();");
		testExpiredId.setValue(null);
		Clients.evalJavaScript("testExpired();");
		...
		if(searchVal.isEmpty() || (Constants.PHONE_NUMBER.equalsIgnoreCase(searchVal))) {
			...
			if(checkIfLong(searchByTbId.getValue().trim())){
				...
				//	searchByTbId.setValue(Constants.STRING_NILL);
					...
					flag = true;
					String searchKey =	searchbyLbId.getSelectedItem().getValue();
					logger.info("user id is"+currUser.getUserId());
					findLoyaltyDetails(currUser, searchKey, searchVal);
				}
			}
			else {
...
//						prevKeyTier = nextKeyTier;
	...
	private LoyaltyTransactionChild createPurchaseTransaction(ContactsLoyalty loyalty, Double adjustValue, String earnType,
															String entAmountType, String activationDate,String earnStatus, String adjType,String description2,LoyaltyBalance ltyBalance){
...
//			transaction.setEventTriggStatus(OCConstants.LOYALTY_TRANSACTION_STATUS_NEW);
	...
	private LoyaltyTransactionExpiry createRewardExpiryTransaction(ContactsLoyalty loyalty, 
			 ...
			 String valueCode,Long specialRewardId) {
...
public CouponCodeEnqReq prepareCoupEnqReq(Users user, ContactsLoyalty loyalty){
...
*/	
//	Receipt receipt = digitalReceipt.getBody().getReceipt();
	CouponCodeInfo couponCodeInfo = new CouponCodeInfo("", "ALL", loyalty.getPosStoreLocationId(), loyalty.getCustomerId(), 
			loyalty.getMobilePhone(), loyalty.getEmailId(), "", loyalty.getSubsidiaryNumber(), "", "");

	couponCodeInfo.setRECEIPTAMOUNT("");
	couponCodeInfo.setDISCOUNTAMOUNT("");
	couponCodeInfo.setCARDNUMBER(loyalty.getCardNumber());
	
	List<PurchasedItems> purchasedItems = new ArrayList<PurchasedItems>();
	
	UserDetails userDetails = new UserDetails(Utility.getOnlyUserName(user.getUserName()), Utility.getOnlyOrgId(user.getUserName()), user.getUserOrganization().getOptSyncKey());
...
column1.appendChild(new Label(couponDiscountInfo.getCOUPONCODE()));
column1.setParent(row);
Column column2=new Column();
column2.appendChild(new Label(discountDescription));
column2.setParent(row);*/

public void constructDisplayTemplate(Users user,ContactsLoyalty loyalty){
	
	try {
		boolean showStatusOnDiscount = false;
		Components.removeAllChildren(discountsRowsId);
		CouponCodeEnqReq couponCodeEnqReq = prepareCoupEnqReq(user, loyalty);
		...
		CouponCodeEnquiryServiceImpl couponEnqService = (CouponCodeEnquiryServiceImpl)ServiceLocator.getInstance().getServiceByName(OCConstants.COUPON_CODE_ENQUIRY_BUSINESS_SERVICE);
		...
		if(coupoRespon != null && coupoRespon.getCOUPONCODERESPONSE() != null && 
			...
			code.setStyle("display:inline-block;font-weight:bold;");
			heading.appendChild(code);
			Label description = new Label("Description");
			description.setStyle("display:inline-block;font-weight:bold;");
			heading.appendChild(description);
			Label action = new Label("Suggestions");
			action.setStyle("display:inline-block;font-weight:bold;");
			heading.appendChild(action);
			/*heading.appendChild(new Label("Description"));
			heading.appendChild(new Label("Action"));*/
			heading.setParent(discountsRowsId);
			for (CouponDiscountInfo couponDiscountInfo : coupDisc) {
				Row row = new Row();
				row.setParent(discountsRowsId);
				if(couponDiscountInfo.getNUDGEPROMOCODE().equals("NO") || couponDiscountInfo.getNUDGEPROMOCODE().isEmpty()){
					logger.debug("dicount=="+couponDiscountInfo.getCOUPONCODE());
					String discountDescription=couponDiscountInfo.getCOUPONCODE()+" expiry: "+couponDiscountInfo.getVALIDTO()+System.lineSeparator()+couponDiscountInfo.getDESCRIPTION();
					row.appendChild(new Label(couponDiscountInfo.getCOUPONCODE()));
					row.appendChild(new Label(discountDescription));
					row.appendChild(new Label("--"));
				}else{
					logger.debug("nudge=="+couponDiscountInfo.getCOUPONCODE());
					//if(nudgeLine.length() > 0)nudgeLine.append("<br></br><br></br>");
					//String nudgeDescription=couponDiscountInfo.getDESCRIPTION()+ System.lineSeparator() +couponDiscountInfo.getNUDGEDESCRIPTION();
					
					row.appendChild(new Label(couponDiscountInfo.getCOUPONCODE()));
					row.appendChild(new Label(couponDiscountInfo.getDESCRIPTION()));
					row.appendChild(new Label(couponDiscountInfo.getNUDGEDESCRIPTION()));
					showStatusOnDiscount=true;
				}
			}
		}
		discountGbId.setVisible(showStatusOnDiscount);
