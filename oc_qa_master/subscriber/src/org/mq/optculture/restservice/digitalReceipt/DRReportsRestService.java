...
public class DRReportsRestService extends AbstractController{
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		//		htmlContent = htmlContent.replaceAll(patternToReplace,"");
				// (<a[^><]*?href\\s*?=\\s*?[\"'])(.*?)([\"'])
			//	patternToReplace = "<a\\s+[^>]*href=['\"][^'\"]*styleunion\\.in[^'\"]*['\"][^>]*>.*<\\/a> "; // for pattern.
			//	patternToReplace = "<a\\s+[^>]*href=['\"][^'\"]*https:\\/\\/styleunion\\.in\\/search\\?q=[^'\"]*['\"][^>]*>.*<\\/a>";
            ...
            try {
            	logger.info("Entered try block for phanthom conversion.");
