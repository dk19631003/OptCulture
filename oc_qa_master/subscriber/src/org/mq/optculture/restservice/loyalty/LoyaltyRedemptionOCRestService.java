...
public class LoyaltyRedemptionOCRestService extends AbstractController{
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try{
			...
			trans = findRequestByReqIdAndDocSid(redemptionRequest.getUser().getUserName() + "__" +
	...
	private LoyaltyTransaction logTransactionRequest(LoyaltyRedemptionRequest requestObject, String jsonRequest){
		...
		try {
			...
			//APP-4766
			if(requestObject.getAmount().getType()!=null && !requestObject.getAmount().getType().isEmpty() && 
					requestObject.getAmount().getType().equals("Void"))
				transaction.setType("VoidRedemption");
			else
