...
public synchronized int updateSmsPreferenceTempContacts( Long categoryId) {
    ...
    public void updateEmailStatusByUserId(String emailIdStr,Long userId,String targetStatus,String currectStatus) {
 	   try {
		   ...
		   if(currectStatus != null && currectStatus.equalsIgnoreCase(Constants.CONT_STATUS_PURGE_PENDING)) {
			   
				  appendQry = " and email_status='"+Constants.CONT_STATUS_PURGE_PENDING+"'";
			}
 		  ...
 		  String query = "UPDATE contacts SET email_status='"+ targetStatus +"',last_status_change='"+formattedDate+"'" +
 		  		" where  user_id = " + userId + " AND email_id='"+ emailIdStr +"' "+appendQry;
  		...
  		try {
 			int count =0;
 				if(category != null && !category.equals(Constants.CAMP_CATEGORY_TRANSACTIONAL_ID)){
 					...
 					String subPreQry = " DELETE FROM tempcontacts WHERE categories IS NOT NULL OR "
 							+ "(last_mail_date IS NOT NULL AND  DATEDIFF(DATE(NOW()),DATE(last_mail_date) ) < last_mail_span )";
 					
 					logger.debug("category is null qry for delete :: "+subPreQry);
...
/**
 	...
 	public void deleteTempContacts() {
 	  ...
 	  public synchronized int createWATempContacts(String qryStr) {

 		  try {
 			  //if(deleteFlag) deleteWATempContacts();
 			  if(logger.isDebugEnabled()) logger.debug("the query to be executed is====>"+qryStr);
 			  int count = jdbcTemplate.update(qryStr);
 			  return count;

 		  } catch (DataAccessException e) {
 			  logger.error(" ** Exception : while inserting the contacts into wa_tempcontacts table", e);
 			  return 0;
 		  }
 	  } 
	...
	public void deleteWATempContacts() {
		   
		   try{
		    jdbcTemplate.update("DELETE FROM wa_tempcontacts");
		    
		   }catch (Exception e) {
		    logger.error(" ** Exception : while deleting the contacts from watempcontacts table", e);
		  }
...
public synchronized int createNotificationTempContacts(String qryStr, Short categoryWeight, Users user, boolean deleteFlag, short mobileCarrier) {
	   
	   try {
	    
	    if(deleteFlag) deleteNotificationTempContacts();
	  ...
	  /*if(userOrganization.isRequireMobileValidation()){
	   updatePhoneByConcatCarrier(mobileCarrier,user);
	   //delete from the bounce-category-table also when reinstalled delete from the same table
	    int phoneUpdatecount = jdbcTemplate.update(" DELETE FROM notification_tempcontacts WHERE  instance_id is null and push_notification is null");
	   if(phoneUpdatecount > 0){
	    count -= phoneUpdatecount;
	   }
	  }*/
	   if(user!=null && user.getUserId() != null) {
	    ...
	    // remove suppressed contacts
	    count -= jdbcTemplate.update(
	      " DELETE FROM notification_tempcontacts WHERE instance_id IN(SELECT instance_id FROM " +
	      " notification_suppressed_contacts WHERE user_id="+user.getUserId()+" and org_id="+user.getUserOrganization().getUserOrgId()+")");
	   }
	   ...
	   return 0;
		...
		public void deleteNotificationTempContacts() {
			 try{
		 		   jdbcTemplate.update("DELETE FROM notification_tempcontacts");
		 		   }catch (Exception e) {
		 		    logger.error(" ** Exception : while deleting the contacts from smstempcontacts table", e);
		 		  }		
		}
		
		public int updatelastNotificationDate(Long notificationCrId, String notificationDate) {
	 		try {
				String qry = "UPDATE contacts c, Notification_campaign_sent cs SET c.last_notification_date='"+notificationDate+"'  WHERE cs.notification_cr_id="+notificationCrId.longValue()+" AND cs.contact_id=c.cid ";
				return executeJdbcUpdateQuery(qry);
			} catch (Exception e) {
				logger.error("Exception ::::" , e);
				return 0;
