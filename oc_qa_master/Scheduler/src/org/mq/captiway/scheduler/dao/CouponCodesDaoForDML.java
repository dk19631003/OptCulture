...
/*	public long findIssuedCoupCodeByCoup(Long CouponId) {
    ...
    public int updateReceiptNumberFromSalesFile(Long userId,Long orgId,Long startSalesID, Long endSalesID){
		
    	int updateCount =0;
		try {
			String query= " update coupon_codes cc JOIN "
					+ "(SELECT distinct doc_sid, reciept_number, user_id from retail_pro_sales where user_id="+userId+" and sales_id between "+startSalesID+" AND "+endSalesID+" AND  doc_sid is not null) "
							+ "rs ON cc.doc_sid = rs.doc_sid set cc.receipt_number=rs.reciept_number where cc.orgId="+orgId+" "
									+ "and  cc.status='Redeemed' and cc.receipt_number is null";
			logger.info("qry "+query);
			updateCount = executeJdbcUpdateQuery(query);
 ...
 public int updateReceiptNumberFromDRExtraction(Long userId,Long orgId,String docsid,Long startSalesID, Long endSalesID){
	 int updateCount=0;
		
		try {
			String query= " update coupon_codes cc JOIN " 
					+ "(SELECT distinct doc_sid, reciept_number, user_id from retail_pro_sales where user_id="+userId+" and sales_id between "+startSalesID+" AND "+endSalesID+" and doc_sid ='"+docsid+"') "
							+ "  rs ON cc.doc_sid = rs.doc_sid set cc.receipt_number=rs.reciept_number "
							+ "where cc.orgId="+orgId+" and  cc.status='Redeemed' AND cc.doc_sid IS NOT NULL and cc.receipt_number is null";
