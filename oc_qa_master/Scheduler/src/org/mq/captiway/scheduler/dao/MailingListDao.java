...
public class MailingListDao extends AbstractSpringDao {
	...
	public List<MailingList> getLastModifiedList(String currentDateStr) {
		
		List<MailingList>  list = null;
		try {
		String queryStr = "SELECT list_id FROM mailing_lists ml " +
				" WHERE ml.last_modifiedDate >= DATE_SUB('"+currentDateStr+"',INTERVAL 24 HOUR)";
		
		logger.info("queryStr :"+queryStr);
		list= jdbcTemplate.query(queryStr, new RowMapper() {
	        public MailingList mapRow(ResultSet rs, int rowNum) throws SQLException {
	        	MailingList object = new MailingList();
	        	object.setListId((rs.getLong(1)));
