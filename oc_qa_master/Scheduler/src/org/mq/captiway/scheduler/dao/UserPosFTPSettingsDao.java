...
public class UserPosFTPSettingsDao extends AbstractSpringDao{
	...
	private static final Logger logger = LogManager.getLogger(Constants.SCHEDULER_LOGGER);
	 ...
	 public UserPosFTPSettingsDao(){}
	    ...
	    public List<UserPosFTPSettings> findListByTimeDiff(String filename) {
	    	
	    	List<UserPosFTPSettings> posSettingList = null;
	    	logger.info("entering findListByTimeDiff method ");

	    	String qry = "FROM UserPosFTPSettings A WHERE LOCATE(A.fileFormat,'"+filename+"')>0  AND  A.enabled = true AND  NOW() > TIMESTAMPADD (MINUTE, A.scheduledFreqInMintues, "
	    			+ " A.lastFetchedTime) ";
	    	
	    //	String qry = "FROM UserPosFTPSettings A WHERE   A.enabled = true AND  NOW() > TIMESTAMPADD (MINUTE, A.scheduledFreqInMintues, "
	    	//		+ " A.lastFetchedTime) ";
