...
//    	contactsDao.createTempContacts(qryStr, mailingList.getUsers().getUserId());
    ...
    // only loyalty tags are being replaced.
    public synchronized Contacts getNext() {
        ...
        if (this.iter != null && this.iter.hasNext()) {
        	...
        	logger.info("iter is not empty sending the contact from the getnext() :"+tempContact);
            return tempContact;
        }
        ...
        if(totalSizeInt ==0 ) {
    		...
    		logger.info(" Getting the 5000 contact objects from Contacts Table from :"+
    				currentSizeInt+"-"+(currentSizeInt+5000) + " thread : "+Thread.currentThread().getName());
    		
    	List<Contacts> contacts = null;
    	contacts = contactsDao.getSegmentedContacts("SELECT * FROM tempcontacts ",
    												currentSizeInt, 5000);
    	logger.info("the sending campaigns : currentSizeInt :"+currentSizeInt+" Thread : "+Thread.currentThread().getName());
		...
		currentSizeInt += 5000;
		logger.info("current size : "+currentSizeInt+" Thread name :"+Thread.currentThread().getName());
		...
		if(campaignReport != null) {
			...
			while(this.iter != null && this.iter.hasNext()) {
				...
				CampaignSent campaignSent = new CampaignSent(currentSentId,tempContact.getEmailId(),
						...
						campaignReport, Constants.CS_STATUS_SUBMITTED, tempContact.getContactId(),campaignId);
		...
		this.iter = contacts.listIterator();
		logger.info("size of the iterator : "+contacts.size()+" same assigned to iter");
		tempContact = iter.next();
logger.info("returning tempContact checking for ses : "+tempContact.getEmailId());
        return tempContact;
	...
	private String getMembershipPassword(ContactsLoyalty contactsLoyalty,String defVal) {
		...
		try {
			password =getresetPasswordURL(contactsLoyalty, defVal);
