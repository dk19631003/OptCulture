		...
		try {
			...
			if(specificDirExists != null && specificDirExists.size()>0) {
				for (Users users : specificDirExists) {
					...
					String sourceFolders = SftpUserPath+"/"+username;
					String unProcessedFolders = SftpUserPath+"/"+username+"/timestamped_csv_files/";
					String DonePath = SftpUserPath+"/"+username+"/opt_sync/done/optintel";
			
					moveFilesFromOcftToUnProces(sourceFolders,unProcessedFolders,DonePath);
					getPOSFilesFromFTP(unProcessedFolders);
				}
			}
		} catch (Exception e) {
...
//			if(!file.canWrite()) continue;
				...
				if(tempFile.getName().contains(" ")) {
					logger.info("filename contains space "+tempFile.getName());
					logger.info("file path "+tempFile.getPath());
					String newfileName = tempFile.getPath().replace(" ", "_");
                    tempFile.renameTo(new File(newfileName));
					logger.info("renamed to "+tempFile.getName());
				}
...
//				String fileNamewithOutExt = tempFile.getName().substring(0, tempFile.getName().length()-4);
				...
				boolean flag = false;
				try {
					String moveCmd = "mv "+tempFile.getPath()+" "+(unProcessedFolder+tempFile.getName()+"_OC_"+currTimeStr + cCal.get(Calendar.MILLISECOND)+ "."+extensionStr);
	...
	private void moveFilesFromOcftToUnProces(String sourceFolders, String unProcessedFolders, String donepaths) {
//		String outboxoptIntelPath = PropertyUtil.getPropertyValue("outboxoptintel");
		String donePath = donepaths;
		try {
			//if(logger.isInfoEnabled()) 
			logger.info(" sourceFolder :: "+sourceFolders);
			File sourceFold = new File(sourceFolders);
...
//				String fileNamewithOutExt = tempFile.getName().substring(0, tempFile.getName().length()-4);
				...
				boolean flag = false;
				try {
					String moveCmd = "mv "+tempFile.getPath()+" "+(unProcessedFolders+tempFile.getName()+"_OC_"+currTimeStr + cCal.get(Calendar.MILLISECOND)+ "."+extensionStr);
					logger.info("moveCmd ::"+moveCmd);
					Runtime.getRuntime().exec(moveCmd);
					flag = true;
				} catch (Exception e) {
	...
	} // moveFilesFromOcftToUnProces for Specific user
	...
	private void getPOSFilesFromFTP() {
		...
		for (File eachTimeStampedFile : sourceFileLst) {
			 try {
				...
				//APP-2077
				 /*boolean moveToFailFolder = FTPPOSFileAccess.fileCopyMoveToUserAndProcessFolder(eachTimeStampedFile);
				if(!moveToFailFolder) {
					
					try {
						//String destLocation_FailFolder = ""; // home/ocftpuser/failedfiles 
						
						
						String moveCmd = "mv "+eachTimeStampedFile.getPath()+" "+failFolder;
						logger.info("moveCmd ::"+moveCmd);
						Runtime.getRuntime().exec(moveCmd);
						
					} catch (Exception e) {
						logger.error("Exception While moving to FailFolder :"+ e.getMessage());
					}
				}*/
...
private void getPOSFilesFromFTP(String unProcessedFolders) {
		
		
		//logger.info("userPosSettingList.size() is :.."+userPosSettingList.size());
		
		
		File sourceFold = new File(unProcessedFolders);
		File[] sourceFileLst =  sourceFold.listFiles();
