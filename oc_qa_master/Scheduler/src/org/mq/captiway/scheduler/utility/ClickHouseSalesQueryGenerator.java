...
public class ClickHouseSalesQueryGenerator {
	...
	private static String generalSql = 
		...
		"SELECT DISTINCT any(c.cid) FROM contacts c " +
		"<FROM_TABLES>" +
		"" +
		"" +
	...
	private static String qry_HomePassed = " SELECT DISTINCT any(h.hp_id) FROM homespassed h WHERE h.user_id IN(<USERIDS>)  AND (<COND>)  ";
	
	
	
	private static String tot_subQuery = " SELECT DISTINCT c.cid FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS>  AND bitAnd(c.mlbits,<MLBITS> )>0 " +
	...
	private static String tot_subQuery_mobile = " SELECT DISTINCT c.cid FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND bitAnd(c.mlbits,<MLBITS> )>0" +
											" <FROM_TABLE_FILTERS> AND (c.mobile_phone is not null AND c.mobile_phone !='' AND c.mobile_status='Active'  <MOBILEOPTIN> ) AND (<COND>) ";
	
	private static String general_mobile_subQuery = " SELECT DISTINCT c.cid FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
	" <FROM_TABLE_FILTERS> AND (c.mobile_phone is not null AND c.mobile_phone !='' AND c.mobile_status='Active'  <MOBILEOPTIN> ) AND (<COND>)   ";

	
	private static String not_received_email_status =  " SELECT c.cid,  c.email_id, c.first_name, " +//group by email_id, how can we aggrigate multiple columns ?
		...
		" null, splitByChar('@', c.email_id)[-1], null FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT any(contact_id)  " +
			...
			"  AND c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
...
private static String department_and_item_category_sub_query = " sal.cid NOT IN (SELECT DISTINCT sal.cid FROM retail_pro_sales sal, retail_pro_sku sku WHERE sal.user_id IN(<SALESUSERID>) AND sal.cid IS NOT NULL AND sku.user_id in(<SALESUSERID>)"
		+ " AND sku.sku_id = sal.inventory_id AND (DATEDIFF('dd',Date(sal.sales_date),now() ) >= 0 AND DATEDIFF('dd',Date(sal.sales_date),now() ) <= 365) AND <DEPT_ITEM_CAT_ONLYLIKE>)";
...
private static String last_purchase_qry_template = "( SELECT sal.* FROM retail_pro_sales sal, ( SELECT sal.cid, MAX(date(sales_date)) last_purchase_date  FROM retail_pro_sales sal WHERE sal.user_id IN(<SALESUSERID>)"
		+ " AND  sal.cid IS NOT NULL GROUP BY sal.cid HAVING <RPS_LAST_PURCHASE_DATE_COND>  ) maxsal WHERE sal.user_id IN(<SALESUSERID>) AND  sal.cid IS NOT NULL AND sal.cid = maxsal.cid AND toDate32(sal.sales_date) =  maxsal.last_purchase_date ) sal ";
//SQL-TO-CH
private static String aggr_query_template = " ( SELECT any(sal.cid) cid <SALESSELECTFIELDS> FROM retail_pro_sales sal<RPS_LAST_PURCHASE_DATE_TEMP> <SALESFROMTABLES> "
...
private static String product_purchase_qry_template = "  sal.cid <NOTCOND> IN(SELECT DISTINCT sal.cid FROM retail_pro_sales sal, retail_pro_sku sku "
		+ " WHERE sal.user_id IN(<SALESUSERID>) AND sku.user_id IN(<SALESUSERID>) AND sal.inventory_id=sku.sku_id AND  sal.cid IS NOT NULL AND <PRODUCTCOND>)";
...
//SQL-TO-CH
private static String loyaltyaggr_query_template = " ( SELECT DISTINCT any(ltytrxch.loyalty_id) loyalty_id <LTYTRXCHSELECTFIELDS> FROM loyalty_transaction_child ltytrxch  "
...
private static String generalSqlWithEmailStatusCountForHavenotIsuued ="SELECT DISTINCT c.cid  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in "+promotion_have_not_issued_qury_template+"  <GROUPBYSTR> <HAVINGSTR>";

private static String email_generalSqlWithEmailStatusCountForHavenotIsuued ="SELECT DISTINCT c.cid  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0  <FROM_TABLE_FILTERS> <COND>  AND c.cid not in  "+promotion_have_not_issued_qury_template
		    + "  AND (c.email_id is not null AND c.email_id!='') AND c.email_status='Active'  GROUP BY c.email_id <GROUPBYSTR> <HAVINGSTR>";

private static String mobile_generalSqlWithEmailStatusCountForHavenotIsuued ="SELECT DISTINCT c.cid  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in "+promotion_have_not_issued_qury_template
		    + " AND (c.mobile_phone is not null AND c.mobile_phone !='') GROUP BY c.mobile_phone <GROUPBYSTR> <HAVINGSTR>";

private static String generalSqlWithEmailStatusForHavenotIsuued = " SELECT c.cid,  c.email_id, c.first_name, " +
		" c.last_name, c.created_date, c.purged, c.email_status," +
		" c.last_status_change, c.last_mail_date, c.address_one, " +
		" c.address_two, c.city, c.state, c.country, c.pin, c.phone, " +
		" c.optin, c.subscription_type, c.optin_status, c.external_id, c.optin_medium, c.gender, c.birth_day," +
		...
		" c.udf10 ,c.udf11 ,c.udf12 ,c.udf13 ,c.udf14, c.udf15, c.opted_into, c.optin_per_type, c.home_store,c.loyalty_customer, c.hp_id, " +
		" c.mobile_status,c.user_id,c.zip,c.mobile_phone,c.home_phone,c.last_sms_date,c.mobile_opt_in, c.mlbits, c.categories, c.last_mail_span, c.last_sms_span,  " +
		" null, splitByChar('@', c.email_id)[-1], null  FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in "+promotion_have_not_issued_qury_template+"  <GROUPBYSTR> <HAVINGSTR>";

private static String email_generalSqlWithEmailStatusForHavenotIsuued = " SELECT c.cid,  c.email_id, c.first_name, " +
		" c.last_name, c.created_date, c.purged, c.email_status," +
		" c.last_status_change, c.last_mail_date, c.address_one, " +
		" c.address_two, c.city, c.state, c.country, c.pin, c.phone, " +
		" c.optin, c.subscription_type, c.optin_status, c.external_id, c.optin_medium, c.gender, c.birth_day," +
		...
		" c.udf10 ,c.udf11 ,c.udf12 ,c.udf13 ,c.udf14, c.udf15, c.opted_into, c.optin_per_type, c.home_store,c.loyalty_customer, c.hp_id, " +
		" c.mobile_status,c.user_id,c.zip,c.mobile_phone,c.home_phone,c.last_sms_date,c.mobile_opt_in, c.mlbits, c.categories, c.last_mail_span, c.last_sms_span,  " +
		" null, splitByChar('@', c.email_id)[-1], null FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0 <FROM_TABLE_FILTERS> <COND>  AND c.cid not in  "+promotion_have_not_issued_qury_template
		    + "  AND (c.email_id is not null AND c.email_id!='') AND c.email_status='Active'  GROUP BY c.email_id <GROUPBYSTR> <HAVINGSTR>";

private static String mobile_generalSqlWithEmailStatusForHavenotIsuued = " SELECT c.cid,  c.email_id, c.first_name, " +
		" c.last_name, c.created_date, c.purged, c.email_status," +
		" c.last_status_change, c.last_mail_date, c.address_one, " +
		" c.address_two, c.city, c.state, c.country, c.pin, c.phone, " +
		" c.optin, c.subscription_type, c.optin_status, c.external_id, c.optin_medium, c.gender, c.birth_day," +
...
*/	
	...
	private static String not_received_mobile_status =  " SELECT c.cid,  c.email_id, c.first_name, " +
		...
		" c.udf10 ,c.udf11 ,c.udf12 ,c.udf13 ,c.udf14, c.udf15, c.opted_into, c.optin_per_type, c.home_store,c.loyalty_customer, c.hp_id, " +
		" c.mobile_status,c.user_id,c.zip,c.mobile_phone,c.home_phone,c.last_sms_date,c.mobile_opt_in, c.mlbits, c.categories, c.last_mail_span, c.last_sms_span, " +
		" null, splitByChar('@', c.email_id)[-1], null FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
			"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
			+Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
			"  WHERE o.contact_id is null  "+
			"  AND c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
	...
	/*private static String generalSql_mobile=
		...
		" null, splitByChar('@', c.email_id)[-1], null  " +
		" FROM contacts c  " +
	...
	*/
	...
	private static String generalSql_mobile=
		...
		" c.udf10 ,c.udf11 ,c.udf12 ,c.udf13 ,c.udf14, c.udf15, c.opted_into, c.optin_per_type, c.home_store,c.loyalty_customer, c.hp_id, " +
		" c.mobile_status,c.user_id,c.zip,c.mobile_phone,c.home_phone,c.last_sms_date,c.mobile_opt_in, c.mlbits, c.categories, c.last_mail_span, c.last_sms_span,  " +
		" null, splitByChar('@', c.email_id)[-1], null FROM contacts c "+
		" <FROM_TABLES> WHERE c.user_id=<USERIDS> AND bitAnd(c.mlbits,<MLBITS> )>0 " +
		" <FROM_TABLE_FILTERS>  AND (c.mobile_phone IS NOT NULL AND c.mobile_phone !='' AND c.mobile_status='Active' <MOBILEOPTIN> )  <COND>   GROUP BY c.mobile_phone <SALESFIELDS> <HAVINGSTR> ";
	*/
	...
	private static String mobile_generalSqlWithMobileStatusCount =	
		...
		" c.udf10 ,c.udf11 ,c.udf12 ,c.udf13 ,c.udf14, c.udf15, c.opted_into, c.optin_per_type, c.home_store,c.loyalty_customer, c.hp_id, " +
		" c.mobile_status,c.user_id,c.zip,c.mobile_phone,c.home_phone,c.last_sms_date,c.mobile_opt_in, c.mlbits,  " +
		" null, splitByChar('@', c.email_id)[-1], null FROM contacts c WHERE c.cid IN( SELECT c.cid" +
			" FROM contacts c  " +		
		" <FROM_TABLES> WHERE c.user_id=<USERIDS> AND (c.mlbits&<MLBITS> )>0 " +
		" <FROM_TABLE_FILTERS>  AND (c.mobile_phone IS NOT NULL AND c.mobile_phone !='' AND c.mobile_status='Active' <MOBILEOPTIN> )  <COND>   GROUP BY c.mobile_phone <SALESFIELDS> <HAVINGSTR>)";
	*/
...
" <FROM_TABLE_FILTERS>   AND c.email_status in ('" + Constants.CONT_STATUS_ACTIVE+"','"+Constants.CONT_STATUS_PURGE_PENDING +"') AND c.email_id!='' <COND> GROUP BY c.email_id  <SALESFIELDS> <HAVINGSTR>  ";
	...
	*/
	...
	private static String tot_subQuery_notification = " SELECT DISTINCT c.cid FROM contacts c <FROM_TABLES> WHERE c.user_id=<USERIDS> AND bitAnd(c.mlbits,<MLBITS> )>0" +
			" <FROM_TABLE_FILTERS> AND (c.pushNotification =1 ) AND (<COND>) ";
			
			
	private static String not_received_notification_status =  " SELECT c.cid,  c.email_id, c.first_name, " +
			" c.last_name, c.created_date, c.purged, c.email_status," +
			" c.last_status_change, c.last_mail_date, c.address_one, " +
			" c.address_two, c.city, c.state, c.country, c.pin, c.phone, " +
			" c.optin, c.subscription_type, c.optin_status, c.external_id, c.optin_medium, c.gender, c.birth_day," +
			" c.anniversary_day, c.udf1, c.udf2, c.udf3, c.udf4, c.udf5, c.udf6, c.udf7, c.udf8, c.udf9, " +
			" c.udf10 ,c.udf11 ,c.udf12 ,c.udf13 ,c.udf14, c.udf15, c.opted_into, c.optin_per_type, c.home_store,c.loyalty_customer, c.hp_id, " +
			" c.mobile_status,c.user_id,c.zip,c.mobile_phone,c.home_phone,c.last_sms_date,c.mobile_opt_in, c.mlbits, c.categories, c.last_mail_span, c.last_sms_span,c.push_notification, c.instance_id, c.device_Type " +
			" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
				"  FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
				+ Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
				"  WHERE o.contact_id is null  "+
				"  AND c.user_id=<USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
				" <FROM_TABLE_FILTERS> AND c.push_notification=1 AND (c.instance_id is not null AND c.instance_id !='') AND (c.device_Type IS NOT NULL AND c.device_Type !='') <COND> " +
				" GROUP BY c.mobile_phone <SALESFIELDS> <HAVINGSTR>";
	
	private static String not_received_notification_count_status =	
				" SELECT DISTINCT c.cid"+
				" FROM (contacts c <FROM_TABLES>) LEFT OUTER JOIN (SELECT contact_id  " +
				" FROM campaign_sent WHERE campaign_id IN(" +Constants.INTERACTION_CAMPAIGN_IDS_PH +")" 
				+ Constants.INTERACTION_CAMPAIGN_CRID_PH +" GROUP BY email_id , campaign_id   )o on o.contact_id=c.cid " +
	...
	private static String notification_generalSqlWithNotificationStatusCount =	
			...
			" FROM contacts c " +
			" <FROM_TABLES> WHERE c.user_id = <USERIDS> AND  bitAnd(c.mlbits,<MLBITS> )>0" +
			" <FROM_TABLE_FILTERS> AND c.push_notification = 1  AND (c.instance_id IS NOT NULL AND c.instance_id !='') AND (c.device_Type IS NOT NULL AND c.device_Type !='') " +
			" <COND>  GROUP BY c.instance_id <SALESFIELDS> <HAVINGSTR> ";

	private static String generalSql_notification=
			...
			" c.anniversary_day, c.udf1, c.udf2, c.udf3, c.udf4, c.udf5, c.udf6, c.udf7, c.udf8, c.udf9, " +
			" c.udf10 ,c.udf11 ,c.udf12 ,c.udf13 ,c.udf14, c.udf15, c.opted_into, c.optin_per_type, c.home_store,c.loyalty_customer, c.hp_id, " +
			" c.mobile_status,c.user_id,c.zip,c.mobile_phone,c.home_phone,c.last_sms_date,c.mobile_opt_in, c.mlbits, c.categories, c.last_mail_span, c.last_sms_span,c.instance_id,c.push_notification ,c.device_Type ," +
			" splitByChar('@', c.email_id)[-1]" +
			" FROM contacts c "+
			" <FROM_TABLES> WHERE c.user_id=<USERIDS> AND bitAnd(c.mlbits,<MLBITS> )>0 " +
			" <FROM_TABLE_FILTERS> AND c.push_notification = 1  AND (c.instance_id IS NOT NULL AND c.instance_id !='')  AND (c.device_Type IS NOT NULL AND c.device_Type !='') <COND>   GROUP BY c.instance_id <SALESFIELDS> <HAVINGSTR> ";
	...
	static {
		...
		tokensHashMap.put("string:startswith", " ilike '<VAL1>%'");
		tokensHashMap.put("string:endswith", " ilike '%<VAL1>'");
		tokensHashMap.put("string:contains", " ilike '%<VAL1>%'");
		tokensHashMap.put("string:starts with", " ilike '<VAL1>%'");
		tokensHashMap.put("string:ends with", " ilike '%<VAL1>'");
		tokensHashMap.put("string:does not contain", " not ilike '%<VAL1>%'");
		...
		tokensHashMap.put("number:date_diff_equal_to", " DATEDIFF('dd', MAX(<FIELD>), NOW()) = <VAL1>  "); //APP-4201

		tokensHashMap.put("date:is", " = '<VAL1>'");
		tokensHashMap.put("date:istoday", " DATEDIFF('dd', <FIELD>, now()) = 0 " );
...
//		"AND DATEDIFF('<FIRSTYEAR_DATE>', DATE_ADD(<FIELD>, INTERVAL -YEAR(<FIELD>) YEAR) ) % 365 <= <VAL2> ");
		tokensHashMap.put("date:before_between", " DATEDIFF('dd', subtractYears((<FIELD>), (YEAR(<FIELD>)-1900)) , Date('<FIRSTYEAR_DATE>')) % 365 >= <VAL1> " +
				"AND DATEDIFF('dd', subtractYears((<FIELD>), (YEAR(<FIELD>)-1900)) , Date('<FIRSTYEAR_DATE>')) % 365 <= <VAL2> ");
		
		tokensHashMap.put("date:range_between", " DATEDIFF('dd', <FIELD>, now()) >= <VAL1> AND  DATEDIFF('dd', <FIELD>, now()) <= <VAL2> ");
...
//				" AND DATEDIFF(DATE_ADD(<FIELD>, INTERVAL -YEAR(<FIELD>)+1 YEAR), '<ZEROYEAR_DATE>' ) % 365 <= <VAL2> ");
		tokensHashMap.put("date:after_between", " DATEDIFF('dd', Date('<ZEROYEAR_DATE>'), subtractYears((<FIELD>), (YEAR(<FIELD>)-1901))) % 365 >= <VAL1> " + 
				" AND DATEDIFF('dd', Date('<ZEROYEAR_DATE>'), subtractYears((<FIELD>), (YEAR(<FIELD>)-1901))) % 365 <= <VAL2> ");
		
		tokensHashMap.put("date:between", " between '<VAL1>' and '<VAL2>'");
		
		tokensHashMap.put("date:withinlast", "DATEDIFF('dd', <FIELD>, now()) >=0 AND DATEDIFF('dd', <FIELD>, now()) <= <VAL1>");
		tokensHashMap.put("date:notwithinlast", "DATEDIFF('dd', <FIELD>, now() ) <= <VAL1>");
		...
		tokensHashMap.put("date:is within last", "DATEDIFF('dd', <FIELD>, now() ) <= <VAL1>");
		tokensHashMap.put("date:not within last", "DATEDIFF('dd', <FIELD>, now() ) <= <VAL1>");
		tokensHashMap.put("date:is within next", "DATEDIFF('dd', <FIELD>, now() ) >= <VAL1>");
		//tokensHashMap.put("date:iswithinnext", "DATEDIFF(<FIELD>,now()) = <VAL1>");
		tokensHashMap.put("date:iswithinnext", "<EXP> - (DATEDIFF('dd', <FIELD>, now() )) = <VAL1>");
		tokensHashMap.put("date:iswithinnextS", "(DATEDIFF('dd', <FIELD>, now() )) = <VAL1>");
		...
		//tokensHashMap.put("date:ignoreyearDATEDIFF(now(),<FIELD> ) <= <VAL1>_is", "(toMonth(<FIELD>) = toMonth(Date('<VAL1>')) AND toDayOfMonth(<FIELD>) = toDayOfMonth(Date('<VAL1>')))");
		tokensHashMap.put("date:ignoreyear_is", "((toMonth(<FIELD>), toDayOfMonth(<FIELD>)) = (toMonth(Date('<VAL1>')), toDayOfMonth(Date('<VAL1>'))))");
		tokensHashMap.put("date:ignoreyear_between", "((toMonth(<FIELD>), toDayOfMonth(<FIELD>)) between (toMonth(Date('<VAL1>')), toDayOfMonth(Date('<VAL1>'))) AND (toMonth(Date('<VAL2>')), toDayOfMonth(Date('<VAL2>'))))");
		tokensHashMap.put("date:ignoreyear_after", "((toMonth(<FIELD>), toDayOfMonth(<FIELD>)) > (toMonth(Date('<VAL1>')), toDayOfMonth(Date('<VAL1>'))))");
		tokensHashMap.put("date:ignoreyear_before", "((toMonth(<FIELD>), toDayOfMonth(<FIELD>)) < (toMonth(Date('<VAL1>')), toDayOfMonth(Date('<VAL1>'))))");
		tokensHashMap.put("date:ignoreyear_onorafter", "((toMonth(<FIELD>), toDayOfMonth(<FIELD>)) >= (toMonth(Date('<VAL1>')), toDayOfMonth(Date('<VAL1>'))))");
		tokensHashMap.put("date:ignoreyear_onorbefore", "((toMonth(<FIELD>), toDayOfMonth(<FIELD>)) <= (toMonth(Date('<VAL1>')), toDayOfMonth(Date('<VAL1>'))))");
	...
	public static String generateNotificationListSegmentQuery(String queryString, long mlBit) {
		return generateListSegmentQuery(queryString, false, Constants.SEGMENT_ON_NOTIFICATION, mlBit);
	}
	...
	public static String generateNotificationListSegmentCountQuery(String queryString, long mlBit) {
		return generateListSegmentCountQuery(queryString, false, Constants.SEGMENT_ON_NOTIFICATION, mlBit);
	}
				...
				for (int tokenIndex = 0;tokenIndex<tempTokenArr.length ;tokenIndex++) {
					...
					if(chkAggrLtyColumnsFound == false && (fieldsArr[1].toLowerCase().trim().startsWith("aggrlty_") 
						...
						if(fieldsArr[1].toLowerCase().trim().contains("aggrlty_reward")) {
							...
							fromTables = " , loyalty_balance ltybalance , contacts_loyalty loyalty " + fromTables;
						 ...
						 //fromTableFilters  += " AND c.cid=totloyal.contact_id  ";
							 if(chkLoyaltyFlagFound == false ){
									if (ltySelectStr.isEmpty())
										ltySelectStr = Constants.DELIMETER_COMMA/* +OCConstants.replaceLtySelectStr */;
									chkLoyaltyFlagFound = true;
									if(!fromTables.contains("contacts_loyalty loyalty")) fromTables = " , contacts_loyalty loyalty "+ fromTables;
					...
					if(chkLoyaltyFlagFound == false && 
						...
						if (ltySelectStr.isEmpty())
							ltySelectStr = Constants.DELIMETER_COMMA/* +OCConstants.replaceLtySelectStr */;
						chkLoyaltyFlagFound = true;
						fromTables = " , contacts_loyalty loyalty " + fromTables;
						fromTableFilters  += " AND loyalty.user_id=<CONTLTYUSERID> AND loyalty.contact_id=c.cid "; 
			...
			if(filterMap.size()==1 && filterMap.containsKey("GENERAL")) {
				...
				/*String tempStr = (ignoreEmailStatus==true) ?
					...
					if(considerEmailStatus == true && qryType.equals(Constants.SEGMENT_ON_EMAIL)) {
						...
						tempStr = generalSql_notification.replace("<USERIDS>", userIdStr).replace("<MLBITS>", mlBit+"").replace("<AGGRFIELDS>", aggrFieldsSB.toString()).
						replace("<HAVINGSTR>", HavingSB.length()>0 ? " HAVING "+HavingSB.toString() : HavingSB.toString() ).
						replace("<SALESFIELDS>", groupBySB.toString()).replace("<GROUPBYSTR>", groupBySB.length() > 0 ? "GROUP BY "+groupBySB.toString().substring(1) : groupBySB.toString());
						...
						if(isCsNotReceivedUnionExist ) {
							...
							logger.debug("==================entered to replace the PHS =================");
							csNotReceivedUnionQryStr = not_received_notification_status.replace("<USERIDS>", userIdStr).replace("<MLBITS>", mlBit+"")
													.replace("<MLBITS>", mlBit+"").replace("<AGGRFIELDS>", aggrFieldsSB.toString()).
													replace("<HAVINGSTR>", HavingSB.length()>0 ? " HAVING "+HavingSB.toString() : HavingSB.toString() ).
													replace("<SALESFIELDS>", groupBySB.toString()).replace("<GROUPBYSTR>", groupBySB.length() > 0 ? "GROUP BY "+groupBySB.toString().substring(1) : groupBySB.toString());
				 ...
				 *  	HAVING COUNT(t.email_id) = SUM(IF(t.some >0, 1,0)) AND COUNT(t.email_id) >=2  
					...
					logger.info("ltyHavingSB : "+ltyHavingSB);
					...
					//SQL-CH
					if(!tempStr.contains("GROUP BY")) {
						tempStr = tempStr.replace("any(c.cid)", "c.cid");
					}
					...
					//SQL-CH
					if(salesSelectFields.isEmpty()) {
						tempStr = tempStr.replace("any(sal.cid) cid", "(sal.cid) cid");
					}
					if(ltySelectFields.isEmpty()) {
						tempStr = tempStr.replace("any(ltytrxch.loyalty_id) loyalty_id", "(ltytrxch.loyalty_id) loyalty_id");
					}
					if(tempStr.contains("totSal.totAmt")) {
						tempStr = tempStr.replace("any(sal.cid) cid", "any(sal.cid) cid, sal.doc_sid");
					}
				...
				String zeroYear  = "1900-"+currMonthDate;
				String firstYear = "1901-"+currMonthDate;
...
//			logger.debug(listIdStr+"=FN="+finalStr);
			...
			// SQL-TO-CH
			if (!finalStr.contains("GROUP BY")) {
				finalStr = finalStr.replace("any(c.cid)", "c.cid");
			}

			finalStr = finalStr.replace("Date(", "toDate32(");

			finalStr = finalStr.replace("SUBSTRING_INDEX(c.email_id, '@', -1)", "splitByChar('@', c.email_id)[-1]");
	...
	*/								
								...
								ltyConditionSB.append("("+replacedVal+")");
									...
									logger.info("replacedVal : "+replacedVal);
									ltyORHavingSB.append(replacedVal);
									logger.info("ltyORHavingSB : "+ltyORHavingSB);
			...
			if(filterMap.size()==1 && filterMap.containsKey("GENERAL")) {
				...
				/*String tempStr = (ignoreEmailStatus==true) ?
					...
					if(considerEmailStatus == true && qryType.equals(Constants.SEGMENT_ON_EMAIL)) {
						...
						tempStr = notification_generalSqlWithNotificationStatusCount.replace("<USERIDS>", userIdStr).replace("<MLBITS>", mlBit+"").replace("<AGGRFIELDS>", aggrFieldsSB.toString()).
						replace("<HAVINGSTR>", HavingSB.length()>0 ? " HAVING "+HavingSB.toString() : HavingSB.toString() ).
						replace("<SALESFIELDS>", groupBySB.toString()).replace("<GROUPBYSTR>", groupBySB.length() > 0 ? "GROUP BY "+groupBySB.toString().substring(1) : groupBySB.toString());
				...
				if(isHaveNotFlag  ) {
						tempStr = generalSqlWithEmailStatusCountForHavenotIsuued.replace("<USERIDS>", userIdStr).replace("<MLBITS>", mlBit+"")
								.replace("<MLBITS>", mlBit+"").replace("<AGGRFIELDS>", aggrFieldsSB.toString()).
											replace("<HAVINGSTR>", HavingSB.length()>0 ? " HAVING "+HavingSB.toString() : HavingSB.toString() ).
											replace("<SALESFIELDS>", groupBySB.toString()).replace("<GROUPBYSTR>", groupBySB.length() > 0 ? "GROUP BY "+groupBySB.toString().substring(1) : groupBySB.toString());
...
//			logger.debug(listIdStr+"=FN="+finalStr);
	...
	public static String prepareValueStr(String ValToBePrepare, String type) {
		...
		for (String valToken : strArr) {
			...
			if(type.equalsIgnoreCase("string")) {
				valStr += " '"+ valToken.trim().toLowerCase()+"'";
			}
	...
	public static String replaceToken(String fieldName, String[] tokenArr) {
		...
		try {
			//String fitreStr = "<OR>";
			boolean skunotequals = false;
			String notDateStr = "";
			String DVal = "";
			String productsubQry=null;
			
			String token = tokenArr[2].toLowerCase().trim();
			String value = tokenArr[3].toLowerCase().trim();
			
			//lowerCase
			if(token.contains("string:is") || token.contains("string:is not") || token.contains("string:equal")) {
				fieldName = "lowerUTF8("+fieldName+")";
				
				if(!token.contains(" in") && !token.contains("string:is value") && !value.contains("'")) {
					value = "'"+value+"'";
				}
			}
			...
			if(fieldName.toLowerCase().trim().startsWith("sku.") ){
				productsubQry = product_purchase_qry_template; //.replace("<NOTCOND>", "NOT");
				if(token.contains("is not")) {
					mapStr = tokensHashMap.get("string:is not equal");
					productsubQry = productsubQry.replace("<NOTCOND>", "NOT");
					//mapStr = mapStr.replace("", newChar)
				}else{
					productsubQry = productsubQry.replace("<NOTCOND>", "");
				}
			}
			...
			String tempStr="";
				...
				if(token.contains("not in")) {
					tempStr +=  " AND "+fieldName+" != '' " ;
				}
			...
			if(token.contains("date:ignoreyear_")) outStr=" ("+opensClicksSubCond + tempStr +") ";
				...
				outStr=" ("+opensClicksSubCond + fieldName+ tempStr +") ";
				if(token.equalsIgnoreCase("string:is not") || token.contains("string:is not equal")) {
					outStr += " AND ("+ fieldName + " != '' ) ";
				}
				if(token.contains("string:none")) {
					outStr += " OR ("+ fieldName + " = '' ) ";
				}
			}
			...
			if(token.contains("number:!=") || token.contains("string:does not contain")) {
				outStr = outStr + " AND "+ fieldName + " != ''";
			}
			
			if(productsubQry != null){
				outStr = productsubQry.replace("<PRODUCTCOND>", "("+fieldName+ tempStr +") ");
			}
		} catch (Exception e) {
			logger.error("Exception ::::", e);
		}
	...
	public static String prepareDateSubQuery(String fieldName, String[] tokenArr, String userIdStr , String qryType,long mlBit) {
		...
		if(qryType.equals(Constants.SEGMENT_ON_EMAIL)) {
			...
			tempStr = tot_subQuery_notification.replace("<USERIDS>", userIdStr).replace("<MLBITS>", mlBit+"");
