...
public class CouponProvider {
	...
	private CouponProvider(CouponCodesDao couponCodesDao) {
		...
		try {
			...
			couponCodesDao = (CouponCodesDao)locator.getDAOByName("couponCodesDao");
			couponCodesDaoForDML = (CouponCodesDaoForDML)locator.getDAOForDMLByName("couponCodesDaoForDML");
	...
	private static CouponProvider couponProvider;
	 ...
	 public String getAlreadyIssuedCoupon(String coupPHStr,  Long contactID){
		 try {
			 if (couponCodesDao == null)
			 ...
			 // TODO Auto-generated catch block
			 logger.error("no dao found with id ::" + OCConstants.COUPONCODES_DAO);
		 }

		 String[] strArr = coupPHStr.split("_");
		 ...
		 if (coupon != null &&
			 ...
			 List<CouponCodes> couponCodesList = couponCodesDao.findBy(coupon.getCouponId(), contactID);
			 int numOfDays = 0;
			 ...
			 if (expiryType.equalsIgnoreCase("D")) {
				 ...
				 if (expiryDetails != null) {
					 ...
					 try {
						 ...
						 Calendar validPeriodTime = Calendar.getInstance();
						 validPeriodTime.add(Calendar.DAY_OF_MONTH, -numOfDays);
						 for (CouponCodes couponCode : couponCodesList) {
							 if (couponCode.getIssuedOn().after(validPeriodTime)) {
		...
		public synchronized String  getNextCouponCode(String tempStr, String campaignName, String campaignType, 
			...
			try {
				...
				if(campaignType.equalsIgnoreCase(Constants.COUP_GENT_CAMPAIGN_TYPE_SINGLE_SMS)) {
