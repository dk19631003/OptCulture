	...
	public List<TemplateCategory> updateUnsubscribe(Long userId, Long crId, Long sentId,
		...
		if(categoryWeight > 0 ) {
			...
			campaignSentDaoForDML.updateCampaignSent(sentId, Constants.CS_STATUS_UNSUBSCRIBED);
			 {
				if(logger.isDebugEnabled()) logger.debug(" CampaignSent is updated as unsubscribed for sentId :"+sentId);
			}
		}
		else {
		...
		CampaignReportDaoForDML campaignReportDaoForDML = (CampaignReportDaoForDML)
				context.getBean("campaignReportDaoForDML");
		...
		if(categoryWeight == 0 ) {
			...
			unsubscribe.setUnsubscribedDate(dt);
...
/*				if(unsubscribesDao.updateUnsubscribe(emailId, userId, 
						reasonStr, categoryWeight, dt) == 0)*/
				if(unsubscribesDaoForDML.updateUnsubscribe(emailId, userId, 
						reasonStr, categoryWeight, dt) == 0){
					if(logger.isWarnEnabled()) logger.warn(" Could not update the unsubscribe for (emailId, userId)"
							+emailId+", "+userId);
				}
