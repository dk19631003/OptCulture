...
public class WARecipientProvider {
	...
	private boolean isLastWADateUpdated = false;
    ...
    private String loyaltyUrl = null;

	private List<WACampaignSent> sentList = new ArrayList<WACampaignSent>();

	private Contacts tempContact;
    private ApplicationContext context;


	public WARecipientProvider(ApplicationContext context, WACampaignReport waCampaignReport,
		...
		waCampaignSentDao = (WACampaignSentDao)context.getBean("waCampaignSentDao");
		...
		waCampaignReport.setSent(totalSizeInt);
		currentSentId = waCampaignSentDao.getCurrentSentId();
	...
	public synchronized Contacts getNext() {
		...
		if(totalSizeInt ==0 ) {
			if(logger.isWarnEnabled()) logger.warn("********** Found 0 Active contacts to send the WhatsApp");
			return null;
		}
		...
		contacts = contactsDao.getSegmentedContacts("SELECT * FROM wa_tempcontacts ",
				currentSizeInt, 10000);
		
		
    	//added for substitution tags
    	StringBuffer cidsStr = new StringBuffer();
    	
    	
    	if(contacts != null && contacts.size() > 0 && totalPhSet != null && totalPhSet.size() > 0) {//only for each 10,000
    				...
    				for (ContactsLoyalty contactsLoyalty : retList) {
						if(user == null){
							try {
								...
								user = usersDao.find(contactsLoyalty.getUserId());
		...
		if(contacts == null || contacts.size() == 0) {
			...
			/**
			 * If lastContactId not null then updates the 'lastWADate'
			 * for the contacts to which whatsapp sent
			 */
			if(lastContactId != null && !isLastWADateUpdated) {

				DateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				String updateQryStr = null;
				if(waCampaignReport != null) { // campaign submission

					updateQryStr ="update contacts c, wa_tempcontacts s  set c.last_wa_date ='"+format.format(lastWADate)+"' where c.cid=s.cid"; 
				}
				...
				isLastWADateUpdated = true;
			}
			return null;
		}
		/**
		 * Last Contact Id in the list for reference to
		 * update the lastWADate of contacts table till this contact
		 */
		...
		if(waCampaignReport != null) {
			...
			// Generates the sentId for reference to WACampaignSent object
			...
			while(this.iter != null && this.iter.hasNext()) {
				...
				WACampaignSent waCampaignSent = new WACampaignSent(currentSentId, tempContact.getMobilePhone(), 
						waCampaignReport, "Status Pending",tempContact.getContactId());

				tempContact.setTempObj(waCampaignSent);

				sentList.add(waCampaignSent);
			}
			if(logger.isDebugEnabled()) {
				logger.debug(">>>>>Total Campaign Sent objects to save :"+sentList.size());
			}

			/**
			 *    Saves the WACampaignSent objects into the wa_campaign_sent table
			 *    and update the lastWADate for the contact to which campaign is sent.
			 */
			try {

				//waCampaignSentDao.saveByCollection(sentList);
				waCampaignSentDaoForDML.saveByCollection(sentList);
				sentList.clear();
	 ...
	 public String getlastPurchaseDate(String contactId) {
		...
		public String getlastPurchaseStore(String contactId) {
			try {
				...
				String storeAddr = WACampaignSubmitter.getStoreAddress(store);
		...
		public String getlastPurchaseDetails(String contactId, String cfStr) {
			try{
				...
				if(cfStr.toLowerCase().endsWith(PlaceHolders.CAMPAIGN_PH_LASTPURCHASE_DATE.toLowerCase())){
					...
					String storeVal = WACampaignSubmitter.getStorePlaceholder(storeDetails, cfStr);
	    ...
	    public String getLoyaltyPlaceHolderVal(String contactId, String placeholder) {
	    	...
	    	try {
				...
				loyaltyPlaceholder = replaceLoyaltyPlaceHolders(placeholder,contactsLoyalty,null);
		...
		private String replaceLoyaltyPlaceHolders(String placeholder,ContactsLoyalty contactsLoyalty,String defVal) throws Exception {
			...
			//LOYALTY_POINTS_BALANCE
				...
				loyaltyPlaceholder = (contactsLoyalty.getLoyaltyBalance() != null&& contactsLoyalty.getMembershipStatus().equalsIgnoreCase(OCConstants.LOYALTY_MEMBERSHIP_STATUS_ACTIVE)) ? contactsLoyalty.getLoyaltyBalance().longValue()+" Points" : defVal;
			...
			//LOYALTY_LIFETIME_PURCHASE_VALUE
	    		...
	    		logger.info("LOYALTY_LIFETIME_PURCHASE_VALUE inside  ::");
				Double lpv = null;
				lpv=LoyaltyProgramHelper.getLPV(contactsLoyalty);
				loyaltyPlaceholder = (lpv != null&& contactsLoyalty.getMembershipStatus()==OCConstants.LOYALTY_MEMBERSHIP_STATUS_ACTIVE) ? decimalFormat.format(lpv) : defVal;
				logger.info("LOYALTY_LIFETIME_PURCHASE_VALUE ::"+loyaltyPlaceholder);
			}
			...
			//redeemable currency
	    		...
	    		logger.info("LRC"+loyaltyPlaceholder);
	    	}
		...
		private String getresetPasswordURL(ContactsLoyalty contactsLoyalty,String defVal) {
			...
			return loyaltyUrl!= null ? loyaltyUrl : defVal;
