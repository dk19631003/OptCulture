...
public class LoyaltyProgramHelper {
	...
	public static void updateLoyaltyMembrshpPhone(Long userId, Contacts contact, String newPhone){
		try{
			...
			List<ContactsLoyalty> contactLoyaltyList = contactsLoyaltyDao.findOCLoyaltyListByContactId(userId, contact.getContactId());
				...
				if(isDifferentMobile) {
					if(OCConstants.LOYALTY_MEMBERSHIP_TYPE_CARD.equalsIgnoreCase(loyalty.getMembershipType())) {
						...
						if(prgmObj.getUniqueMobileFlag() == OCConstants.FLAG_YES) {
							ContactsLoyalty tempLtyContact = contactsLoyaltyDao.getLoyaltyByPrgmAndPhone(loyalty.getUserId(), loyalty.getProgramId(),newPhone);
