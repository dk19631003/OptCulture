...
import org.mq.captiway.scheduler.beans.UserScoreSettings;
import org.mq.captiway.scheduler.beans.Users;
import org.mq.captiway.scheduler.dao.ContactScoresDao;
import org.mq.captiway.scheduler.dao.ContactScoresDaoForDML;
import org.mq.captiway.scheduler.dao.UserScoreSettingsDao;
import org.mq.captiway.scheduler.dao.UserScoreSettingsDaoForDML;
...
public class ContactScoreSetting extends Thread {
   ...
   public synchronized void updateScore(Users user, String emailId, 
		...
		if(logger.isDebugEnabled()) logger.debug("-----just Enter in updateScore- ------");
			
		scoresQueue.add(new Object[]{ user,emailId, scoreGroupName, dataOne});
		if(logger.isDebugEnabled()) logger.debug("Leaving the method ..");
		
		if(!this.isRunning()) {
			if(logger.isDebugEnabled()) logger.debug("starting the thread ...");
   ...
   /*public synchronized void googleAnalyticsUpdateScore(Users user,String emailId,String scoreGroupName){
	   if(logger.isDebugEnabled()) logger.debug("-----just Enter in googleAnalyticsUpdateScore- ------");
		...
		try{
			...
			while((tempScoreObject = scoresQueue.peek()) != null) {
				...
				try {
					 ...
					 if(logger.isDebugEnabled()) logger.debug("OBJECT[] VAL : Data one : "+ dataOne + " ** score group name : "+ scoreGroupName + " ** user Id : "+ user.getUserId() );
					...
					if(logger.isDebugEnabled()) logger.debug("userScoreSetting scoreGroupName  :"+scoreGroupName + "maxScore is"+maxScore);
					
					if(userScoreSettings == null) {
						if(logger.isErrorEnabled()) logger.error("*** Exception: User Setting entry for "+scoreGroupName + " does not exist, User : "+ 
					...
					if(scoreGroupName.equalsIgnoreCase(Constants.SCORE_EMAIL_OPEN)) {
						...
						if(logger.isDebugEnabled()) logger.debug("open count :" + openCount + " ** userScoreSettings.getScore() =" + userScoreSettings.getScore());
					} //emailOpen
					...
					//***********************  EmailClick ********************
						...
						if(logger.isDebugEnabled()) logger.debug("click  count :" + clickCount + " ** userScoreSettings.getScore() =" + userScoreSettings.getScore());
					} //  emailClick
					...
					//***********************  EmailUnSubscribed ********************
						...
						if(logger.isDebugEnabled()) logger.debug("unsubsribe count :" + unSubScribeCount + " ** userScoreSettings.getScore() =" + userScoreSettings.getScore());
					} // emailUnsubscribe
...
//									 scoreCndMap.put(PVISISTALLVISIT+index,obj);
						...
						if(logger.isDebugEnabled()) logger.debug("PageVisited Count :" + pageVistCount + " ** userScoreSettings.getScore() =" + score);
							 ...
							 // logger.debug("Downloaded records listSize  :"+list.size());
							  for (Object obj : list) {
								  ...
								  if(str.contains(dataOne)) {
									...
									if(logger.isDebugEnabled()) logger.debug("download  count :" + downloadCount + " ** userScoreSettings.getScore() =" + userScoreSettings.getScore());  
								  }
							}
						  }else return;
					} //downloaded
					...
					//***********************  SourceOfVisit********************
						 ...
						 if(list.size()>0){
							 if(logger.isDebugEnabled()) logger.debug("SorurceOfRecords list size is ====>"+list.size());
							 userScoreSettings=(UserScoreSettings)list.get(0);
							 maxScore=userScoreSettings.getMaxScore(); 
							 if(logger.isDebugEnabled()) logger.debug("src vists maxScore is"+userScoreSettings.getMaxScore());
							 
							 for (Object object : list) {
								 userScoreSettings=(UserScoreSettings)object;
								 Object[] obj=new Object[]{ userScoreSettings.getDataOne(),  userScoreSettings.getScore()};
								 if(logger.isDebugEnabled()) logger.debug("-- Anonymous object created---");
						 ...
						 for(int j=0;j<set.size();j++){
							 if(scoreCndMap.containsKey(SRCVISISTCNT+j)) {
								 ...
								 if(logger.isDebugEnabled()) logger.debug("dateOne is  :"+str  +"score i val is :"+score);
								 if(str.contains( dataOne)){
						...
						// logger.debug("minScore is====>"+score);
						 ...
						 int sourceOfVisitCount = contactScore.getSourceOfVisitCount() != null ?  contactScore.getSourceOfVisitCount() :  0;	 
							...
							if(logger.isDebugEnabled()) logger.debug("SourceOfVisitCount :" + sourceOfVisitCount + " ** userScoreSettings.getScore() =" + userScoreSettings.getScore());
	...
	public Object  findByContactScoresObj(Users  user,String emailId){
		...
		try {
			...
			if(logger.isDebugEnabled()) logger.debug("ContactScore Object is ===>"+contactScore.getEmailId());
			contactScore.setLastModifiedDate(Calendar.getInstance());
			return contactScore;
		} catch (Exception e) {
