...
*/
	...
	public int updateInitialStatusBsmsItsPak(Set<Long> bsmsItsSentIdsSet, String status) {
		 ...
		 try {
			 	...
			 	String queryStr = "UPDATE sms_campaign_sent SET status='"+status+"' WHERE sent_id IN ("+sentIdsStr+") ";
				logger.info("queryStrBsmsItsPak >>> "+queryStr);
	...
	public int updateStatus(String status, String mobile, String reqId) {
		/*if(mobile.length() == 12) {
			...
			String queryStr = "update SMSCampaignSent set status ='"+status+"' where requestId='"+reqId+"' and mobileNumber = '"+mobile+"'";
	...
	public int updateStatusBySentId(String status, String mobile, Long sentId) {
		if(mobile.length() == 12) {
			...
			String queryStr = "update SMSCampaignSent set status ='"+status+"' where sentId="+sentId;// and mobileNumber like '%"+mobile+"'";
			int updateCount = executeUpdate(queryStr);
			return updateCount;
	}
...
*/		String queryStr = "update SMSCampaignSent set status ='"+status+"',apiMsgId='"+apiMsgId+"' where sentId="+cliMsgId;
	...
	*/
	...
	public int updateApiMsgId(String actualSentId, Long smsCampRepId, String apiMsgId) {	
		...
		String updateQry = " UPDATE sms_campaign_sent SET api_msg_id='"+apiMsgId+"' WHERE  sent_id="+actualSentId;				
		
		logger.debug(updateQry);		
		return executeJdbcUpdateQuery(updateQry);		
	
	}
	
	public int updateMrId(String actualSentId, Long smsCampRepId, String apiMsgId,String mobile) {	
	...
	public int updateSynapseRefId(String actualSentId, Long smsCampRepId, String apiMsgId) {	
		
		String updateQry = " UPDATE sms_campaign_sent SET api_msg_id='"+apiMsgId+"' WHERE  sent_id in("+actualSentId+") ";				
		
		logger.debug(updateQry);		
		return executeJdbcUpdateQuery(updateQry);		
	
	}
	public int updateInfocommAPIId(String actualSentId, Long smsCampRepId) {	
		
		String updateQry = " UPDATE sms_campaign_sent SET api_msg_id=sent_id WHERE "+(smsCampRepId == null ? "" :			
			"sms_cr_id="+smsCampRepId+" ")
				//+" sent_id in("+actualSentId+") "
				;				
...
public List<SMSCampaignSent> findByMsgIds(String msgIds) {
	...
	public List<SMSCampaignSent> findByRepId(Long smsCampRepId, String status, int startFrom, int count) {
	 ...
	 public int updateReqId_BsmsIts(Set<Long> bsmsItsSentIdsSet, String requestId){
		 ...
		 try {
			 	...
			 	String queryStr = "UPDATE sms_campaign_sent SET request_id='"+requestId+"',status='"+SMSStatusCodes.CLICKATELL_STATUS_RECEIVED+"'  WHERE sent_id IN ("+sentIdsStr+") ";
				logger.info("queryStrBsmsIts >>> "+queryStr);
