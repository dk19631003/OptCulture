...
//    	super.saveOrUpdateAll(retailProSalesCollection);
    ...
    */
    ...
    public void updateSalesAggregateData(Long sourceMlistId, Long userId, Long startcontactID, Long endContactID ) {
    	...
    	String sql = " REPLACE INTO sales_aggregate_data(customer_id, tot_reciept_count ," +
    			...
    			" WHERE  "+
    			" user_id="+userId.longValue()+" AND cid between " + startcontactID +" AND "+endContactID+"    GROUP BY doc_sid) AS t2 " +
    			" Where user_id="+userId.longValue()+" AND cid between " + startcontactID +" AND "+endContactID+"  GROUP BY cid";
    ...
    public void updateSalesAggregateData(Long sourceMlistId, Long userId, Long cid) {	//APP-3944 //APP-4403
		...
		/*String sql = " REPLACE INTO sales_aggregate_data(customer_id, tot_reciept_count ," +
				...
				" sum((sales_price*quantity)+tax-(IF(discount is null,0,discount))) as t_sum, list_id,cid, user_id FROM retail_pro_sales AS t1" +
				" WHERE  "+
				"  user_id="+userId.longValue()+" AND cid="+cid+" GROUP BY doc_sid) AS t2 " +
				" Where user_id="+userId.longValue()+" AND cid="+cid+" AND t2.cid IS NOT NULL  GROUP BY t2.cid";*/
    	
    	//REPLACE takes time, so if a record already exists, UPDATE it first; if not, INSERT it. 
    	
    	String UpdateQry = "UPDATE sales_aggregate_data aggr JOIN ( SELECT COUNT(t1.t_count) tot_reciept_count,"
    					+ "	SUM(t1.t_sum) tot_purchase_amt, AVG(t1.t_sum) avg_purchase_amt,MAX(t1.t_sum) max_purchase_amt "
    					+ "	FROM (SELECT count(doc_sid) as t_count, sum((sales_price*quantity)+tax-(IF(discount is null,0,discount))) as t_sum"
    					+ "		FROM retail_pro_sales AS t1"
    					+ "		WHERE  user_id="+userId.longValue()+" AND cid="+cid+" GROUP BY doc_sid) AS t1 ) AS t2 "
    					+ " SET aggr.tot_reciept_count =t2.tot_reciept_count,"
    					+ "    aggr.tot_purchase_amt=t2.tot_purchase_amt, "
    					+ "    aggr.avg_purchase_amt=t2.avg_purchase_amt,"
    					+ "    aggr.max_purchase_amt=t2.max_purchase_amt "
    					+ "    WHERE aggr.user_id="+userId.longValue()+" AND aggr.cid="+cid;

    	
    	logger.info("UpdateQry>>>>>>>"+UpdateQry);
    	int updatedCount = executeJdbcUpdateQuery(UpdateQry);
    	
    	if(updatedCount==0) {//then Insert new record
			
			String InsertQry = "INSERT INTO sales_aggregate_data"
							+ " (customer_id, tot_reciept_count , tot_purchase_amt, avg_purchase_amt, max_purchase_amt,"
							+ " list_id, cid, user_id) "
							+ "	SELECT customer_id, COUNT(t2.t_count), SUM(t2.t_sum), AVG(t2.t_sum), MAX(t2.t_sum) ,"
							+ 	sourceMlistId.longValue()+", "+cid+", " +userId.longValue()
							+ "	FROM (	SELECT customer_id, COUNT(doc_sid) as t_count,"
							+ "			SUM((sales_price*quantity)+tax-(IF(discount is null,0,discount))) as t_sum "
							+ "    		FROM retail_pro_sales AS t1 WHERE user_id="+userId.longValue()+" AND cid="+cid
							+ "    		GROUP BY doc_sid) AS t2 ";
							//+ " WHERE user_id="+userId.longValue()+" AND cid="+cid;
			
			logger.info("InsertQry>>>>>>>"+InsertQry);
			executeJdbcQuery(InsertQry);
    	}
 ...
 public void updateSalesAggregateData(Long sourceMlistId, Long userId, String cidStr) {	//APP-3944
    	...
    	String sql = " REPLACE INTO sales_aggregate_data(customer_id, tot_reciept_count ," +
    			...
    			" sum((sales_price*quantity)+tax-(IF(discount is null,0,discount))) as t_sum, list_id,cid, user_id FROM retail_pro_sales AS t1" +
    			" WHERE  "+
    			"  user_id="+userId.longValue()+" AND cid IN ("+cidStr+") GROUP BY doc_sid) AS t2 " +
    			" Where user_id="+userId.longValue()+" AND cid IN ("+cidStr+")  GROUP BY t2.cid";
...
*/ 	
 	public void  updateSalesToContactsMappings(Set<MastersToTransactionMappings> mappingSet, 
 			Long listId, Long userId , Long startSaleID, Long endSaleID) {
		...
		try {
			...
			String salesToContactsQry =" UPDATE retail_pro_sales rs , contacts  c  SET rs.cid=c.cid   WHERE "
					+ "c.user_id ="+userId+" AND rs.cid IS null  AND rs.user_id="+userId + " AND rs.sales_id Between "+startSaleID+" AND "+endSaleID+ salesToContactStr;
			  String salesToInventoryQry =" UPDATE retail_pro_sales rs ,"
			  		+ " retail_pro_sku s set rs.inventory_id=s.sku_id WHERE s.user_id="+userId+" AND rs.inventory_id IS null AND rs.user_id="+userId +
			  		" AND rs.sales_id Between "+startSaleID+" AND "+endSaleID+ salesToInventoryStr;
			 ...
			 // jdbcTemplate.batchUpdate(new String[] {salesToContactsQry,salesToInventoryQry});
			  ...
			  try {
				...
				logger.error("in catch of CannotAcquireLockException ", le);
				Calendar now = Calendar.getInstance(); 
				SalesUpdateErrorLogDao salesUpdateErrorLogDao = null;
				SalesUpdateErrorLogDaoForDML salesUpdateErrorLogDaoForDML = null;
				try {
			  ...
			  try {
				...
				if(salesUpdateErrorLogDao != null ){
					...
					logger.error("in catch of DeadlockLoserDataAccessException ", le);
					Calendar now = Calendar.getInstance(); 
					SalesUpdateErrorLogDao salesUpdateErrorLogDao = null;
					SalesUpdateErrorLogDaoForDML salesUpdateErrorLogDaoForDML = null;
