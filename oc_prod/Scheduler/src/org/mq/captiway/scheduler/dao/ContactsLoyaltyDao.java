  ...
  //TODO modify
	public List<ContactsLoyalty> getLoyaltyForPlaceHolderVal( String cids){
		...
		try {
			...
			"c.contactId IN ("+ cids+" ) AND cl.userId=c.users.userId AND cl.contact.contactId=c.contactId"; //HS: Add userId match to pick right index.
...
public ContactsLoyalty getContactsLoyaltyByCustId(String customerId, Long userId){
	...
	public ContactsLoyalty getContactsLoyaltyByCardId(Long cardId, Long userId) {
		  ...
		  String query = " FROM ContactsLoyalty WHERE userId = "+userId+" AND cardNumber='"+cardId+"' ";//APP-4728
	...
	//This Only for duplicate ContactLoyalty exist for same Card
		...
		public ContactsLoyalty getLoyaltyByPrgmAndMembrshp(long userId, Long programId,String mobilePhone) {
			try {
	    		List<ContactsLoyalty> list = null;
				list = getHibernateTemplate().find("from ContactsLoyalty where userId="+userId+" AND programId = " + programId+" AND cardNumber = '" + mobilePhone +"' " );//APP-4728
		...
		public ContactsLoyalty getLoyaltyByPrgmAndPhone(Users user, Long programId, String phone) {
			try {
				...
				list = getHibernateTemplate().find("from ContactsLoyalty where userId="+user.getUserId()+" AND programId = " + programId+" AND mobilePhone " + valueStr+" " );
