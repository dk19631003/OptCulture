	...
	public void run() {
		...
		try {
			...
			try{
				...
				if(logger.isDebugEnabled()) logger.debug("inactiveSmsList="+inactiveList);
			...
			/*
			 * removed as it is running for every 5 mins and taking 12 sec to execute- //
			 * Get active Double optin lists List<MailingList> list =
			 * mailingListDao.findConfirmOptInList();
			 *  if (list != null && list.size() > 0) {
			 * if (logger.isInfoEnabled())
			 * logger.info(">>>> No. of Active Mailing Lists found: " + list.size());
			 * pmtaQueue.addCollection(list); 
			 * } 
			 * else { if (logger.isInfoEnabled())
			 * logger.info(">>>> No. of Active Mailing Lists found: 0"); }
			 */
