...
import org.mq.captiway.scheduler.dao.MQSRequestDaoForDML;
import org.mq.captiway.scheduler.utility.Constants;
import org.mq.captiway.scheduler.utility.XMLReader;
...
public class MQService {
	...
	MQSRequestDaoForDML mqsRequestDaoForDML = (MQSRequestDaoForDML)SpringUtil.getBean("mqsRequestDaoForDML");
	...
	public void loadProperties(){
		try {
			...
			if(logger.isDebugEnabled()) logger.debug("Loading the mqs properties file successful");
	...
	public HashMap<String, String> getSubscriptionValidity(String customerNo){
		...
		if(logger.isDebugEnabled()) logger.debug("Reguest XML : " + requestXML);
		...
		if(logger.isDebugEnabled()) logger.debug("getSubscriptionValidity Resonce : " + respMap);
		return respMap;
	}
	...
	public HashMap<String, String> getAvailableUsageLimit(String customerNo){
		...
		if(logger.isDebugEnabled()) logger.debug("getAvailableUsageLimit Resonce : " + respMap);
		return respMap;
	}
	...
	public HashMap<String, String> processOrder(String customerNo,String serviceType,String itemName, String itemPrice){
		...
		if(logger.isDebugEnabled()) logger.debug("processOrder Resonce : " + respMap);
		return respMap;
	}
		 ...
		 try {
			...
			//logger.debug("Request : " + request);
			    ...
			    try {
			        ...
			        MQSRequest mQSRequest = new MQSRequest(referenceNum, service, status, new Date(),request,response);
			        try {
			        	//mqsRequestDao.saveOrUpdate(mQSRequest);
			        	mqsRequestDaoForDML.saveOrUpdate(mQSRequest);
			        	if(logger.isDebugEnabled()) logger.debug("MQSRequest is stored with RefNum : " + referenceNum);
			        } catch (Exception e) {
