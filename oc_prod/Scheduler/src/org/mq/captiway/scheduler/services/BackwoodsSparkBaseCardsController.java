...
import org.mq.captiway.scheduler.dao.SparkBaseCardDaoForDML;
import org.mq.captiway.scheduler.dao.SparkBaseLocationDetailsDao;
import org.mq.captiway.scheduler.dao.SparkBaseLocationDetailsDaoForDML;
...
public class BackwoodsSparkBaseCardsController extends TimerTask implements ApplicationContextAware{
	...
	public void run() {
		...
		try {
			...
			contactLoyaltyDaoForDML = (ContactsLoyaltyDaoForDML)context.getBean("contactsLoyaltyDaoForDML");
			...
			List<SparkBaseLocationDetails> list = sparkBaseLocationDetailsDao.findAllActiveLocByTimeDiff(Constants.USER_AND_ORG_SEPARATOR+"Bw");
...
//									contactsLoyalty2.setOptinMedium(Constants.CONTACT_LOYALTY_TYPE_POS);
									...
									contactLoyaltyDaoForDML.saveOrUpdate(contactsLoyalty2);
									sparkBaseCardDaoForDML.saveOrUpdate(sparkBaseCard);
									...
									object = SparkBaseAdminService.fetchData(SparkBaseAdminService.CUSTOMERINFO, accountId, contactsLoyalty2,true);
										...
										if(object != null && object instanceof CustomersViewResponse) {
											...
											if(customer == null) {
												...
												contact = contactsDao.findContactByUniqPriority(prioMap, inputContact, msgUser);
			  ...
			  if(templateId != null) {
				 ...
				 if(custTemplate!=null) {
					  if(custTemplate.getHtmlText() != null && !custTemplate.getHtmlText().isEmpty()) {
						...
						}else if(custTemplate.getEditorType().equalsIgnoreCase(Constants.EDITOR_TYPE_BEE) && custTemplate.getMyTemplateId()!= null) {
