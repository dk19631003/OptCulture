	...
	public void run(){
		...
		try {
			...
			for(UserPosFTPSettings userSetting : userPosFTPSettings){
				...
				Users userObj = usersDao.find(userId);
				if(userSetting.isCheckAlert() && userObj.isEnabled() && (userObj.getPackageExpiryDate() != null && userObj.getPackageExpiryDate().after(Calendar.getInstance()))) {
