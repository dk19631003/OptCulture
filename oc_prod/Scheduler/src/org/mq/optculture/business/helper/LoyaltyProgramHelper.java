...
public class LoyaltyProgramHelper {
	...
	public static void updateLoyaltyMembrshpPhone(Users user, Contacts contact, String newPhone){
		try{
			...
			List<ContactsLoyalty> contactLoyaltyList = contactsLoyaltyDao.findOCLoyaltyListByContactId(user.getUserId(), contact.getContactId());
				...
				if(isDifferentMobile) {
					if(OCConstants.LOYALTY_MEMBERSHIP_TYPE_CARD.equalsIgnoreCase(loyalty.getMembershipType())) {
						...
						if(prgmObj.getUniqueMobileFlag() == OCConstants.FLAG_YES) {
							ContactsLoyalty tempLtyContact = contactsLoyaltyDao.getLoyaltyByPrgmAndPhone(user, loyalty.getProgramId(),newPhone);
	...
	public static void sendSmsAlert(Users user, String phone, String message) {
		...
		try{
			...
			captiwayToSMSApiGateway.sendSingleSms(ocGateway, msgContent, phone, senderId,null,null);
			/**
			 * Used SMS Count
			 */
			try {
				UsersDaoForDML usersDao = (UsersDaoForDML)ServiceLocator.getInstance().getDAOForDMLByName(OCConstants.USERS_DAOForDML);
...
public static double calculatePointsAmount(ContactsLoyalty contactsLoyalty, org.mq.captiway.scheduler.beans.LoyaltyProgramTier tier) throws Exception {
	...
	public static String preparePhoneNumber(Users user,String phoneNumber) {//APP-4485
