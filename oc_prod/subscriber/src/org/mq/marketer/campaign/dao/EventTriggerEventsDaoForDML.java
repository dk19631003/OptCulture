...
public class EventTriggerEventsDaoForDML extends AbstractSpringDaoForDML{

	public EventTriggerEventsDaoForDML() {
 ...
 */
	 /*public int findSearchEmailCount(Long userId, Long eventTrigId, String fromDate,String toDate,String key) {
			...
			try {
				String subQry="SELECT COUNT(tot_count) FROM ( SELECT COUNT(e.event_id) as tot_count " +
								" FROM event_trigger_events e, campaign_sent cs WHERE e.user_id ="+userId.longValue()+
	 ...
	 public int findSearchSmsCount(Long userId, Long eventTrigId, String fromDate,String toDate,String key) {
		...
		try {
			String subQry="SELECT COUNT(tot_count) FROM (SELECT COUNT(e.event_id) as tot_count " +
							" FROM event_trigger_events e, sms_campaign_sent scs WHERE e.user_id ="+userId.longValue()+
	...
	public int findAllCampaignEventsCount(Long userId, Long eventTrigId, String fromDate, String endDate, boolean isEmail ) {
		...
		try {
			...
			String hql = " SELECT COUNT(eventId) FROM  EventTriggerEvents WHERE userId ="+userId.longValue()+
 ...
 */
	...
	/*public long findAllEmailSentReport(Long userId, Long eventTrigId, String fromDate, String endDate) {
		...
		String qry = " SELECT COUNT(e.eventId) FROM  EventTriggerEvents e, CampaignSent cs WHERE e.userId ="+userId.longValue()+
 ...
 */
	...
	/*public List<Map<String, Object>> findUniqueOpensAndClicks(Long userId, Long eventTrigId, String fromDate, String endDate){
			...
			try {
				String qry = " SELECT SUM(IF(cs.opens>0,1,0)) as uniqueOpens ,SUM(IF(cs.clicks>0,1,0))  as uniqueClicks FROM  event_trigger_events e, campaign_sent cs WHERE e.user_id ="+userId.longValue()+
				" AND e.event_trigger_id="+eventTrigId.longValue()+" AND e.camp_sent_id IS NOT NULL  AND e.camp_cr_id=cs.cr_id " +
	...
	public List<Map<String, Object>> findUniqueSMSClicks(Long userId, Long eventTrigId, String fromDate, String endDate){
		...
		try {
			String qry = " SELECT SUM(IF(cs.clicks>0,1,0))  as uniqueClicks FROM  event_trigger_events e, sms_campaign_sent cs WHERE e.user_id ="+userId.longValue()+
			" AND e.event_trigger_id="+eventTrigId.longValue()+" AND e.camp_sent_id IS NOT NULL  AND e.camp_cr_id=cs.sms_cr_id " +
			" AND e.camp_sent_id = cs.sent_id AND e.campaign_sent_date BETWEEN '"+fromDate+"' AND '"+endDate+"' AND cs.status='Delivered'";
 ...
 */	
	...
	/*public long findAllSmsSentReport(Long userId, Long eventTrigId, String fromDate, String endDate) {
		...
		String qry = " SELECT COUNT(e.eventId) FROM  EventTriggerEvents e, SMSCampaignSent scs WHERE e.userId ="+userId.longValue()+
  ...
  */
	/*public List<Object[]> findTotEmailRepRate(Long userId, Long eventTrigId, String fromDate, 
		...
		try {
			...
			String qry = " SELECT COUNT(e.eventId),"+appenQry+" FROM  EventTriggerEvents e, CampaignSent cs WHERE e.userId ="+userId.longValue()+
							" AND e.eventTriggerId="+eventTrigId.longValue()+" AND e.campSentId IS NOT NULL  AND e.campCrId=cs.campaignReport.crId " +
							" AND e.campSentId = cs.sentId AND e.campaignSentDate BETWEEN '"+fromDate+"' AND '"+endDate+"'" +
							subQry+grpQry;
  ...
  */
	...
	/*public List<Map<String, Object>> findUniqueOpensAndClicks(Long userId, Long eventTrigId, String fromDate, String endDate, String type){
			...
			try {
				...
				String qry = " SELECT SUM(IF(cs.opens>0,1,0)) as uniqueOpens ,SUM(IF(cs.clicks>0,1,0))  as uniqueClicks,"+appenQry+" FROM  event_trigger_events e, campaign_sent cs WHERE e.user_id ="+userId.longValue()+
				" AND e.event_trigger_id="+eventTrigId.longValue()+" AND e.camp_sent_id IS NOT NULL  AND e.camp_cr_id=cs.cr_id " +
  ...
  */
	...
	/*public List<Object[]> findTotSmsRepRate(Long userId, Long eventTrigId, String fromDate, 
		...
		try {
			...
			String qry = " SELECT COUNT(e.eventId),"+appenQry+" FROM  EventTriggerEvents e, SMSCampaignSent scs WHERE e.userId ="+userId.longValue()+
							" AND e.eventTriggerId="+eventTrigId.longValue()+" AND e.smsSentId IS NOT NULL  AND e.smsCrId=scs.smsCampaignReport.smsCrId " +
							" AND e.smsSentId = scs.sentId AND e.smsSentDate BETWEEN '"+fromDate+"' AND '"+endDate+"'" +
							subQry+grpQry;
 ...
 */
	...
	/*public List< EventTriggerEvents> findEmailReports(Long userId, Long eventTrigId, int start,int end,String fromDate,String toDate,String key) {
		...
		try {
			...
			String subQry="SELECT cs.email_id,MAX(e.campaign_sent_date) as lastSent, " +
							...
							"SUM(IF(cs.clicks >0, 1,0)) as uniclicks, e.event_id,e.camp_sent_id FROM event_trigger_events e, campaign_sent cs WHERE e.user_id ="+userId.longValue()+
							" AND e.event_trigger_id="+eventTrigId.longValue()+" AND e.camp_sent_id IS NOT NULL  AND e.camp_cr_id=cs.cr_id " +
							" AND e.camp_sent_id = cs.sent_id AND e.campaign_sent_date BETWEEN '"+fromDate+"' AND '"+toDate+"' " +
							" AND cs.email_id IS NOT NULL  ";
			if(key != null){
	...
	public List< EventTriggerEvents> findEmailReportsNew(Long userId, Long eventTrigId, int start,int end,String fromDate,String toDate,String key, String emailSatusTobeFetched) {
		try {
			List<EventTriggerEvents > list = null;
 ...
 */
	...
	/*public List< EventTriggerEvents> findSmsReports(Long userId, Long eventTrigId, int start,int end,String fromDate,String toDate,String key) {
		...
		String subQry="SELECT scs.mobile_number,MAX(e.sms_sent_date) as lastSent, " +
					...
					" e.event_id, e.sms_sent_id FROM event_trigger_events e, sms_campaign_sent scs WHERE e.user_id ="+userId.longValue()+
					" AND e.event_trigger_id="+eventTrigId.longValue()+" AND e.sms_sent_id IS NOT NULL  AND e.sms_cr_id=scs.sms_cr_id  " +
					" AND e.sms_sent_id = scs.sent_id AND e.sms_sent_date BETWEEN '"+fromDate+"' AND '"+toDate+"' " +
					" AND scs.mobile_number IS NOT NULL  ";
		if(key != null){
	...
	public List< EventTriggerEvents> findSmsReportsNew(Long userId, Long eventTrigId, int start,int end,String fromDate,String toDate,String key, byte considerDeliveredStatus) {
	...
	/*public String findEtById(boolean isEmail, Long eventId){
		...
		try {
			...
			if(isEmail) {
				...
				qry = "SELECT cr.content FROM EventTriggerEvents e, CampaignReport cr WHERE e.eventId="+eventId.longValue()+
						" AND e.campCrId IS NOT NULL AND e.campCrId=cr.crId";
			}else{
				
				
				qry = "SELECT scr.content FROM EventTriggerEvents e, SMSCampaignReport scr WHERE e.eventId="+eventId.longValue()+
 ...
 */
	...
	/*public List<Object[]> findAllSentRep(boolean isEmail, EventTriggerEvents etEvent, Long etId, Long userId, String fromDate, String endDate  ) {
		...
		try {
			...
			if(isEmail) {
				...
				qry = " SELECT cs.emailId, e.campaignSentDate, cs.status  FROM  EventTriggerEvents e, CampaignSent cs WHERE e.userId ="+userId.longValue()+
					" AND e.eventTriggerId="+etId.longValue()+" AND e.campSentId IS NOT NULL  AND e.campCrId=cs.campaignReport.crId " +
					" AND e.campSentId = cs.sentId AND cs.emailId='"+etEvent.getEmailId()+"' AND e.campaignSentDate BETWEEN '"+fromDate+"' AND '"+endDate+
					"' ORDER BY e.campaignSentDate DESC";
						
				
			}else{
				
				qry = " SELECT scs.mobileNumber, e.smsSentDate, scs.status  FROM  EventTriggerEvents e, SMSCampaignSent scs WHERE e.userId ="+userId.longValue()+
						" AND e.eventTriggerId="+etId.longValue()+" AND e.smsSentId IS NOT NULL  AND" +
						" e.smsCrId=scs.smsCampaignReport.smsCrId  AND e.smsSentId = scs.sentId AND scs.mobileNumber='"+etEvent.getMobileNumber()+"'" +
	...
	public int executeJdbcUpdateQuery(String queryStr){
		...
		return super.executeJdbcUpdateQuery(queryStr);
