...
public class CampaignSentDao extends AbstractSpringDao {
	...
	public Long getTotalOpensByCrId(Long crId){
	  ...
	  public int getNotSentCount(Long crId) {
		  int notSentCount = 0;
		  try {
			 List res = getHibernateTemplate().find("select count(sentId) from CampaignSent where   campaignReport =" + crId+ " and status='" + CampaignSentStatus.SUBMITTED + "'");
			 if(res.size()>0){
				 notSentCount = ((Long)res.get(0)).intValue();
			 }
		  } catch (Exception e) {
			  logger.error("Exception : ", e);
			  notSentCount = 0;
	 ...
	 public List<CampaignSent> findSentByStatusAndCrId( String status, Long crId) {
			
			try {
				String qry = " FROM CampaignSent WHERE campaignReport="+crId.longValue()+" AND status='"+status+"'";
