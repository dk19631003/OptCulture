...
public class LoyaltyTransactionDao extends AbstractSpringDao {
	...
	public LoyaltyTransaction findByRequestId(String requestId, String userDetail, String type){
		
		String queryStr = " FROM LoyaltyTransaction WHERE  userDetail = '"+userDetail+"' and requestId = '"+requestId+
		"' and type = '" + type + "'";
	...
	public LoyaltyTransaction findByDocSIdRequestIdAndType(String userDetail,String requestId, String docSId, String type){
		String subStr = Constants.STRING_NILL;
		if(docSId != null && !docSId.trim().isEmpty())
			subStr = " and docSID ='"+docSId.trim()+"'";
		else 
			subStr = " and requestId ='"+requestId+"'";
		//String queryStr = " FROM LoyaltyTransaction WHERE requestId = '"+requestId + "' and type = '" + type + "'";
		String queryStr = " FROM LoyaltyTransaction WHERE userDetail = '"+userDetail + "' and type = '" + type + "'"+subStr;
		List transList = getHibernateTemplate().find(queryStr);
		if(transList != null && transList.size() > 0){
			return (LoyaltyTransaction)transList.get(0);
		}
	...
	public LoyaltyTransaction findRequestByTypeReqId(String requestId, String userDetails ) {
		String queryStr = " FROM LoyaltyTransaction WHERE requestId = '"+requestId +"' AND userDetail='"+userDetails+"' AND type = '" + OCConstants.TRANS_TYPE_STOREINQUIRY + "'";
				/*+ " AND requestStatus = '"+OCConstants.JSON_RESPONSE_SUCCESS_MESSAGE +"'";*/
		List transList = getHibernateTemplate().find(queryStr);
		if(transList != null && transList.size() > 0){
			return (LoyaltyTransaction)transList.get(0);
