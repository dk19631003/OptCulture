...
public class CampaignReportDao extends AbstractSpringDao {

    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    private static final Logger logger = LogManager.getLogger(Constants.SUBSCRIBER_LOGGER);
 
	public CampaignReportDao() {}
...
public List<CampaignReport> getAllReports(String userId,String campName,String fromDateStr,String toDateStr, String sourceType, int startIndex, int endIndex,String orderby_colName,String desc_Asc) throws Exception{
 		
 		return getCampaignReports(campName, userId, fromDateStr, toDateStr, false, sourceType, startIndex, endIndex,orderby_colName,desc_Asc);
    }
...
public List<CampaignReport> getReportsByCampaignName(String campaignName,Long userId)  {
	...
	String qry  = " FROM CampaignReport WHERE  campaignName ='" +StringEscapeUtils.escapeSql(campaignName) + "'" +
   ...
   /* this method is called from reports to get the campreports specific to the user (in case of dashboard)*/ 
    public List<CampaignReport> getReportByCampaignName(Long userId,String campaignName,String fromDate,String todate,int startIndx,int endIndx) throws Exception{
    	String queryStr="from CampaignReport where campaignName='"+StringEscapeUtils.escapeSql(campaignName)+"'and user="+userId+" AND sentDate between '" + fromDate + "' AND '" + todate + "'";
    	logger.debug("===========>"+queryStr);
    	return executeQuery(queryStr, startIndx, endIndx);
    	
    }
    public int getTotalCount(Long userId,String campaignName,String fromDate,String todate){
    	return ((Long)getHibernateTemplate().find("select count(*) from CampaignReport" +
    			" where campaignName ='" + StringEscapeUtils.escapeSql(campaignName) + "' and user="+userId+" AND sentDate between '" + fromDate + "' AND '" + todate + "'").get(0)).intValue();
    }
    ...
    public long getReportCountByCampaign(String campaignName,Long userId){
    	...
    	return ((Long)getHibernateTemplate().find("select count(*) from CampaignReport" +
    			" where campaignName ='" + StringEscapeUtils.escapeSql(campaignName )+ "' and user= " + userId).get(0)).longValue();
    }
    ...
    public List<Object[]> getCampaignConsolidatedReport(String campaignName,Long userId){
    	...
    	return getHibernateTemplate().find("select sum(sent),sum(opens),sum(clicks),sum(unsubscribes)," +
    			...
    			userId + " and campaignName ='" + StringEscapeUtils.escapeSql(campaignName) + "' ");
    }
   ...
   public List<String> getUrlListByCampaignId(String campaignName, Long userId) {
	   ...
	   try {
		   ...
		   //what happens if he deleted a campaign which are sent 
			...
			" cr.user_id = "+ userId +" AND cr.campaign_name='"+StringEscapeUtils.escapeSql( campaignName) +"' ";
	   ...
	   public List getCampaignBasedOnContact(long user_id, String emailId ,  String limit) {
		   ...
		   try {
				...
				if(limit.equals(Constants.LIMIT) ) {
					...
					/*qry = "select cr.campaign_name , cs.email_id, cr.sent_date " +
...
/*public int findCoupBasedOnDates(Long userOrgId, String status, String fromDate, String endDate, String promotionName) {
