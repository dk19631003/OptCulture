...
public class SecRolesDao extends AbstractSpringDao {
   ...
   /* public void saveOrUpdate(SecRoles role){
    ...
    public  List<SecRoles> findRoleStoreByType(String type,String RoleName){
     	try {
 			String qry="FROM SecRoles WHERE type in("+type+") and name='"+RoleName+"'";
 			logger.info("StoreOperator query"+qry);
 			return getHibernateTemplate().find(qry);
 		} catch (DataAccessException e) {
 			// TODO Auto-generated catch block
 			logger.error("Exception ::" , e);
