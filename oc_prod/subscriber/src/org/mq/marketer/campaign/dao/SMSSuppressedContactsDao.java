...
public class SMSSuppressedContactsDao extends AbstractSpringDao{

	


    public SMSSuppressedContactsDao() {}
   ...
   /* public void deleteById(Long id) {
    ...
    public List<SMSSuppressedContacts> searchContactsById(Users user,String searchStr) {
		...
		logger.info("searchStr is"+searchStr);
		if(!searchStr.startsWith(user.getCountryCarrier().toString())){
			
			logger.info("entering withoout countryCarrier");
			mobilewithcarrier = user.getCountryCarrier().toString()+searchStr;

		}else {

			logger.info("entering with countryCarrier");
			mobilewithcarrier=searchStr;
			searchStr= searchStr.substring(carriersize);
		}

		logger.info("searchStr value is "+searchStr);
		logger.info("mobilewithcarrier value is "+mobilewithcarrier);
		query = "from SMSSuppressedContacts where user.userId="+user.getUserId().longValue()+ " and mobile in ('"+ searchStr +"','"+ mobilewithcarrier+ "') ";
		logger.info("query for searchContactsById is"+query);
...
*/
 ...
 //APP-1977
 public boolean isMobileOptedOut(Long userId, String mobile,String type){
	 String query = "from SMSSuppressedContacts where user.userId="+ userId +" and mobile like '%" + mobile + "' and type='"+type+"' ";
	 List<SMSSuppressedContacts> list = executeQuery(query);
	 if(list.size() > 0) return true;
	 else return false;
 }
...
//change start 2.5.3.0
 ...
 public List<SMSSuppressedContacts> findSuppressedContactsByReport (Users user,Report report)	
 {
	 ...
	 logger.info("Enter findSuppressedContactsByDates");
	    ...
	    String qry = " FROM SMSSuppressedContacts WHERE user=" +user.getUserId();
...
//	
		...
		List<SMSSuppressedContacts> smsSuppressedContactList = executeQuery(qry+subQry);
		
		logger.info("Exit findSuppressedContactsByDates");
		
		return smsSuppressedContactList; 
 }
//change end 2.5.3.0
