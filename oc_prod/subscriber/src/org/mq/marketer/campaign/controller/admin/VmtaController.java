...
import org.mq.marketer.campaign.dao.VmtaDaoForDML;
...
public class VmtaController extends GenericForwardComposer  {
	...
	private Textbox vmtaDescTbId, vmtaAccountTxtBxId, selectedVmtaNameTextBxId, passTxtBxId, hostTxtBxId, vmtaAccountNameCbId, vmtaAccountAPINameCbId;
	...
	private VmtaDaoForDML vmtaDaoForDML;
	...
	public VmtaController() {
		...
		vmtaDaoForDML = (VmtaDaoForDML)SpringUtil.getBean("vmtaDaoForDML");
//		this.vmtaDao =(VmtaDao) SpringUtil.getBean("vmtaDao");
	...
	private void showVmtaPanel(boolean isFromAdd, Vmta vmta) {
		...
		if(isFromAdd) {
			...
			vmtaAccountAPINameCbId.setValue(selectedVmta.getAccountAPIName());
			emailRouteCbId.setValue(selectedVmta.getVmtaName());
			vmtaAccountNameCbId.setDisabled(true);
			vmtaAccountAPINameCbId.setDisabled(true);
...
*/			
	...
	private boolean isValidVmtaDetails(boolean isAdd) {
		try {
			...
			vmtaAccountAPINameCbId.getChildren().clear();
...
*/			
			...
			if(vmtaAccountAPINameCbId.getValue()==null && vmtaAccountAPINameCbId.getValue().trim().isEmpty() ){
				MessageUtil.setMessage("Please give an account API name to edit.", "color:red", "TOP");
				return false;
			}
...
*/		
			...
			String vmtaAccountAPIName = vmtaAccountAPINameCbId.getValue().trim();
...
*/			
			...
			if(isAdd){
				...
				selectedVmta.setAccountAPIName(vmtaAccountAPIName);
			...
			vmtaDaoForDML.saveOrUpdate(selectedVmta);
			//MessageUtil.setMessage("VMTA details updated successfully.", "color:blue;", "TOP");
	...
	private class MyRenderer implements RowRenderer {
		...
		public void render(Row row, java.lang.Object data,int arg2) {
			...
			if(data instanceof Vmta) {
				...
				new Label(vmta.getAccountAPIName()).setParent(row);
