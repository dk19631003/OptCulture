...
public class SMSRecipientActivityReportController extends GenericForwardComposer{
	...
	public SMSRecipientActivityReportController(){
		smsCampaignReport = (SMSCampaignReport)Sessions.getCurrent().getAttribute("smsCampaignReport");
		this.smsCampaignSentDao = (SMSCampaignSentDao)SpringUtil.getBean("smsCampaignSentDao");
	 ...
	 public void onClick$exportBtnId() {
		...
		public void export(Combobox fileType) throws Exception{
			...
			if(type.equalsIgnoreCase("csv")){
		        ...
		        long totalContacts  = smsCampaignReport.getClicks();
		        int size = 1000;
		        ...
		        if(!isOpenedClicked) {
		        	...
		        	totalContacts  = smsCampaignSentDao.getCount(smsCampaignReport.getSmsCrId(), statusType);
		        }
				...
				for(int i=0;i < totalContacts; i+=size) {
					...
					if(!isOpenedClicked) {
						rcptRepList = smsCampaignSentDao.getRepByStatus(smsCampaignReport.getSmsCrId(), statusType, i, size);
						...
						if(isOpen) {
							rcptRepList = smsCampaignSentDao.getOpenedMobilesByCrId(smsCampaignReport.getSmsCrId(), i, size);
						}else{
							
							rcptRepList = smsCampaignSentDao.getClickedMobilesByCrId(smsCampaignReport.getSmsCrId(), i, size);
						}
				 ...
				 Filedownload.save(sb.toString().getBytes(), "text/plain", "SMSRecipientActivityReport.csv");
		...
		private void getPending() {
			...
			int totCount = smsCampaignSentDao.getCount(smsCampaignReport.getSmsCrId(), (String)repPagingId.getAttribute("type"));
	...
	*/
	...
	public void getClicks(){
		...
		int clickCount = smsCampaignReport.getClicks();
		...
		super.onEvent(event);
			...
			if(event.getTarget() instanceof Toolbarbutton){
				...
				if(toolBarBtnAttr.equalsIgnoreCase(STATUS_DELIVERED)){
					...
					int totCount = smsCampaignSentDao.getCount(smsCampaignReport.getSmsCrId(), type);
					repPagingId.setTotalSize(totCount);
		...
		try {
			...
			for(;count>0;count--){
			 ...
			 if(type.equalsIgnoreCase("opened")) {
				...
				List<Object[]> list = smsCampaignSentDao.getOpenedMobilesByCrId(smsCampaignReport.getSmsCrId(),start_index,_size);
				...
				List<Object[]> list = smsCampaignSentDao.getClickedMobilesByCrId(smsCampaignReport.getSmsCrId(),start_index,_size);
				...
				List<Object[]> list = smsCampaignSentDao.getRepByStatus(smsCampaignReport.getSmsCrId(),type, start_index,_size);
				logger.debug("the list size is====>"+list.size());
				Listitem li;
				for (Object[] obj : list) {
	...
	private void doPopUp(long sentId,String mobileNumber,String type){
		try {
			...
			if(type.equals(TYPE_CLICKED)){
				...
				List<Object[]> clickList = smsClicksDao.getClickInfoBySentId(sentId);
				Listitem li ;
				Listcell lc ;
				Label lbl ;
	...
	public int checkForPendingAndStatusPending(String status){
		int count = smsCampaignSentDao.getCount(smsCampaignReport.getSmsCrId(),status);
