...
public class WABounceReportController extends GenericForwardComposer{
	...
	public WABounceReportController() {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		List<Object[]> catList = waBouncesDao.getAllBounceCategories(waCampaignReport.getWaCrId());
	  ...
	  logger.info("waCampaignReport.getWaCrId(), defaultType"+waCampaignReport.getWaCrId()+"....."+ defaultType);
	  int totCount = waBouncesDao.getCount(waCampaignReport.getWaCrId(), defaultType);
...
public void onClick$undeliveredTbarBtnId(){
	...
	public void export(Combobox fileType) throws Exception{
		...
		if(type.equalsIgnoreCase("csv")){
			...
			long totalContacts  = waBouncesDao.getCount(waCampaignReport.getWaCrId(), statusType);
			...
			for(int i=0;i < totalContacts; i+=size) {
				...
				if((Boolean) bouncePagingId.getAttribute("OtherCat")) {
						List<WABounces> bounceContList = waBouncesDao.getBouncesCatOthers(waCampaignReport.getWaCrId(), statusType, i, size);
						
						
						
						if(bounceContList.size()>0){
							
							for (WABounces bounceCont : bounceContList) {
								 String j = "";
				...
				List<WABounces> bounceContList = waBouncesDao.getBouncesAndReasonByCat(waCampaignReport.getWaCrId(), statusType, i, size);
					
					
					
					if(bounceContList.size()>0){
						
						for (WABounces waBounces : bounceContList) {
							 String j = "";
							
							 j +=  "\""+waBounces.getMobile()+"\"" + ",";
							 j +=  "\""+waBounces.getMessage()+"\"" + ",";
							 sb.append(j + "\r\n");
						}
			 ...
			 Filedownload.save(sb.toString().getBytes(), "text/plain", "WABouncedMobiles.csv");
		}
	} // fileDownload
...
public void setReptActRepSubContent(String subContentName){
		...
		if(subContentName.equalsIgnoreCase("UnDelivered")) {
			...
			Components.removeAllChildren(bounceLbHeaderId);
			int totCount = waBouncesDao.getCount(waCampaignReport.getWaCrId(), WAStatusCodes.WA_STATUS_DELIVERY_ERROR);
			bouncePagingId.setTotalSize(totCount);
			getDefault();
			currentPage = 1;
	...
	super.onEvent(event);
		...
		if(event.getTarget() instanceof Toolbarbutton){
			...
			if(toolBarBtnAttr.equalsIgnoreCase("Other Categories") ){
				...
				int totCount = waBouncesDao.getCount(waCampaignReport.getWaCrId(), type);
				bouncePagingId.setTotalSize(totCount);
...
private void redraw(String type,int start_index, byte _size) {
	try {
		...
		if((Boolean) bouncePagingId.getAttribute("OtherCat")) {
			...
			List<WABounces> list = waBouncesDao.getBouncesCatOthers(waCampaignReport.getWaCrId(),type, start_index,_size);
			//logger.debug("the list size is====>"+list.size());
			Listitem li;
			for (WABounces obj : list) {
				...
				bounceGenericLbId.appendChild(li);
					...
					List<WABounces> list = waBouncesDao.getBouncesAndReasonByCat(waCampaignReport.getWaCrId(),type, start_index,_size);
			//	logger.debug("the list size is====>"+list.size());
				Listitem li;
				for (WABounces obj : list) {
