...
public class SingleViewContactsController extends GenericForwardComposer {
	...
	private Window singelviewContactListWinId;
...
//	private Map<Long,MailingList> mlMap;
	...
	public SingleViewContactsController() {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		PageUtil.setHeader("Single View Of Contact","",style,true);
...
*/
			...
			this.singelviewContactListWinId = singelviewContactListWinId;
			*/
	    ...
	    try {
            	...
            	if(viewType == null) {
            		...
            		for(String contactId : contactIdsArray){
            			...
            			mlSetItr = conMlList.iterator();
            			while(mlSetItr.hasNext()){
            			MailingList mailingList = mlSetItr.next();
             			contact.setMlBits(contact.getMlBits().longValue()& (~mailingList.getMlBit().longValue()));
             			logger.info("mailingList.getMlBit().longValue()-----"+mailingList.getMlBit());
                ...
                int count1=contactListLBId.getSelectedCount();
                for(;count1>0;count1--){
			...
			for(Listitem li:selSet){
				try{
    				...
    				/*	long newConBit = (contact.getMlBits().longValue() & ~mailingList.getMlBit().longValue()) | toMList.getMlBit().longValue();
    					...
    					mailingListDao.saveOrUpdate(toMList);
    					}
...
//						searchBoxId.getValue())));//(searchBoxId.getValue(), contactStatusLbId.getSelectedItem().getLabel())));
			...
			if(tb.getAttribute("CONTACT_VIEW") != null) {
				...
				Window viewScoreWinId = ((Window)singelviewContactListWinId.getFellowIfAny("viewScoreWinId"));
...
// exporting here
	...
	public void onClick$searchEmailBtnId() {
		try {
			...
			contActionsBandBoxId.setVisible(true);
			showFilter.setVisible(true);
			myPagingId.setVisible(true);
			...
			try {
				...
				if(searchLBId.getSelectedIndex()>5 )
				{
					...
					if(searchby_Date_String){
								...
								filterCountQuery = findCountQueryfromMlSet(mlSet , searchLBId.getSelectedIndex(), tempCal,null,
								emailStatusStr, mobileStatusStr,customFieldName);
								filterQuery =  findContListFromMlSet(mlSet , searchLBId.getSelectedIndex(), tempCal,null, 
								emailStatusStr, mobileStatusStr,customFieldName);
					}
					else{
							filterCountQuery = findCountQueryfromMlSet(mlSet , searchLBId.getSelectedIndex(), searchFieldStr.trim(), null,
							emailStatusStr, mobileStatusStr,customFieldName);
							filterQuery =  findContListFromMlSet(mlSet , searchLBId.getSelectedIndex(), searchFieldStr.trim(), null,
							emailStatusStr, mobileStatusStr,customFieldName);						
					}
				}
					...
					filterCountQuery = findCountQueryfromMlSet(mlSet, searchLBId.getSelectedIndex(), searchFirstStr,
							searchLastStr, emailStatusStr, mobileStatusStr, null);
...
*/	
	...
	public void onClick$resetSearchCriteriaAnchId() {
		...
		try {
			...
			myPagingId.setVisible(false);
			contActionsBandBoxId.setVisible(false);
			setDefaultMethod();
			/*if(contactListLBId.getItemCount() == 0) {
	...
	public void onClick$delContactsBtnId() {
	 ...
	 private  String findFilterContactsListCount(MailingList mList, int selectFieldIDx, String searchValStr,  String emailStatus,String mobStatus) {
		 ...
		 String subQry = "";
			...
			if(selectFieldIDx == 1) {
				subQry += " AND c.emailId like '%"+ StringEscapeUtils.escapeSql(searchValStr)  +"%'";
...
//				subQry += " AND c.contactId ="+ searchValStr  +""
				...
				if (!searchValStr.isEmpty() && !searchLastStr.isEmpty()) {
					subQry += " AND  ( c.firstName like '%" + StringEscapeUtils.escapeSql(searchValStr) + "%' AND c.lastName like '%" + StringEscapeUtils.escapeSql(searchLastStr)
							+ "%' )  ";
				} else if (!searchValStr.isEmpty() && searchLastStr.isEmpty())  {
					subQry += " AND  c.firstName like '%" + StringEscapeUtils.escapeSql(searchValStr) + "%'";
				} else if (searchValStr.isEmpty() && !searchLastStr.isEmpty()) {
					subQry += " AND c.lastName like '%" +StringEscapeUtils.escapeSql(searchLastStr)  + "%'  ";
				}
			}
//			else if(selectFieldIDx == 4) {
//				subQry += " AND c.firstName like '%"+ searchValStr  +"%'";
//			}
			//Note - index 5 was remove to match new requirement 
	    ...
	    private String findContListFromMlSet(Set<MailingList> mlSet, int selectFieldIDx, String searchValStr,  
				String searchLastStr,String emailStatus,String mobStatus,String customFieldName) {
	    	long count=0;
			...
			if(selectFieldIDx >5){
				...
				if (!searchValStr.isEmpty() && !searchLastStr.isEmpty()) {
					/*subQry += " AND  ( c.firstName like '%" + StringEscapeUtils.escapeSql(searchValStr) + "%' OR c.lastName like '%" + StringEscapeUtils.escapeSql(searchLastStr)
							+ "%' )  ";*/
					subQry += " AND  ( c.firstName like '%" + StringEscapeUtils.escapeSql(searchValStr) + "%' AND c.lastName like '%" + StringEscapeUtils.escapeSql(searchLastStr)
					+ "%' )  ";
				} else if (!searchValStr.isEmpty() && searchLastStr.isEmpty()) {
...
//				queryStr =  queryStr+ " "+subQry;
	    ...
	    private String findCountQueryfromMlSet(Set<MailingList> mlSet, int selectFieldIDx, String searchValStr,String searchLastStr,  
...
*/
	...
	private boolean validateFullName() {
		if(("eclipse-javadoc:%E2%98%82=subscriber/src%3Corg.mq.marketerFirst Name".equalsIgnoreCase(first_nameTbId.getValue().trim())) && ("Last Name".equalsIgnoreCase(last_nameTbId.getValue().trim()))||
