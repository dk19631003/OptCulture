 ...
 * @author vinod.bokare
 */
public class LoyaltyCustomerLookUpFbbController extends GenericForwardComposer<Window> implements EventListener{
	...
	private Window loyaltyCustomerLookUpFbbWinId,latestDetailsSubWinId,custExport;

	private Div datesDivId,skuDetailsDivId,promotionDetailsDivId,whichprogramDivId,custExport$chkDivId,searchCriteriaDivId,latestDetailsSubWinId$descDivId;
	...
	private Div latestDetailsSubWinId$viewPurchaseDetailsDivId,addTypeDivId,transferLblDivId,transferTableDivId, editContactDivId;
	private Radiogroup latestDetailsSubWinId$multipleMatchesRgId;
	private Label nameLblId,membershipLblId,moblieNumberLblId,emailAddressLblId, membershipMSummaryLblId,membershipStatusMSummaryLblId,optInDateMSummaryLblId,sourceMSummaryLblId,subsidiaryNumberMSummaryLblId, storeNumberMSummaryLblId,
	...
	giftAmountRedeemedTSummaryLblId,holdBalLbId,holdBalValLbId;
	private Label latestDetailsSubWinId$searchCriteriaLblId,latestDetailsSubWinId$closedCardLblId,latestDetailsSubWinId$descLBId;
	...
	private Textbox searchByTbId,subTbId,first_nameTbId,last_nameTbId;
	private Label programNameLbId,cardSetNameLbId,descpopupId$descLableId;
	private Button addBtnId;
	private Listbox transListLbId,timeDurLbId,searchbyLbId,pageSizeLbId,addTypeLbId,transferListLbId;
	...
	private Listitem listItemHoldAmt;
	...
	private Textbox ltyPwdTbId, firstNameTbId, lastNameTbId, emailTbId, phoneTbId;
	...
	private A viewDashboardAnchId;
	/**
	 * default constructor
	 */
	public LoyaltyCustomerLookUpFbbController(){
		super();
		currUser = GetUser.getUserObj();
		String style = "font-weight:bold;font-size:15px;color:#313031;font-family:Arial,Helvetica,sans-serif;align:left";
		PageUtil.setHeader("Look-up & Redeem", Constants.STRING_NILL, style, true);
	...
	public void doAfterCompose(Window comp) throws Exception {
		...
		//CountryCodeDao countryCodesDao = (CountryCodeDao)ServiceLocator.getInstance().getDAOByName(OCConstants.COUNTRY_CODES_DAO);
		//userCurrencySymbol = Utility.countryCurrencyMap.get(currUser.getCountryType());
		//countryCodesDao.findCurrencySymbolByCountry(currUser.getCountryType());
		...
		//setting view dashboard to true
		long orgOwnerUserId=orgOwner.getUserId();
		long currUserId=currUser.getUserId();
		logger.info("orgOwner.getUserId() "+orgOwner.getUserId());
		logger.info("currUser.getUserId() "+currUser.getUserId());
		if(orgOwnerUserId == currUserId) {
			logger.info("org owner = curr user");
			viewDashboardAnchId.setVisible(true);
		}
		if(orgOwnerUserId != currUserId) {
			currUser = orgOwner; //APP-1015 (modified long)
		}
...
/*	    String currSymbol = currUser.getCountryCurrency();
	...
	public void onClick$filterBtnId(){
		...
		loyaltyCustomerLookUpFbbWinId.setHeight((900+transferListLbId.getChildren().size()*30+heightForTrans)+"px");*/
		displayChangingTier();
...
*/
	...
	public void onSelect$pageSizeLbId(){
		...
		try {
			...
			loyaltyCustomerLookUpFbbWinId.setHeight((900+heightForTrans+transferListLbId.getChildren().size()*30)+"px");
	...
	public void onClick$viewDashboardAnchId() {
		logger.debug(">>>>>>>>>>>>> entered in view Dashboard");
		Redirect.goTo(PageListEnum.EMPTY);
		//Redirect.goTo(PageListEnum.ADMIN_DASHBOARD);
			...
			try {
				...
				while(resultSet.next()){
					...
					if(indexes[2]==0) {
						...
						if((resultSet.getString("docsid") != null && !resultSet.getString("docsid").isEmpty()) && (resultSet.getString("receipt_number") == null || resultSet.getString("receipt_number").isEmpty()) ){
...
//			transaction.setEventTriggStatus(OCConstants.LOYALTY_TRANSACTION_STATUS_NEW);
	...
	public void onClick$verifyBtnId(){
		...
		/*if(response!=null) {
			...
			errorMsg = errorMessage(response);
			
		}else {
			errorMsg = "not valid request";
		}*/
