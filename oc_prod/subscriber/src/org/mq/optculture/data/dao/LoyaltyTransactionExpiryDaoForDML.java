...
public class LoyaltyTransactionExpiryDaoForDML extends AbstractSpringDaoForDML{
	...
	public void resetAmountExpiryTransValues(Long loyaltyId, String expDate, String rewardFlag, boolean considerBonus){
		
		String queryStr = " UPDATE loyalty_transaction_expiry "
				+ " SET expiry_amount = 0 WHERE loyalty_id = "+loyaltyId
				+ " AND AND special_reward_id IS NULL "+(!considerBonus ? " AND bonus_id IS NULL" : "") +" AND reward_flag = '"+rewardFlag+"'"
				+ " AND (expiry_points > 0 OR expiry_amount > 0)"
				+ " AND STR_TO_DATE(concat(year(created_date),'-', month(created_date)), '%Y-%m') <= STR_TO_DATE('"+expDate+"', '%Y-%m')";
...
public void resetAmountExpiryTransValues(Long loyaltyId, String expDate, String rewardFlag, String bonusIds){
		
		String queryStr = " UPDATE loyalty_transaction_expiry "
				+ " SET expiry_amount = 0 WHERE loyalty_id = "+loyaltyId
				+ " AND AND special_reward_id IS NULL AND bonus_id  in("+bonusIds+") AND reward_flag = '"+rewardFlag+"'"
				+ " AND (expiry_points > 0 OR expiry_amount > 0)"
				+ " AND STR_TO_DATE(concat(year(created_date),'-', month(created_date)), '%Y-%m') <= STR_TO_DATE('"+expDate+"', '%Y-%m')";
	...
	public void resetAmtAndPtsExpiryTransValues(Long loyaltyId, String expDate, String rewardFlag, boolean considerBonus){
...
public void resetAmtAndPtsExpiryTransValues(Long loyaltyId, String expDate, String rewardFlag, String bonusIds){
...
public void resetAmtAndPtsExpiryTransValues(Long loyaltyId, String expDate, String rewardFlag, Long bonusID){
	...
	public  int transferSourceExpiryTrxnsToDestMembership(Long sourceMembershipLoyaltyId, Long destMembershipLoyaltyId, String transferedOn, Long userId) throws Exception{//APP-4728 tune-up
		
		String qry = " UPDATE LoyaltyTransactionExpiry SET "
				+ "transferedTo="+destMembershipLoyaltyId+", transferedOn='"+transferedOn+"'   WHERE loyaltyId="+sourceMembershipLoyaltyId+" AND userId="+userId;
		
		logger.info("transferSourceExpiryTrxnsToDestMembership == "+qry);
	...
	public  int updateAllChildTrxnsToDestMembership(Long sourceMembershipLoyaltyId, Long destMembershipLoyaltyId, String transferedOn, Long userId) throws Exception{//APP-4728 tune-up
		
		String qry = " UPDATE LoyaltyTransactionExpiry SET transferedTo="+destMembershipLoyaltyId+", transferedOn='"+transferedOn+"' WHERE transferedTo="+sourceMembershipLoyaltyId+" AND userId="+userId;
		
		logger.info("updateAllChildTrxnsToDestMembership == "+qry);
		int count = executeUpdate(qry);
