...
public class MembershipProvider {
	...
	public synchronized ContactsLoyalty getMembership() {
    	...
    	List<ContactsLoyalty> loyaltyList = getMemberships(loyaltyProgram.getUserId(), loyaltyProgram.getProgramId(), currentSizeInt, 1000);
...
public synchronized LoyaltyTransactionExpiryUtil getMembershipForBonusExpiry() {
    	...
    	List<LoyaltyTransactionExpiryUtil> loyaltyList = getMembershipsForBonusExpiry(loyaltyProgram.getUserId(), loyaltyProgram.getProgramId(), currentSizeInt, 1000);
...
private List<LoyaltyTransactionExpiryUtil> getMembershipsForBonusExpiry(Long userId, Long programID, int currentSize, int size){
	...
	private List<ContactsLoyalty> getMemberships(Long userId, Long programID, int currentSize, int size){
		...
		try{
			...
			return loyaltyDao.getAllOCMemberships(userId, programID, currentSize, size);
