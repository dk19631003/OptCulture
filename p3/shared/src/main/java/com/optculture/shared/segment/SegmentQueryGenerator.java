...
public class SegmentQueryGenerator {
	...
	private String interactionSelectQuery = "";
	private static List<String> interactionWhereFields = new ArrayList<>();
	...
	private boolean isInteractionSegmentsSelected = false;
	private boolean fetchCouponNotIssuedContacts = false;
	...
	static {
		...
		contactWhereFields.add("loyalty.total_giftcard_amount");
		...
		salesHavingFieldsMap.put("aggr_avg", "(SUM((sal.sales_price * sal.quantity) + sal.tax - (IF(sal.discount is null, 0, sal.discount))) / COUNT(DISTINCT sal.doc_sid))");
		salesHavingFieldsMap.put("aggr_count", "COUNT(DISTINCT sal.doc_sid)");
		salesHavingFieldsMap.put("aggr_max", "(SUM((sal.sales_price * sal.quantity) + sal.tax - (IF(sal.discount is null, 0, sal.discount))) / COUNT(DISTINCT sal.doc_sid))");
		salesHavingFieldsMap.put("aggr.tot_purchase_amt", "SUM((sal.sales_price * sal.quantity) + sal.tax - (IF(sal.discount is null, 0, sal.discount)))");
		salesHavingFieldsMap.put("aggr.avg_purchase_amt", "(SUM((sal.sales_price * sal.quantity) + sal.tax - (IF(sal.discount is null, 0, sal.discount))) / COUNT(sal.cid))");
		salesHavingFieldsMap.put("aggr.max_purchase_amt", "(MAX((sal.sales_price * sal.quantity) + sal.tax - (IF(sal.discount is null, 0, sal.discount))))");
		salesHavingFieldsMap.put("if((year(now())-year(c.created_date))=0,aggr.tot_purchase_amt,aggr.tot_purchase_amt/(year(now())-year(c.created_date)))", "(SUM((sal.sales_price * sal.quantity) + sal.tax - (IF(sal.discount is null, 0, sal.discount))) / COUNT(DISTINCT sal.doc_sid))");
		...
		couponsWhereFields.add("date(cd.expired_on)");
		couponsWhereFields.add("cp.coupon_expiry_date");
		
		interactionWhereFields.add("ce.event_type");
		...
		tokensMap.put("string:is not in active", " <COLUMN_NAME> NOT IN (<VALUE>)");
		...
		tokensMap.put("number:is in", " <COLUMN_COND> IN (<VALUE>)");
		...
		tokensMap.put("date:isetoday", " DATEDIFF('dd', <DATE_FIELD>, now()) = 0 " );
		tokensMap.put("date:isequal", " <DATE_FIELD> = '<VALUE>'");
		tokensMap.put("date:isebetween", " <DATE_FIELD> BETWEEN '<VALUE1>' AND '<VALUE2>'");
		tokensMap.put("date:iseonOrAfter", " <DATE_FIELD> >= '<VALUE>'");
		tokensMap.put("date:iseonOrBefore", " <DATE_FIELD> <= '<VALUE>'");
		tokensMap.put("date:iseafter", " <DATE_FIELD> > '<VALUE>'");
		tokensMap.put("date:isebefore", " <DATE_FIELD> < '<VALUE>'");
...
//		tokensMap.put("date:ignoreyear_between", "(((toMonth(<DATE_FIELD>), toDayOfMonth(<DATE_FIELD>)) >= (toMonth(Date('<VALUE1>')), toDayOfMonth(Date('<VALUE1>')))) OR ((toMonth(<DATE_FIELD>), toDayOfMonth(<DATE_FIELD>)) <= (toMonth(Date('<VALUE2>')), toDayOfMonth(Date('<VALUE2>')))))");
	...
	public String getSegmentQuery(String segmentRule, String type) {
		...
		contactSelectQuery = (segmentRule.contains("loyalty.")) ? contactSelectQuery.replace("<CONTACT_LEFT_JOIN>", "") : contactSelectQuery.replace("<CONTACT_LEFT_JOIN>", "LEFT");
		...
		if (isCouponSegmentsSelected) {
			...
			contactSelectQuery += " AND c.cid <NOT-ISSUED-CONTACTS> IN (" + couponsSelectQuery + couponsWhereCondAND.toString() + couponsHavingStr +")";
			contactSelectQuery = contactSelectQuery.replace("<NOT-ISSUED-CONTACTS>", fetchCouponNotIssuedContacts ? "NOT" : "");
		}

		if (isInteractionSegmentsSelected)
			contactSelectQuery += " AND c.cid IN (" + interactionSelectQuery + ")";

		finalQuery = contactSelectQuery.replace("<USER_ID>", userIdStr);
	...
	private void processSegmentRuleAND(String andSegmentRule) {
		...
		case "INTERACTION":
			generateInteractionRule(orSegmentsArr);
			isInteractionSegmentsSelected = true;
			break;
	...
	private String findSegmentAttributeType(String segmentField) {
		...
		else if (interactionWhereFields.contains(segmentField))
			return "INTERACTION";
	...
	private void generateCouponsRule(String[] orSegmentsArr) {
		...
		for (String orSegRule : orSegmentsArr) {
			...
			String token = segmentTokenArr[2].toLowerCase().trim();
			if (token.equals("string:is not in active"))
				fetchCouponNotIssuedContacts = true;
	...
	private void generateInteractionRule(String[] orSegmentsArr) {
		
		String[] ruleArr = orSegmentsArr[0].split("\\|");
		
		String query = "SELECT DISTINCT contact_id  FROM communication_event ce WHERE event_type = 'Sent' AND campaign_id IN (<CAMPAIGNS>) AND contact_id not IN (<INNER-QUERY>)";
		String innerQuery = "SELECT DISTINCT c2.contact_id FROM communication_event c1 JOIN ("
				+ "SELECT * FROM communication_event WHERE campaign_id IN (<CAMPAIGNS>) AND event_type = 'Sent') c2 "
				+ "ON c1.campaign_id = c2.campaign_id AND c1.recipient = c2.recipient AND c1.cr_id = c2.cr_id WHERE c1.event_type ";

		String value = ruleArr[3];
		innerQuery += ruleArr[2].contains(" in") ? "IN " + value : "= " + value;

		if (ruleArr[2].contains("not")) {
			query = query.replace("<INNER-QUERY>", innerQuery);
		} else {
			query = innerQuery;
		}
		interactionSelectQuery = query.replace("<CAMPAIGNS>", ruleArr[0]);
	}
	...
	private String replaceTokensAndGetRule(String[] segmentTokenArr) {
		...
		String value = "";
		if (segmentTokenArr[0].equalsIgnoreCase("Promo-code Status") || segmentTokenArr[0].equalsIgnoreCase("Promo-code Date"))
			value = segmentTokenArr[3].trim();
		else
			value = segmentTokenArr[3].trim().replace("'", "\\'");
		...
		if (segmentTokenArr[1].toLowerCase().contains(".coupon_id"))
			...
			mapStr = mapStr.replace("<COLUMN_NAME>", "TRIM(lowerUTF8(" + segmentTokenArr[1] + "))");
		...
		if (segmentTokenArr[1].equalsIgnoreCase("c.gender")) {
			...
			else if (value.equalsIgnoreCase("female") || value.equalsIgnoreCase("f"))
				tempStr = "lowerUTF8(c.gender) IN ('female', 'f')";
			else
				tempStr = "(c.gender != '' AND lowerUTF8(c.gender) NOT IN ('male', 'm', 'female', 'f'))";
		} else
